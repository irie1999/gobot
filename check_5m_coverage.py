    ...
    for s_ in syms:
        ...
        try:
            ...
            if pd.Timestamp(tgt) in d.index:
                ...
                # ⛔ 高値・安値の一致は『全部カバーしている』証明にはならない。
                #    その日の極値が欠損窓の外(=日中)に付いただけかもしれない。
                #    言えるのは「欠損窓は新しい極値を作らなかった」まで。
                if abs(dh - mh) < 0.51 and abs(dl - ml) < 0.51:
                    print("  → 日中の高値・安値は5分足にある = **欠損窓は その日の極値を作らなかった**。")
                    print("     ただし『欠損窓が見えている』ではない。窓の中で損切り/利確に")
                    print("     触れていた可能性は残る(極値でなくても触れうる)。")
                else:
                    print("  → 高値または安値が5分足に無い = **その値は欠損窓で付いた**。")
                    print("     欠損窓に実際の値動きがある証拠。")
