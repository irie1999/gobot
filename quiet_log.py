...
_KEEP_TAGS = (
    ...
    # ⛔⛔ 2026-09-07: `[N]` が入っていなかったので、**2つの重要な行が
    #   毎回消えていた**:
    #     [N] 実際に取れた期間 …   ← 窓が本当に取れたかの唯一の確認(§18.53)
    #     [N] テキストレポート → … ← n_report_<日付>.txt の出力先
    #   `[新方式N]` は別のタグで、部分一致にもならない。
    "[N]",
    # ★ 節目ごとの通算経過秒(`[⏱   6.1s] シグナルタブ完了`)。上の _DROP_RE で
    #   落としていたものを残す。工程別の総括だけでは **計測外の時間**が見えない
    "[⏱",
)
# ⛔ 出力ファイル名は html だけでなく txt / csv も残す(N のテキストレポートと
#   日次CSVの行が消えて「どこに出るの?」となった)
_KEEP_RE = re.compile(r"⛔|⚠|❌|✅|失敗|Traceback|エラー|\.(html|txt|csv)\b")
...
# 進捗行: 先頭が「…」や「  … 200/1540」
# ⛔⛔ 2026-09-07: ここが `^\s*\[⏱\s+\d` で **区切りの経過時間を全部 落として
#   いた**。run_signals_holdout_all の `_phase()` は
#       [⏱   6.1s] シグナルタブ完了
#   の形で節目ごとに **通算の経過秒**を出す。これが唯一「どこで時間が消えたか」
#   を示す情報だったのに、静音時は1行も見えなかった。実際 2026-09-07 に
#   「計測分 26.8s / ★全体 114.2s」で **87秒が行方不明**になり、
#   その答えはこの落としていた行の中にあった。
#   9行しか出ないので残す。
