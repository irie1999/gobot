...
#
#   ⛔ **既定は OFF(0/0 = 時間決済のみ)**。理由は3つ (2026-08-10):
#
#   1. **転換タブは『記録』であって戦略ではない**。実際には損切り注文を出していない。
#      OCO を既定で入れると `.\tenkan`(実取引日の記録) が仮想の話に変わり、
#      18.5.2 の「レポートの転換タブと .\tenkan は完全に同一ロジック」も崩れる。
#
#   2. **lss の 0.1/1.0 をそのまま持ってくるのは不適切**。lss の 0.1ATR は
#      『注文価格(前日終値)から』measure する設計値だが、転換は 09:09 の**成行約定値**
#      から測る。0.1ATR ≒ 株価の0.3% なので、成行で買って0.3%下げたら即撤退 =
#      ほぼ全件が数分で損切りになる。一方 利確1.0ATR(2〜3%)を2時間20分で取るのは稀。
#      **片方だけがほぼ確実に発火する非対称**で、検証になっていない。
#
#   3. **ロング+OCO は既に否定されている**。18.32 でロング版 G(寄成ロング+OCO)の
#      sm/tm を 4×5升 掃いたら **20/20 すべてマイナス**。方向の問題なので
#      パラメータで救える見込みは薄い。
#
#   ★ 使うなら必ず TRAIN/TEST を分けて掃くこと。単一窓で良い升を選んでも過剰適合
#      (18.28 の教訓)。18.26 の 09:05締切では OCO(0.1,1.0)@15:25 が
#      時間決済のみ(−4,361,866)より マシ(−2,342,093) だったが、
#      **決済時刻も同時に変えた比較**なので OCO 単独の効果ではない。
SM = float(os.environ.get("TENKAN_SM", "0") or 0)
TM = float(os.environ.get("TENKAN_TM", "0") or 0)
