...
def run_limit_backtest(
    ...
    #
    # ⛔⛔ **日付は「判定日」で固定する**(2026-08-25 修正)。
    #   旧: entry_dt = df.index[-1] = **実行したときの最終バー**。
    #   これだと同じ1件の注文が、レポートを回す日によって別の日付になる。
    #   E/H/J の母集団は `eh_trades.py:275` が `trades + nofills` から
    #   (銘柄,日) を拾うので、**過去日の未発動シグナルが回すたびに消える**。
    #
    #   実例 3382 セブン＆アイ: 08-21 のシグナル → 逆指値 2,030.5 に対し
    #   08-24 の安値 2,031.5 で 1円届かず未発動。
    #     08-24 の夕方に回す → 発注中(日付=08-24) → J が採点 → 取引ができる
    #     08-25 の夕方に回す → 発注中(日付=08-25) → **(3382,08-24) が消える**
    #   ライブは 09:00 の始値で判定するので普通に約定している(実損益 +60円)。
    #   実測(audit_population.py): 抜け率が古い日ほど単調に増える
    #   08-25 18% / 08-24 29% / 08-19 38% / 08-18 56% / 08-17 100%。
    #
    #   → 判定日 = fill_start_idx (= シグナル翌営業日 + entry_delay) に固定する。
