...
# ★ 同一銘柄で、隣接する結合レコード間の比が跳ぶ件数。
#   分割は 2/3/4/5/10 倍なので、1.8倍以上 or 0.55倍以下の跳びを数える。
#   ⚠ 信用残が実際に週で倍増することはある(急騰銘柄)。**件数の水準**で
#     判断する: 全体の数%なら正常、10%を超えるなら基準ズレを疑う。
_jj = jd.sort_values(["symbol", "date"]).copy()
_pv = _jj.groupby("symbol")["buy_days"].shift(1)
_rt = (_jj["buy_days"] / _pv).replace([np.inf, -np.inf], np.nan)
_jmp = _rt[(_rt >= 1.8) | (_rt <= 0.55)]
_nv = int(_rt.notna().sum())
_pc = len(_jmp) / max(1, _nv) * 100.0
print(f"    隣接レコードで比が跳ぶ(>=1.8倍 or <=0.55倍) {len(_jmp):,}/{_nv:,}件 "
      f"({_pc:.1f}%)")
if _pc > 10.0:
    print("    ⛔ **10%超。信用残(未調整)と出来高(調整済)の基準ズレを疑って"
          "ください**")
    print("       分割銘柄では 買残日数が分割比ぶん化けます。"
          "以下の数字は信用できません")
else:
    print("    ✅ 水準としては妥当(急騰銘柄の残高増でもこの程度は出ます)")
