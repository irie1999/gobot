...
# ══════════════════════════════════════════════════════════════════
# ★★ 貸借区分 — これは「選別軸」ではなく **水準そのもの** の話
# ══════════════════════════════════════════════════════════════════
#   貸借銘柄 … 制度信用の「売り」ができる  → N を実運用で建てられる
#   信用銘柄 … 制度信用の「買い」しかできない → **空売りできない**
#   ⛔ バックテストは信用銘柄も売っている。実運用では存在しないトレード。
#   ★ だから「除外して良くなるか」を測るのではない。除外した後の数字が
#     **実運用で取れる本当の水準**。良くなっても悪くなっても、そちらが正。
if "IssType" in jd.columns:
    print("\n" + "=" * 74)
    print(" ★★ 貸借区分 — バックテストに『建てられない銘柄』が混ざっていないか")
    print("=" * 74)
    _LBL = {1: "信用銘柄(制度の売建 不可)", 2: "貸借銘柄(売建 可)",
            3: "その他"}
    ...
    # ★ 貸借銘柄だけに絞ったら水準がどう変わるか
    _lend = tr[tr["IssType"] == 2]
    if len(_lend) and len(_lend) < len(tr):
        _d = float(_lend["bp"].mean()) - _tot_bp
        print(f"\n   TRAIN 全体            {_tot_bp:+.1f}bp/件")
        print(f"   **貸借銘柄だけ**       {_lend['bp'].mean():+.1f}bp/件  "
              f"({_d:+.1f}bp)")
        print(f"   → 実運用で取れるのは **下の行** です。"
              f"バックテストは {_d * -1:+.1f}bp ぶん "
              + ("過大" if _d < 0 else "過小") + "に出ています")
    print("   ⛔ これは選別軸の検定ではありません(除外して良くなるかを"
          "測っているのではない)。")
    print("      建てられない銘柄を外した後が **本当の水準** です。")
    print("   ⚠ 一般信用(デイトレ)なら売建できる銘柄もあるので、"
          "実際の可否は証券会社によります")
