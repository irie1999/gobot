...
def _newgap_capital_sweep(rows: list, side: str) -> list:
    ...
    if _qs2:
        ...
        _w("     ★ 2つの別の話が混ざるので分ける:")
        _w("       (a) 資金は400万のまま株数だけ増やす")
        _w("           → 建てられる **銘柄数が減る**(分散が落ちる) + レバレッジ")
        _w("       (b) 資金も比例して増やす(800万/200株 等)")
        _w("           → **同じ銘柄・同じ日**をそのまま倍にするだけ。")
        _w("             損益もσも最悪日も **ちょうど倍**になる恒等式で、")
        _w("             ÷σ は定義上まったく変わらない(測るまでもない)。")
        _w("     ⛔ どちらも損益と同じだけ事故も増える。空売り在庫・部分約定・")
        _w("        板への影響が変わるので、100株で測った実約定は転用できない。")
        _h5 = (f"    {'方式':<18}{'件数':>8}{'合計':>14}{'月平均':>11}{'月次σ':>11}"
               f"{'÷σ':>7}{'最悪日':>12}{'最大1銘柄':>12}{'100株比':>9}")
        _w(_h5)
        _w("    " + "-" * (len(_h5) - 4))
        _b100 = None
        # (a) 資金固定 / (b) 資金も比例
        _plan5 = ([(400.0, _NG_QTY, "")]
                  + [(400.0, _q2, "(a)") for _q2 in _qs2]
                  + [(400.0 * _q2 / _NG_QTY, _q2, "(b)") for _q2 in _qs2])
        for _b5, _q2, _tag in _plan5:
            try:
                _s = _newgap_sim(rows, _b5, _NG_WATCH, _NG_GAP_BP, _NG_RET1,
                                 qty_mode="fixed", qty=_q2, max_pct=0.0)
                if not _s or _s["days"].empty:
                    continue
                _o = _stat(_s["days"], _s["det"], _b5)
                _dt2 = _s["det"]
                _big = (float((_dt2["entry_p"] * _dt2["qty"]).max())
                        if not _dt2.empty else 0.0)
                if _b100 is None:
                    _b100 = _o["tot"]
                _rt = (_o["tot"] / _b100) if _b100 else 0.0
                _w(f"    {_tag + f'{_b5:,.0f}万/{_q2}株':<18}{_o['n']:>8,}"
                   f"{_o['tot']:>+14,.0f}{_o['mu']:>+11,.0f}{_o['sd']:>11,.0f}"
                   f"{_o['ratio']:>7.2f}{_o['worst']:>+12,.0f}{_big:>12,.0f}"
                   f"{_rt:>8.2f}倍")
            except Exception as _e:                   # noqa: BLE001
                _w(f"    {_b5:,.0f}万/{_q2}株: ⛔ {str(_e)[:50]}")
        _w("")
        _w("    (b) の『100株比』が株数比とぴったり一致していれば恒等式どおり。")
        _w("    ⚠ ここで問うべきは『儲かるか』ではなく **どこまで負けを許せるか**。")
        _w("       最悪日と MaxDD も同じ倍率で増えます(§18.38 #3b)。")
        _w("")
    return _L
