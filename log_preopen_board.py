...
⛔ ただし寄り前の実測は **0.3〜0.6件/秒**(§18.44)なので、1時間で読めるのは
   **1,000〜2,000銘柄が上限**。しかもそれは1周ぶんで、精度に効く「寄り直前の
   読み」を全銘柄ぶん取るのは無理。**候補(数百銘柄)に絞るのが現実解**。
...
■ ★ 母集団は広く取る (2026-08-16 ユーザー提案)
  ...
  ⛔⛔ 2026-08-17 訂正: **広げる前提が崩れた**。寄り前の実測は 0.3〜0.6件/秒
  (§18.44)で、1,500銘柄なら1周 **約50分**。08:00〜08:45 では1周も終わらない。
  → 既定は **今日の候補(`k_signals_<日付>.csv`・数百銘柄)**。1周10分で4周取れ、
    300件/日なら判定に要る1,000件に **4営業日**で届く。
  無ければ cumul → holdout → full → prime の順。`--max-symbols` の既定は 0=無制限。
...
#
# ⛔⛔ 2026-08-17 訂正: **母集団を広げる前提が崩れた**。
#   昨日は「6.5件/秒(場外実測)だから1時間で3,900銘柄」と計算して 1,500銘柄に
#   広げたが、寄り前の実測は **0.3〜0.6件/秒**(§18.44)。1,500銘柄なら1周に
#   **約50分**かかり、08:00〜08:45 では1周も終わらない。
#   → **今日の候補(数百銘柄)に絞る**のが正解。1周10分で4周取れるし、
#     300件/日なら判定に要る1,000件に **4営業日**で届く(当初想定と同じ)。
#   広げたいときだけ --symbols-file で明示する。
_SRC_CHAIN = [
    (f"k_signals_{_dt.date.today():%Y%m%d}.csv", "★今日の候補(推奨)"),
    ("lss_proposal_cumul.py", "累積マージ(選定あり)"),
    ("holdout_selected_symbols.py", "レポートの選定銘柄"),
    ("lss_proposal_full.py", "選定なしの全候補(⚠1周50分)"),
    ("symbols_listed_prime.py", "プライム全銘柄(⚠さらに遅い)"),
]
