...
# ══════════════════════════════════════════════════════════════════════
# ★★ 大負けする日は「どの段階で」分かるか (2026-09-04 ユーザー質問)
# ══════════════════════════════════════════════════════════════════════
#   ⛔ 上の `_worst_table` は **引けの結果で日を選んでいる**ので事後選択。
#     こちらは **時刻Tまでの情報だけで選ぶ**ので、そのままルールになる。
#
#   測るのは2つ。混同しないこと:
#     ① 見分けられるか … Tの含み損益 と 引け の相関 / 下位K日の的中率
#     ② 見分けて得か   … Tで下位に選んだ日の **T→引けの増分**
#
#   ★★ ①が高くても②が正なら **降りてはいけない**。「悪い日は戻す」
#     (§18.62: 正解率45〜57% / §18.55: 早く降りるほど悪い) が時刻の関数で
#     出るはず。②が負に転じる時刻があれば、そこが初めて候補になる。
#
#   ⚠ 実装するときの注意: 「下位20%」は分布を知らないと決まらないので、
#     実運用では **円の閾値を先に固定する**ことになる。それは §18.62 の
#     固定円ポートフォリオ損切りそのもの(全水準で否定済み)。ここで見るのは
#     『閾値を完璧に置けたとしても勝てるか』という **上限**。
print(f"\n{'=' * 96}")
print(f"■ ★★ 大負けする日は、どの段階で分かるか")
print(f"{'=' * 96}")

if len(_c10) < 20:
    print(f"  ⛔ {a.worst_n}件以上の日が {len(_c10)}日しかないので判定できません")
else:
    _NN = len(_c10)
    _KK = max(1, int(round(_NN * 0.20)))
    _p0 = _KK / _NN
    _sd = (_p0 * (1 - _p0) / _KK) ** 0.5
    _fin_all = sum(_days[d][2] for d in _c10)
    _wc = set(sorted(_c10, key=lambda d: _days[d][2])[:_KK])   # 引けで下位K日
    print(f"  母集団 {a.worst_n}件以上の {_NN}日 / 引けまで持った合計 "
          f"{_fin_all:+,.0f}円")
    print(f"  「下位20%」= {_KK}日。**でたらめに選んでも的中率 {_p0 * 100:.0f}%** "
          f"なので、有意なのは **{(_p0 + 2 * _sd) * 100:.0f}% 超**だけ")
    print(f"\n  {'時刻':<8}{'相関':>7}{'R²':>7}{'的中率':>8}"
    ...
    for _t in _WT:
        ...
        _ys = np.array([_days[d][2] for d in _c10], dtype="float64")
        _r = (float(np.corrcoef(_xs, _ys)[0, 1])
              if _xs.std() > 0 and _ys.std() > 0 else float("nan"))
        _sel = set(sorted(_c10, key=lambda d: _v[d])[:_KK])
        _hit = len(_sel & _wc) / _KK * 100.0
        ...
        print(f"  {_t:<8}{_r:>+7.2f}{_r * _r:>7.2f}{_hit:>7.0f}%{_mk}"
              f"{_now:>+15,.0f}{_cls:>+12,.0f}{_dlt:>+12,.0f}"
              f"{_tot:>+13,.0f}{_tot - _fin_all:>+12,.0f}")
    print(f"  {'(引け)':<8}{1.0:>+7.2f}{1.0:>7.2f}{100:>7.0f}% "
          f"{_fin_all / _NN:>+15,.0f}{'—':>12}{'—':>12}"
          f"{_fin_all:>+13,.0f}{0:>+12,.0f}")
    print(f"  * = 的中率が でたらめの2σ を超えた時刻")
    print(f"\n  ★ 読み方")
    if _neg:
        print(f"    T→引け が **負**の時刻: {', '.join(_neg)}")
        print(f"      → その時刻に下位20%だった日は **その後さらに悪化**する。")
        print(f"        畳む価値がある可能性。『畳んだ合計』が引けを上回るか確認")
    else:
        print(f"    ⛔ **T→引け が全時刻で正**。")
