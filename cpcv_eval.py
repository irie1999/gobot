...
def _save_cpcv_flags(records: list[dict], mode_suf: str) -> None:
    """CPCV評価結果から警告フラグを cpcv_flags{mode_suf}.py に保存する。

    level:
      'danger'  — 過学習 (PBO>50%) または OOS PF<1.0 (損失)
      'warning' — 要注意 (PBO 30-50%) または OOS PF<1.3 または OOS WR<50%
    シグナル表示スクリプトがこのファイルを読んで警告バッジを付ける。
    """
    flags: dict[str, dict] = {}
    for r in records:
        sym    = r["symbol"]
        ...
        if pbo > 0.5 or oos_pf < 1.0:
            level = "danger"
        elif pbo > 0.3 or oos_pf < 1.3 or (oos_wr > 0 and oos_wr < 50.0):
            level = "warning"
        else:
            continue  # 正常銘柄はフラグなし
        ...
        reasons = []
        if pbo > 0.5:
            reasons.append(f"過学習 PBO {pbo:.1%}")
        elif pbo > 0.3:
            reasons.append(f"要注意 PBO {pbo:.1%}")
        if oos_pf < 1.0:
            reasons.append(f"OOS PF {oos_pf:.2f} (損失)")
        elif oos_pf < 1.3:
            reasons.append(f"OOS PF {oos_pf:.2f}")
        if oos_wr > 0 and oos_wr < 50.0:
            reasons.append(f"OOS勝率 {oos_wr:.1f}%")
    ...
    fname = f"cpcv_flags{mode_suf}.py"
    lines = [
        f"# Auto-generated by cpcv_eval.py --watchlist  {TODAY}\n",
        "# level: 'danger'=過学習/OOS損失  'warning'=要注意\n",
        "CPCV_FLAGS = {\n",
    ]
    for sym, info in sorted(flags.items()):
        lines.append(
            f'    "{sym}": {{"level": "{info["level"]}", '
            f'"pbo": {info["pbo"]}, '
            f'"oos_pf": {info["oos_pf"]}, '
            f'"oos_wr": {info["oos_wr"]}, '
...
def _run_watchlist(args) -> None:
    ...
    # CPCV 警告フラグ
    _save_cpcv_flags(summary_records, mode_suf)
