...
if a.list_axes:
    ...
    _AX = {
        ...
        "res60_bp": "60日高値までの余地bp — **正なら まだ高値の下**。上に抵抗線",
        "res120_bp": "120日高値までの余地bp — 同上の長い窓",
        "touch120": "同じ価格帯を通った日数(120日/±1.5%) — 出来高が溜まっているか",
        "up_streak": "連続上昇日数",
...
if a.hedge:
    ...
    # ⛔ **どの指数で測ったかを必ず出す**(2026-09-13)。ティッカーを
    #   GAP_INDEX_TICKER で差し替えられるようにしたのに、この経路では
    #   出力に出しておらず「1321.T で走ったのか ^N225 のままか」を
    #   出力から判別できなかった(§18.40b の形をまた作りかけた)。
    from preopen_market import _SAMEDAY_TICKER as _HIDX       # noqa: E402
    print(f"  指数 = **{_HIDX}**"
          + ("   ⚠ ^N225 の **始値は取引できません**(§18.71)。"
             '$env:GAP_INDEX_TICKER = "1321.T" で差し替えられます'
             if _HIDX == "^N225" else "   ✅ 取引できる指数で測っています"))
    ...
    #   **出さないと判断できない**。
    _mkm = float(_mk.mean())
    # ⛔ ラベルが紛らわしかった(2026-09-13)。**損失を負の数**で出していたので
    #   「-268円/日 失う」= 二重否定になり、得なのか損なのか読めなかった。
    #   ヘッジは日経ロング。日経が平均で下がっていれば **払う**。
    _lose = -_beta * _mkm            # 比率1.0 のときのヘッジの寄与(円/日)
    # ⚠ 月換算を ×20 で決め打ちしていた。実際の営業日/月 を使う。
    _dpm = len(_kd) / max(1.0, len(set(_d[:7] for _d in _kd)))
    print(f"    日経の日中% 平均 = **{_mkm:+.3f}%**"
          f"(§18.19 ではドリフトはゼロ) / 営業日 {_dpm:.1f}日/月")
    print(f"    → 比率1.0 のヘッジの寄与 = **{_lose:+,.0f}円/日**"
          f"(月 {_lose * _dpm:+,.0f}円)"
          + ("  ⛔ **払う側**(日経が平均で下がっているので、買うと損)"
             if _lose < 0 else "  ✅ もらう側"))
    if abs(_lose * _dpm) > abs(_pn.mean() * _dpm) * 0.3:
        print(f"      ⛔ 月平均 {_pn.mean() * _dpm:+,.0f}円 の3割を超えます。"
              f"**ヘッジは σ と一緒に利益も削ります**")
    # ★★ **先物コストをゼロにしても勝てるか**を先に出す(2026-09-13)。
    #   §18.65③ は「2.7bp を切れば黒字」と逆算したが、ドリフトのほうが
    #   大きい期間ではコストを消しても負ける。**コスト論に入る前にここを見る。**
    print(f"    ★ 先物コストを **ゼロ** にしたときの比率1.0 の月平均 = "
          f"{_pn.mean() * _dpm + _lose * _dpm:+,.0f}円 "
          f"(現行 {_pn.mean() * _dpm:+,.0f}円)")
    ...
    for _lbl, _kk, _pp, _cc, _mm in (("TRAIN", _kd, _pn, _cap, _mk),) + (
        ...
        if _lbl == "TEST":
            ...
            _r2w = 1.0 - (((_pn - np.polyval(_bt, _mk)) ** 2).sum()
                          / ((_pn - _pn.mean()) ** 2).sum())
        ...
        for _h in _hs:
            ...
            if _h > 0.0 and (_sdmin is None or _sd2 < _sdmin[1]):
                _sdmin = (_h, _sd2)
        ...
        # ★★★ **期間のミスマッチ**(2026-09-13)。ここが効く。
        #   R² は **日次** で測っている。天井 1/√(1-R²)-1 も日次の話。
        #   ところが判断に使うのは **月次σ**。日次の市場要因は月の中で
        #   上下が打ち消し合うので、月次で見ると市場の取り分はずっと小さい。
        #   → 「日次 R² が高いからヘッジの余地がある」は誤り。必ず両方出す。
        if _sdmin and _bsd:
            _cut = 1.0 - _sdmin[1] / _bsd              # 実測の月次σ 削減率
            _ceil = 1.0 - _math.sqrt(max(0.0, 1.0 - _r2w))   # 日次R² からの天井
            _r2m = 1.0 - (1.0 - _cut) ** 2             # 月次に直した実効 R²
            print(f"      ★ 日次R² {_r2w:.3f} → 天井は σ **-{_ceil * 100:.1f}%**。"
                  f"実測の月次σ 削減は 比率{_sdmin[0]:.2f} で **-{_cut * 100:.1f}%**"
                  f"(実効 月次R² {_r2m:.3f})")
            if _ceil > 0 and _cut < _ceil * 0.5:
                print(f"      ⛔ **期間のミスマッチ**。日次では {_r2w * 100:.0f}% "
                      f"説明できても、月次σ は {_cut * 100:.1f}% しか減りません"
                      f"(日次の市場要因は月の中で打ち消し合う)。")
                print(f"         判断に使うのは月次なので、**ヘッジの実際の天井は"
                      f" -{_cut * 100:.1f}%** です。日次R² から見積もらないこと。")
...
if a.sweep_cands:
    ...
    if _o50T >= 60.0:
        print(f"    ⚠ TRAIN の {_o50T:.0f}% の日が50超 = "
              f"**n_cand50 はその全部が同じ値(50)**。上位分位が巨大な塊になり、\n"
              f"      空の分位も出ます。軸としてほぼ機能しません(宣言どおり報告)")
    ...
    for _t, _p in _CP.items():
        ...
        for _k, _q in _RC:
            ...
            # ⛔ 整数の軸は分位の境界が重なる。0日の分位・巨大な分位は
            #   検定を1つ無駄にしているだけなので、その場で言う(2026-09-06)
            _deg = ("  ⛔空(境界が重複)" if _nd5 <= 0 else
                    f"  ⚠{_nd5 / max(1, len(_p['y'])) * 100:.0f}%を除外"
                    if _nd5 > len(_p["y"]) * 0.35 else "")
            print(f"    {_k:<13}{'Q' + str(_q):>4}{_nd5:>8,}"
                  f"{_st['mu']:>+11,.0f}{_st['r']:>6.2f}{_st['cvar']:>+11,.0f}"
                  f"{_st['worst']:>+11,.0f}{_st['mdd']:>11,.0f}"
                  f"{_eq:>+11,.0f}{'  ✅' if _st['mu'] > _eq else '  ⛔'}{_deg}")
...
if a.explore:
    ...
    for _ax in _want:
        ...
        # ⛔⛔ **✅ が宣言した合格ラインを実装していなかった** (2026-09-13)。
        #   :1150 で「bp≥PASS_BP / t≥2.0 / 帰無95%点超え」と宣言しながら、
        #   探索の ✅ は **帰無しか見ていなかった**。その結果 2026-09-13 の
        #   鏡像で、日t が -1.56〜+0.10 の行まで ✅ が付いて11個出た。
        #
        #   ★★ なぜ帰無だけだと通ってしまうのか(これが本質):
        #     bp は **スケール不変ではない**。帰無は日の中でラベルを入れ替える
        #     ので、分位の **ボラ構成が壊れる**。高ボラ銘柄に偏った分位は
        #     |bp| が自動的に大きくなり、ボラの混ざった帰無を必ず上回る。
        #     = **ボラと相関する軸は、エッジが無くても ✅ になる**。
        #     実測: ATR% Q5 は bp +53.0(Q1 の7倍)だが 日t は +0.10(Q2 は +2.63)。
        #   ★ 日t は mean/se なので **スケール不変**(全体が k 倍でも t は不変)。
        #     だからボラ効果はここで落ちる。帰無と日t の両方を要求する。
        _ok_bp = _b[2] >= PASS_BP
        _ok_t = _b[3] >= 2.0
        if _res["hit"] and _ok_bp and _ok_t:
            _mk = "✅ 候補"
            if _qpd == _qpd and _qpd <= 2.0:
                _mk = "⚠ 候補(較正弱)"
            _hits.append(_res)
        elif _res["hit"] and not _ok_t:
            # ボラ相関で帰無だけ超えた典型。bp が大きくても効果ではない
            _mk = f"— (帰無超えだが 日t {_b[3]:+.2f} < 2.0)"
            _null_only.append(_res)
        elif _res["hit"] and not _ok_bp:
            _mk = f"— (帰無超えだが bp < {PASS_BP})"
            _null_only.append(_res)
        else:
            _mk = "—"
        print(f"  {_res['label']:<22}{_b[0]:>6}{_b[1]:>9,}{_b[2]:>+9.1f}"
    ...
    if _null_only:
        print(f"  ⚠ **帰無だけ超えて落ちた軸 {len(_null_only)} 個**: "
              + " / ".join(f"{_r['col']}(日t {_r['best'][3]:+.2f})"
                           for _r in _null_only))
        print(f"     bp はスケール不変ではありません。帰無は日の中でラベルを"
              f"入れ替えるので **分位のボラ構成が壊れ**、高ボラに偏った分位は"
              f"\n     |bp| が自動的に大きくなって帰無を上回ります。"
              f"日t(=mean/se)はスケール不変なのでここで落ちます。"
              f"\n     ★ **ボラと相関する軸は、エッジが無くても帰無を超える。**"
              f" bp の大きさを発見と読まないこと。")
    # ⛔⛔ 候補が期待の3倍を超えたら、**軸が効いたのではなく較正が壊れている**
    #    可能性のほうが高い(2026-09-13 に鏡像で 11/21 が出た)。
    #    「たくさん見つかった」は良い報せではない。
    ...
    if _hits:
        ...
        # ★★ **候補の数 ≠ 発見の数** (2026-09-13)。軸どうしが強く相関して
        #   いると、同じ1つのことが何通りにも出る。TEST は1候補につき1回なので、
        #   同じ族から複数を confirm するのは **TEST の無駄遣い**。
        #   族は事前に固定(結果を見てから足し引きしない)。
        _FAM = {
            "動きの大きさ(前日〜20日のリターン / ボラ)":
                {"ret1", "ret2", "ret3", "ret5", "ret20", "atr_pct"},
            "ギャップの大きさ(既に閾値で切っている軸)":
                {"gap_bp", "gap_hi_bp"},
            "価格帯の滞留日数": {"touch120", "touch120_pc"},
            "過去高値への距離": {"res60_bp", "res120_bp", "res60_pc_bp",
                                 "res120_pc_bp", "nearhi60_bp", "nearhi120_bp"},
        }
        _cols = {_r["col"] for _r in _hits}
        for _fn, _fs in _FAM.items():
            _in = sorted(_cols & _fs)
            if len(_in) >= 2:
                print(f"\n  ⚠ **同じ族から {len(_in)} 個**: {_fn}\n"
                      f"     {' / '.join(_in)}\n"
                      f"     → これは {len(_in)} 個の発見ではなく、"
                      f"**1つのことを {len(_in)} 通りに測った**ものです。"
                      f"confirm するなら族ごとに1つだけ。")
        if "gap_bp" in _cols or "gap_hi_bp" in _cols:
            print(f"\n  ⛔ **ギャップの大きさは新しい選別軸ではありません。**"
                  f" --min-gap-bp で既に切っている軸そのもので、"
                  f"\n     『Q5 が良い』= 『閾値を上げろ』です。"
                  f"それは --sweep-grid / --ops-gap-list で掃く話で、"
                  f"\n     §18.53 で帯を選ぶのは多重検定だから やらないと"
                  f"決めています。TEST を使わないこと。")
