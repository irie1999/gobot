...
■ 実行方法:
  ...
  python backtest_macd_scan_v2.py --days 45          # 直近45日
  python backtest_macd_scan_v2.py 7203.T --years 2   # 特定銘柄詳細
  python backtest_macd_scan_v2.py --years 3 --top 30 # 上位30件表示
"""
...
def fetch_df(symbol: str, backtest_days: int = BACKTEST_DAYS) -> pd.DataFrame | None:
    ...
    buf_days  = 200 + 30
    dl_start  = (datetime.today() - timedelta(days=int((365 + buf_days) * 1.5))).strftime("%Y-%m-%d")
...
def print_signals(sig: dict, portfolio: dict | None = None) -> None:
    ...
    if not buy:
        ...
        print(f"  {'順位':<4} {'銘柄':<20} {'業種':5} {'スコア':>6} "
              f"{'始値':>8} {'終値':>8} {'推奨指値':>9} {'MACD':>8} {'RSI':>5} {'出来高比':>6} {'ATR%':>5}  メモ")
        print("  " + "─" * 100)
        for i, c in enumerate(buy, 1):
            ...
            print(f"  {i:<4} {label:<20} {c['sector']:5} {c['score']:>6.1f} "
                  f"{c['open']:>8,.0f} {c['close']:>8,.0f} {lp:>9,.0f} {c['macd_hist']:>+8.3f} "
...
{portfolio_section_html}
    ...
    path = Path(f"signal_v2_{today}.html")
