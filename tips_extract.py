...
(B) tips  … 銘柄に依らない再現性のある手法・ルール。最大 {max_tips} 件。
     手法解説の動画では calls が 0 件でも構わない。**仕組みの説明 (なぜそうなるか)、
     条件 (どういう時に有効か)、数値 (勝率・期間・%)、失敗パターンも tips に含める。**
...
- **字幕が英語など日本語以外でも、出力はすべて日本語で書く** (原語の用語は括弧で補う。
  例: 「ギャップ&ゴー (gap and go)」)。日本の証券コードが無い銘柄 (米国株など) は
  ticker を空にして company にティッカーを書く (例: AAPL)。
