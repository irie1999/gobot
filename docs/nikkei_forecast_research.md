...
- `preopen` … 09:00 前に先物から観測できるギャップを使う (default)
...
**結論: ギャップは 09:00 前に実質的に既知。** 08:45 から大取デイセッション先物が
動いており、08:59 の先物価格と現物始値の乖離は通常わずか。
「ギャップを使えないから予測不能」という制約にはならない。

**実装上の注意 (重要):**
yfinance の日足では 08:59 の先物スナップショットが取れないため、
`--asof preopen` は **現物始値をその代理として**使っている。
これは数 bp ぶん楽観側にずれる近似。したがって:

1. まず `--asof strict` (当日ギャップを一切使わない) で **下限**を測る
2. 次に `--asof preopen` で **上限**を測る
3. 実際の運用値はこの間に入る
4. 本番投入する前に、**`kabu_token.py` 経由の kabu station API で 08:59 の
   日経先物 / 板寄せ気配を実際に取得**し、`open_gap` をその実データに置き換えて再検証する

この 1→4 を飛ばして preopen の数字だけ見ると、実運用でその成績は出ない。

### 10.3 判定基準 — 精度ではなくリスク指標、そして「ランダム休場」との比較

**この検証で最も間違えやすい点:**
何日か休めば σ も最大DD も **機械的に** 下がる。だから
「veto なし vs veto あり」を比べると、**予測力がゼロでもほぼ必ず改善して見える**。

`n225_veto.py` の自己テスト (シグナルを一切含まない合成データ) が実際にそれを示す:

```
                          稼働日   月平均    月σ   月平均/σ  Sharpe   CVaR95   最大DD
veto なし (現行N)            1799   0.346  4.534    0.076    0.26   -2.008  -26.31
veto 上位10% (180日)         1619   0.478  4.284    0.112    0.38   -1.958  -21.79
```

Sharpe が 0.26 → 0.38、最大DD が −26.3 → −21.8。一見大成功だが、
**このデータには予測可能なシグナルが 1 つも入っていない。**

正しいベンチマークは **同じ日数をランダムに休んだ場合の分布**。
`n225_veto.py` はブートストラップ (default 4,000 回) でその分布を作り、
実際の veto がどの分位に入るかを p 値として出す。上の例の p 値は 0.09〜0.34 で、
**ランダム休場と区別がつかない** = 予測力なし、と正しく判定される。

一方、既知シグナルを注入したケースでは全指標 p < 0.01 になる。

**判定ルール:**
- `p < 0.05` かつ複数の veto 率で一貫している → 予測力あり
- veto 率 5 通り × 指標 4 種を見るので、Bonferroni 目安は `0.05/20 = 0.0025`
- `p > 0.15` → ランダム休場と同じ。この方向は諦める
...
# Step 2: そもそも日中方向に信号があるか (下限と上限)
python n225_research.py --eval --target day --asof strict
python n225_research.py --eval --target day --asof preopen

# Step 3: N の実損益で veto 効果を検証 [本命]
python n225_veto.py --npnl n_pnl.csv --asof strict
python n225_veto.py --npnl n_pnl.csv --asof preopen

# Step 4: 停止ではなく縮小の方が良いか
python n225_veto.py --npnl n_pnl.csv --veto-size 0.5

# Step 5: 閾値感度 (「強い上昇日」の定義を動かして頑健性を見る)
python n225_veto.py --npnl n_pnl.csv --strong-pct 0.3
python n225_veto.py --npnl n_pnl.csv --strong-pct 0.8
```
