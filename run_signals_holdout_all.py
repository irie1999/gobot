...
if _args.both and not _args.short:
    ...
    # ── E/H タブについて ────────────────────────────────────────
    #   2026-08-10: レポート本体(nikkei_analysis.py)に組み込んだので、ここでの
    #   後付け差し込み(analyze_overnight_lss.py --inject-html)は**やめた**。
    #   両方走らせるとタブが2つ出る。本体側は _EH_TRADES が空なら何も出さない
    #   ので、無効化は set LSS_EH_TAB=0 のみで足りる。
    #   別条件で見たいときだけ手動で .\eh を使う(--exclude-months 等)。
...
# v20 (2026-08-25): **期限切れの注文も判定日で1件だけ記録**するようにした。
#   v19 は「まだ生きている注文」の日付を固定しただけで、3営業日 触れずに失効した
#   注文は依然として何も残らず、母集団から永久に消えていた。それは
#   「ギャップアップして前日終値まで戻らなかった日」= ライブの J がまさに建てる日。
#   あわせて nikkei_analysis 側の『最新シグナル日だけに畳む』フィルタも撤去
#   (日付が判定日で正しくなったので畳む理由が無く、畳むと過去日が落ち続ける)。
_BT_LOGIC_VER = "v20"
