...
# ── scan_breakout_entry.py --max-price 5000 スキャン上位銘柄 ────────
# 100株×株価≤5000円（50万円以下）、複数期間で安定した成績を示した銘柄を優先選定
WATCHLIST: list[tuple[str, str, str]] = [
    # ── DON ドンチャン高値ブレイク（複数期間安定上位）──
    ("4114.T", "日本触媒",                 "DON"),   # 3期間 91-100% PF24  12回
    ("6745.T", "ホーチキ",                 "DON"),   # 3期間 90-100% PF19  11回
    ("7380.T", "十六フィナンシャルグループ", "DON"),   # 3期間 88-100% PF24   9回
    ("4082.T", "第一稀元素化学工業",        "DON"),   # 3期間 87-100% PF139 +250K
    ("1802.T", "大林組",                   "DON"),   # 3期間 80-100%       +160K
    ("5727.T", "東邦チタニウム",            "DON"),   # 3期間 80-100%       +120K
    ...
    ("6235.T", "オプトラン",               "VOL"),   # 3期間 75-100% PF21 +108K
    ("1332.T", "ニッスイ",                 "VOL"),   # 3期間 100% PF∞
    ("1515.T", "日鉄鉱業",                 "VOL"),   # 3期間 100% PF∞  +167K
    ("6058.T", "ベクトル",                 "VOL"),   # 2期間 100% PF∞
    ("4901.T", "富士フイルムHD",           "VOL"),   # 1期間 100% PF∞  +73K
    ("5991.T", "日本発條",                 "VOL"),   # 1期間 100% PF∞  4回
    ("2461.T", "ファンコミュニケーションズ", "VOL"),   # 1期間 100% PF∞  4回
    # ── MOM モメンタムブレイク（複数期間安定上位）──
    ("4114.T", "日本触媒",                 "MOM"),   # 3期間 100% PF∞ (30+90+180d)
    ("4973.T", "日本高純度化学",            "MOM"),   # 3期間 100% PF∞  +194K
    ("5821.T", "平河ヒューテック",          "MOM"),   # 3期間 100% PF∞  +247K
    ("6592.T", "マブチモーター",            "MOM"),   # 3期間 100% PF∞ (90+180+365d)
    ("4221.T", "大倉工業",                 "MOM"),   # 1期間 100% PF∞  5回 +114K
    ("4343.T", "イオンファンタジー",        "MOM"),   # 1期間 100% PF∞  4回  +98K
    ("6310.T", "井関農機",                 "MOM"),   # 1期間 100% PF∞  7回 +106K
