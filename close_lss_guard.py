...
【運用タイミング】毎営業日 15:00〜15:20 JST 目安で実行(引け前)。
  引け成行(MOC)は **15:25-15:30 のクロージング・オークションで約定**する。
  ※東証は 2024/11/5 以降 大引け=15:30(以前は15:00)。MOCは板寄せ前(遅くとも
    15:24頃まで)に出す必要がある。lss_exit_watcher は 15:20 に MOC 切替→15:30
    までリトライするので、watcher を動かしていればこのスクリプトは不要。

【このスクリプトは“バックアップ”】(重要)
  通常運用は lss_exit_watcher.py(監視モード)が引け決済まで自動でやる。
  → **watcher を 15:30 まで動かしている日は close_lss_guard は実行しない**
     (二重決済・注文競合の恐れ)。watcher を立ち上げ忘れた/落ちた日の
     保険としてだけ手動で使う。

【運用メモ・ハマりどころ(このセッションで判明)】
  - kabu の有効トークンは1つ。**発注サーバ(order_server) と watcher を同時に
    動かすと 401 でトークンの取り合い**になり決済失敗する。同時に動かさない。
  - 引け決済の取りこぼし対策 = **watcher を大引け(15:30)まで止めない**こと。
    途中で閉じると MOC が出ずタイムカットが飛ぶ(過去にメドレーで発生)。
  - 同日決済なので **逆日歩は基本かからない**(持ち越さない限り)。
  - 空売り価格規制(51円/アップティック)は lss では実質無関係。逆指値売りは
    「前日終値-1ティック」の小さな下落で約定し、-3%指値下限ガードも効くため、
    -10%規制が発動する手前で約定する。
  - 空売り不可(非貸借/取扱なし)銘柄は選定時に not_shortable.py で除外済み
    (top指定なし版でも除外。check_shortable.py で月1更新)。残る動的リスクは
    「その日たまたま貸株在庫切れ」だけ(稀)。
  - kabu 発注は 一般信用(デイトレ)=MarginTradeType 3 が在庫・コスト面で有利。
