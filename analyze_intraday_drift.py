...
def _report(d: pd.DataFrame, title: str) -> None:
    ...
    if _it0 and _it0["日数"] >= 20:
        ...
        print(f"  ノイズ床(intraday限定): 日次σ={_dl.std(ddof=1):,.0f}円/日 → "
              f"{len(_dl)}営業日で識別できる下限は **±{_floor:,.0f}円/件**。")
        print(f"    ⚠ この床は intraday の σ から出したもので、**他の区間には使えない**。")
        print(f"      区間ごとの判定は各行の 95%CI を見ること: "
              f"**CIが0をまたいでいれば、その区間はゼロと区別できない**。")
        print(f"      ★が1個だけなら多重検定(7区間)の当たり。2〜3個以上か t が"
              f"二桁で初めて意味を持つ。")
        # ⛔ 統計的に有意でも、経済的に意味があるとは限らない。
        #    実測(2026-08-08)で close30 は t=2.92 と有意だったが +51円/件 = 0.019%
        #    = 1.9bp。売買スプレッド(5〜30bp)に全く届かず取引不能だった。
        _sig = [(s, _daily_stats(d, s)) for s, _, _ in SEGMENTS]
        _sig = [(s, st) for s, st in _sig
                if st and (st["CI下"] > 0 or st["CI上"] < 0)]
        if _sig:
            _names = ", ".join("{}({:+,.0f}円/件)".format(s, st["円/件"])
                               for s, st in _sig)
            print(f"  有意な区間(CIが0をまたがない): {_names}")
            for s, st in _sig:
                _bp = abs(st["%"]) * 100          # % → ベーシスポイント
                if _bp < 5:
                    print(f"    ⚠ {s} は有意だが {_bp:.1f}bp しかない。"
                          f"売買スプレッド(5〜30bp)に届かず**取引不能**。")
        else:
            print(f"  有意な区間: **なし**(全区間のCIが0をまたぐ)。"
                  f"この母集団に測定可能なドリフトは存在しない。")
...
print("■ この数字の使い方")
print(f"{'─' * 100}")
# ⛔ 判定は必ず 95%CI で行う。符号だけを見ると、ゼロと区別できない値を
#    『プラス』と呼んでしまう(2026-08-08 の実測で実際にそう出た: intraday
#    +52円/件 だが CI -113〜+212 = ゼロ)。
def _verdict(st) -> str:
    if not st:
        return "測定不能"
    if st["CI下"] > 0:
...
if _it:
    print(f"  intraday(寄り→大引けのショート) = {_it['円/件']:+,.0f}円/件 "
          f"(t={_it['t']:.2f} / CI {_it['CI下']:+,.0f}〜{_it['CI上']:+,.0f}) "
          f"→ {_verdict(_it)}")
    print(f"    lss の実績は 予算タブ11ヶ月で +883,421円 / 2,113件 = **+418円/件**。")
    if _it["CI下"] <= 0 <= _it["CI上"]:
        print(f"    → **市場全体の日中ドリフトは存在しない(測れない)。**")
        print(f"       したがって lss のエッジは機構(トリガー+損切り/利確)の側にある。")
        print(f"       **指数を素で売る先物版は成立しない**(売るべきドリフトが無い)。")
    elif _it["円/件"] >= 418 * 0.8:
        print(f"    → **同水準以上。選定・逆指値・損切りの機構は何も足していない可能性が高い。**")
        print(f"       素の日中ショートの方が銘柄数を稼げるぶん容量(capacity)も大きい。")
    elif _it["CI下"] > 0:
        print(f"    → 有意にプラスだが lss を下回る。機構が上乗せしている部分がある。")
        print(f"       差 {_it['円/件'] - 418:+,.0f}円/件 が『機構の付加価値』の上限。")
    else:
        print(f"    → 有意にマイナス。**lss のエッジは機構の側にある。**")
if _on:
    print(f"  overnight(前日終値→寄り) = ロング {-_on['円/件']:+,.0f}円/件 "
          f"(t={_on['t']:.2f} / CI {-_on['CI上']:+,.0f}〜{-_on['CI下']:+,.0f}) "
          f"→ {_verdict(_on)}")
    if _on["CI下"] <= 0 <= _on["CI上"]:
        print(f"    → **オーバーナイト・アノマリーもこの母集団には存在しない。**")
        print(f"       『上昇は夜に起きているので同日決済が儲かる区間を捨てている』は誤り。")
        print(f"       持ち越しに切り替える理由は無い(リスクだけ増える)。")
    elif -_on["円/件"] > 0:
        print(f"    → 上昇は夜に起きている。同日決済という制約が、儲かる区間を捨てている。")
