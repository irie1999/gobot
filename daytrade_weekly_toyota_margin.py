"""
週次デイトレ バックテスト【信用取引版・高レバレッジ】— 2026/03/16 週（月〜金）
銘柄  : トヨタ自動車 (7203.T)
...
■ 信用取引ルール:
  - 初期委託保証金 : 1,000 万円
  - 委託保証金率   : 10%（一部ネット証券・日計り信用） → 実質レバレッジ最大 10 倍
  - 維持率         : 5% 未満で強制ロスカット（追証ではなく即決済）
...
# 信用取引パラメータ
INITIAL_MARGIN      = 10_000_000  # 初期委託保証金（円）: 1,000 万円
MARGIN_RATE         = 0.10        # 委託保証金率（10%）→ レバレッジ 10 倍
MAINTENANCE_RATE    = 0.05        # 維持率（5% 未満で強制ロスカット）
QTY                 = 3_000       # 1 回の取引株数（10 倍レバレッジ活用）
...
def report(week_result: dict, source: str):
    ...
    print(f"  委託保証金率: {MARGIN_RATE*100:.0f}%  /  実質レバレッジ: {lev:.1f} 倍")
    print(f"  維持率下限  : {MAINTENANCE_RATE*100:.0f}% 未満で強制ロスカット")
    print(f"  初期保証金  : {INITIAL_MARGIN:,.0f} 円  （1,000 万円）")
    print("=" * W)
