...
#     LSS_TAB_TENKAN=1  … 「転換」
#     LSS_TAB_EH_ALL=1  … 「H/J の全取引(予算の制約なし)」
...
def _eq_pref_of(_g, _conf=None, _slow=False) -> str:
    ...
    ⛔ 確認方式の約定価格は2通り。**既定は始値**(2026-08-15 ユーザー決定)。
       名前に『5分』が入る変種だけが 09:05約定 = **保守側の下限**。
       2026-08-16 に一度 09:05 を既定にしてしまったが、これはその決定を
       無断で戻すものだったので始値に戻した。
    """
    if (_EQ_METHOD_CONF if _conf is None else _conf):
        return f"H寄り確認{'5分' if _slow else ''}{float(_g):+.0f}bp"
    return f"H指値{float(_g):+.0f}bp寄指"
...
（未約定・期限切れは含まない近似）。日数が少ないバケツは統計的に不安定。</p>"""
    ...
    if (_LSS_ORDER_MODE and not _LSS_H_ENTRY and cfg_filter is None
        ...
        try:
                ...
                if _eqk2:
                    ...
                    if _eqb2.startswith("H寄り確認"):
                        ...
                        for _wv0 in str(os.environ.get(
                            ...
                            # ★★ 『即時』= 約定は各銘柄の**自分の始値**、
                            #   サイズだけ段の配分で決める版。
                            #   1分ポーリングで寄りを検知できれば判定は
                            #   始値でできる(判定に件数は要らない)。
                            #   段の値で建てる版との差 = **執行速度の価値**。
                            _hvars.append(_eq_var(
                                f"{_eqb2}w{_ws}c{_wc}即時", _eqd2, _eqg2,
                                _eqsm2, _eqtm2, None, _wc, _ws))
            ...
            if str(os.environ.get("LSS_H_VARIANT_TAB", "1")).strip() \
                ...
                # delay1 の根拠(18.9)は「**逆指値**はバーのどこで約定したか
                # 分からないので、そのバーの間は損切りを置けない」という
                # 機構的な理由。H の板寄せ約定は 09:00 に約定価格が確定するので、
                # その理由は H には当てはまらない = 最適な delay が現行と違いうる。
                # 既定と同じ本数はスキップ(先頭の "H" が既にそれ)。
                for _dv in (0, 1, 2, 3):
                    if _dv == _eh_delay:
                        continue
                    _hvars.append((f"{_hte:+d}tick "
                                   ...
                                   _hte, _hae, _dv))
                # ── 損切りアンカー: max(実約定, 前日終値) ────────────────
                # 実約定基準は「寄りが指値より上で約定」に**必須**(前日終値基準だと
                # 損切りが約定値より下に来て建てた瞬間に成立)。だが「指値〜前日終値の
                # 間で約定」では不要で、そこだけ損切りが不必要にタイトになる。
                # max() なら前者は今と同じ、後者だけ広がって破綻もしない。
                # ⚠ 仮説の出所は 2026-08-13 の 4023 クレハ **1件**(現行 -3,000 /
                #   この案 +8,000)。当月は合計・検定から除外されるので、
                #   測る対象に仮説の出所は含まれない。それでも1件から思いついた
                #   案であることは変わらないので、判定は他と同じ厳しさで行う。
                _hvars.append((f"{_hte:+d}tick "
    ...
    _EQ_MAX_LOT = 10
    _EQ_MAX_YEN = 0.0
    _EQ_CAP_N = 0      # try の中で env から読み直す(落ちても未定義にしない)
    ...
    try:
        ...
        #    『上限なし / 150 / 200』の変種で測る。
        # ★ 50万 = **ユーザー判断**(2026-08-15)。1銘柄あたり50万まで。
        #   価格帯 1,000〜6,000円 で1単元は 10万〜60万なので、
        #     1,000円株 → 5単元(500株) / 5,000円株 → 1単元 / 6,000円株 → 1単元だけ
        #   となる(「1件目は最低1単元」があるので5,000円超は100株のみ)。
        # ⚠ 実測の傾向は **きつくするほど損益もσも悪化**(100万 +622,250/σ111,176 →
        ...
        # ★ 上限を効かせる『合格件数の上限』。
        #   0(既定) = **常に効かせる(フラット)**。1銘柄50万を宣言どおり守る。
        #   N>0 = 合格N件以下の日だけ絞る。
        # ⛔ 条件付き(N=3)は **やめた**(2026-08-15 実測)。4件以上の日は上限が
        #   完全に外れるため、同じ銘柄が複数戦略で出ると 予算/4=100万 × 3枠
        #   = 300万 になり、**1銘柄の最大が 280万(予算の70%)** まで膨らんだ。
        #   「1銘柄50万」と言いながら実効280万で、上限が機能していなかった。
        #   フラットなら 最大60万(15%) = 6,000円株の1単元ぶんで止まる。
        #   代償は 月 −11.4万(vs 上限200万)。事象は月2〜3日なので受け入れる。
        ...
        def _size_equal_by_day(_ts, _budget, _top=0, _fill=False, _dedup=False,
            ...
            **単一銘柄のリスク**なので、必ず併記して判断する。
            """
            # ★ 母集団フィルタ(2026-08-16)。_pool = (許可ペア集合, 解禁日dict)。
            #   **選定なしの実行から『選定あり』を切り出す**ために使う。
        ...
        # ★★ 『実装版』と『理想版』を **同じ実行から** 作る (2026-08-16)。
        #   選定あり(3,508ペア) ⊂ 選定なし(9,240ペア) なので、選定なしで回した
        #   結果から選定ありを切り出せる。別実行で比べると比較相手まで動く(18.24)。
        #     実装版 = 選定あり + watch50   ← いま kabu だけで作れる形
        #     理想版 = 選定なし + watch無制限 ← 全候補の始値が読めたら
        ...
        for _k in _EQ_KEYS:
            ...
            if _k in (_EQ_TAB_BASE, str(_EQ_TAB_KEY2).split("資金均等")[0]):
                ...
                if _WATCH_CAP > 0:
                    ...
                    #   **ギャップ上位N件だけ**建てる。
                    #   ⛔ これが上限50件版に勝つなら、**もっと読めるサービス
                    #     (楽天RSS等)に乗り換える価値がある**。負けるなら、
                    #     読める数を増やす調査は全部不要になる。
                    #   ★ ギャップは 09:00 に分かるので、この絞りは先読みでない
                    #     (前夜に決める watch とは別物)。186検定で唯一 単調
                    #     だった軸でもある(利確率 6.8%→14.9%)。
                    #   ⚠ 上位N は cap50 の下では棄却済み(上位3/5/8 とも ❌)。
                    #     ただしそれは「合格7.6件から3件選ぶ」で、ここは
                    #     「合格23件から N件選ぶ」= 母集団が3倍違う。
                ...
                # ⛔ **60万より小さい上限は守れない**。価格帯 1,000〜6,000円で
                #   1単元(100株)は 10万〜60万。5,000円超の銘柄は1単元でも50万を
                #   超えるが、「1件目は最低1単元」なので上限を突破して建つ。
            ...
            for _eqtp, _eqfl, _eqdp, _eqym, _eqcd, _eqwc, _eqpl, _eqkl in _eq_modes:
                _eqnk = (f"{_k}資金均等" + (f"上位{_eqtp}" if _eqtp else "")
                         ...
                         # 登録上限を動かした版だけ名前を分ける
                         + ("" if (_eqwc == _WATCH_CAP or _eqpl == "IDEAL")
                            else ("watch無制限" if _eqwc <= 0
                                  else f"watch{_eqwc}"))
                         # タブ用の2本だけ名前で区別する
...
<table>
  ...
  <tbody>{_poo_sym_rows}</tbody>
    ...
    def _eh_compare_html():
        """注文方式を月ごとに並べ、対応のある検定まで **HTML の中に**出す。

        なぜ HTML に入れるか: 外部ツールの出力と画面を突き合わせる
    ...
    def _strategy_loo_html():
        ...
        _NS = int(os.environ.get("LSS_STRAT_LOO_SEEDS", "16") or 16)

        # ⛔ 帯の σ を _NS 本から推定している以上、閾値は 1.96 ではなく **t(_NS-1)**。
        #    8本だと 2.36 必要で、|z|=2.0 台は「帯の外」ではない。実際 2026-08-12 の
        #    8本実行で VOLTF -2.01 / DON +2.05 が出たが、どちらも t(7)=2.36 に届かず
        #    有意ではなかった。既定を16本に増やしたうえで閾値も t に合わせる。
        ...
        for _name, _src, _nof in _srcs:
            ...
            #    ★これは近似ではなく**厳密**。_run_budget_sim は日ごとに _cap を
            #      0 に戻す = **日をまたぐ状態を持たない**ので、「S を通期で外した」
            #      サマリーの月 m の値は「S を月 m だけ外した」値と完全に一致する。
            #      よって単独 LOO の月別系列から合成してよい(再シミュ不要)。
            #      ⚠ 予算に繰越を入れる改造をしたら、この前提は壊れる。
            if len(_ms) >= 3:
                _wf, _pk = 0.0, []
        ...
        return (
            ...
            f'<b>|z|&lt;{_TC:.2f} は「測れていない」</b>であって「同じ」ではありません(18.24)。<br>'
            f'⚠ 閾値が 1.96 でなく <b>{_TC:.2f}</b> なのは、帯のσを{_NS}本から推定して'
            f'いるからです(t分布 df={_NS - 1})。本数が少ないほど z は出やすくなるので、'
            f'閾値も上げないと偽陽性になります。<br>'
            f'⚠ さらに <b>6戦略×3方式=18セルを同時に見ている</b>ので、帰無でも'
            f'|z|≥{_TC:.2f} が1つ弱は出ます。1セルだけ超えたものは根拠になりません。<br>'
            ...
            f'<b>■ この表はすでにOOSです。</b> 取引そのものは per-symbol START_DATES で'
            f'切られており、月Mの行には<b>初出基準月が M-1 以前のペアだけ</b>が入ります'
            f'(= ローリングfoldと同じプール / 18.29・18.33)。'
            f'E/H も同じ母集団から作られるので同様です。<br>'
            f'唯一OOSでなかったのが<b>「どの戦略を外すか」の選択</b>で、それを消すのが下の行です。<br>'
            f'<b>▶ walk-forward 除外</b> = 各月について<b>その月より前だけ</b>を見て'
            f'「外すのが最も得だった戦略」を1つ外す。'
            f'これは<b>厳密</b>です(近似ではない): 予算シミュは日ごとに枠を0に戻す'
            f'＝<b>日をまたぐ状態を持たない</b>ので、「通期で外した」月Mの値と'
            f'「月Mだけ外した」値は完全に一致します。<br>'
    ...
    def _preopen_day_html():
        ...
        # ── 帰無: **損益の系列を巡回シフト**して、同じ分位の切り方で最小群を取る ──
        # ⛔ 「日をランダムに5等分」を帰無にしてはいけない(2026-08-13 に2回間違えた)。
        #    ① 最小を選ぶ操作でバイアスが出る → 帰無側も最小を選べば相殺できる
        #    ② **シャッフルが時系列クラスタを壊す** → こちらは相殺できない。
        #       VIX水準・DAX・5日リターンのような持続する変数で並べると、
        #       分位は**時期のかたまり**になる。損益にもレジームがあるので、
        #       かたまりの合計は無作為な22日より遥かに大きく振れる。
        #       シミュレーションで実測: ランダム5等分の最小群は、持続変数で
        #       ソートした最小群より **平均 +302,822円 高い** = 情報ゼロでも
        #       z がプラスに出る。18.13 に「帰無較正は日ブロックを保つ。完全
        #       シャッフルは相関を壊して帰無分布が狭くなり偽陽性を過小評価する」
        #       と自分で書いてあった。
        # → 変数の並び(=分位の切り方)はそのままに、**損益の側だけを日付順に
        #   巡回シフト**する。変数の時系列クラスタも損益の自己相関も保たれる。
        _chrono = sorted(_days)
        _pos = {d: i for i, d in enumerate(_chrono)}
        _pv = [_day[d][2] for d in _chrono]
        _N = len(_chrono)

        def _band_groups(groups):
            """groups(日付のリスト群)の切り方を固定し、損益を巡回シフトした帰無。"""
            _o = []
            for _sd in range(1, _NS + 1):
                _k = int(_sd * _N / (_NS + 1)) or 1        # 0シフトは実測そのものなので除く
                _mn = None
                for _g in groups:
                    _sv = sum(_pv[(_pos[d] + _k) % _N] for _g2 in (_g,) for d in _g2)
                    if _mn is None or _sv < _mn:
                        _mn = _sv
                _o.append(-(_mn or 0.0))
            return _o

        _rows = ""
        # ★ 掃く順は「機構が説明できるもの」から。文献の根拠は下の脚注に書く。
        _nkeys = [k for k in _pm.LABELS
        ...
        for _k in _nkeys:
            ...
            _dl, _z, _vd = 0.0, 0.0, "—"
            if _best and _groups:
                _bd = _band_groups(_groups)
                _dl = -_best[0]
        ...
        return (
            ...
            f'合計がどう動くか</b>で、<b>分位の切り方はそのままに損益の系列だけを'
            f'巡回シフトした帯</b>({_NS}本)と比べています。'
            f'|z|&lt;{_TC:.2f} は「測れていない」であって「同じ」ではありません。<br>'
            f'⛔ 帰無の作り方を2回間違えました(2026-08-13)。'
            f'①「同数をランダムに落とす」→ <b>最小を選ぶ操作</b>だけで z が +1.17 出る。'
            f'②「日をランダムに5等分して最小群を落とす」→ ①は相殺できるが、'
            f'<b>シャッフルが時系列クラスタを壊す</b>。VIX水準・DAX・5日リターンのような'
            f'<b>持続する変数</b>で並べると分位は「時期のかたまり」になり、損益にも'
            f'レジームがあるので合計が無作為な22日より遥かに大きく振れます。'
            f'実測でランダム5等分の最小群は持続変数ソートより<b>平均 +302,822円 高く</b>、'
            f'情報ゼロでも z がプラスに出ていました(18.13 の「帰無は日ブロックを保つ」'
            f'に自分で違反)。<br>'
            f'→ <b>変数の並び(分位の切り方)は固定し、損益の側だけを日付順に巡回シフト</b>'
            f'します。変数の時系列クラスタも損益の自己相関も保たれます。<br>'
            f'⚠ {len(_nkeys)}変数を同時に見ているので、帰無でも |z|≥{_TC:.2f} が'
            f'{len(_nkeys) * 0.05:.1f}個ほど出ます。1つ超えただけでは根拠になりません。<br>'
            f'<b>■ 文献から予想される向き</b>(合わなければ偶然を疑う材料になります)<br>'
            f'・<b>VIX</b>: Nagel(2012) は<b>短期リバーサル戦略=流動性供給のリターンは '
            f'VIX で強く予測できる</b>とする。H は<b>指値売り</b>＝上げてくる買いに'
            f'流動性を供給する側なので、VIX が高いほど良いはず。<br>'
            f'・<b>S&amp;P500 前日%</b>: 前日の米国が強いほど<b>東京の寄り30分は弱い</b>'
            f'(ノイズトレーダーの過剰反応→裁定による反転)。H は寄りで売るので'
            f'<b>米国が強い日ほど良いはず</b>。ただし同研究は<b>金融危機時には反転が弱まる</b>'
            f'とも報告しており、VIX と逆向きの予想になる点に注意。<br>'
            f'・<b>|S&amp;P500 前日%| / |先物-現物%|</b>: 反転が過剰反応由来なら、'
            f'効くのは<b>符号ではなくショックの大きさ</b>かもしれないので絶対値も掃く。</p>'
            f'<table style="border-collapse:collapse;font-size:0.8rem">'
    ...
    def _h_variant_html():
        ...
        def _knobs(_nm: str) -> dict:
            ...
            # 『即時』= 約定は自分の始値・サイズだけ段で決める(執行速度の上限)
            if "即時" in _s and _k2["wave"] != "—":
                _k2["wave"] += "即時"
            ...
            _m6 = _re_kn.search(r"H(?:指値|寄り確認(?:5分)?)([+-]\d+)bp", _s)
            ...
            _b0 = _s.split("資金均等")[0]
            # ⚠ 実装版/理想版は **母集団が違う**(選定あり/なし)。つまみではなく
            #    別の方式なので、監査ボードの兄弟判定から外す(2026-08-16)。
            #    外さないと『つまみ0個違い』として基準と同一に見えてしまう。
            _k2["_auc"] = ("実装版" if "実装版" in _s
                           else "理想版" if "理想版" in _s
                           else "H本体" if _b0 in ("H", "H寄指")
        ...
        if _bb:
            ...
            if _wr and _bb[2] > 0 and _wr[2] >= _bb[2] * 0.95:
                ...
                # ⛔ ここで「既定のまま動かさないのが正解」と言い切ってはいけない。
                #    walk-forward < 固定最良 が意味するのは『**毎月選び直すこと**に
                #    価値が無い』だけで、『既定が正しい』ではない。既定より
                #    walk-forward が上なら、既定は選ぶべき1つではないという証拠。
                #    (2026-08-11: delay の掃き出しで既定 +1,636,210 に対し
                #     walk-forward +1,810,934 なのに「既定のまま」と出て誤誘導した)
                _dr = next((r for r in _out if r[0] == _vs[0]), None)
                if _dr and _wr[2] > _dr[2] * 1.02:
                    _wtxt = (
                        f'⚠ <b>walk-forward は固定最良の {_ratio:.1f}% どまり</b>'
                        f'（{_wr[2]:+,.0f}円）= <b>毎月選び直すことに価値はありません</b>。'
                        f'<br>ただし walk-forward は<b>既定({_dr[0]}: {_dr[2]:+,.0f}円)を'
                        f'{_wr[2] - _dr[2]:+,.0f}円 上回っています</b>。'
                        f'<span style="color:#f87171">= 既定が「選ぶべき1つ」ではない'
                        f'ということ。1つに固定するなら別の設定を検討する余地があります'
                        f'（順位は in-sample なので、前半/後半の順位が両方とも上のものを）。</span>')
                else:
        ...
        return (
            ...
            f'<b>損切り遅延(delay)</b>の3つです。'
            f'差/月 と t は<b>現行との対応検定</b>（同じ月どうしを引き算）。<br>'
            f'⚠ <b>delay の行だけは意味が違います。</b>delay1 の根拠(18.9)は'
            f'「<b>逆指値</b>はバーのどこで約定したか分からないので、そのバーの間は'
            f'損切りを置けない」という機構的な理由です。H の板寄せ約定は 09:00 に'
            f'約定価格が確定するので<b>この理由は H には当てはまりません</b>。'
            f'= H の最適な delay は現行と違いうるので測っています。'
            f'<b>採用するならライブ(watcher)も同じ本数に揃えること</b>(18.9 の鉄則)。<br>'
    ...
    def _findings_html():
        ...
        毎日この長文を読む必要はないが、**なぜこの設定なのか**を後から辿れないと
        同じ検証を繰り返すことになる(実際このセッションで転換・BT・発注順を
        何度も掘り直した)。数字は 2026-08-10 時点の実測で固定する。
        上の表は毎回計算し直されるので、そちらが最新。ここは判断の記録。
        """
        _s = 'color:#e2e8f0;font-weight:700;font-size:0.82rem;margin:10px 0 4px'
        _p = 'color:#cbd5e1;font-size:0.78rem;margin:0 0 6px;line-height:1.85'
        return (
            '<details style="background:#0f172a;border:1px solid #475569;'
            'border-radius:8px;padding:10px 14px;margin:0 0 14px">'
            '<summary style="color:#e2e8f0;font-weight:700;font-size:0.88rem;'
            'cursor:pointer">📚 H を採用した理由（2026-08-10 の調査ログ・クリックで展開）</summary>'
            f'<p style="{_p};margin-top:8px;color:#94a3b8">'
            '数字は 2026-08-10 時点の実測（予算400万 / 10ヶ月 2025-10〜2026-07 / '
            'BT30以上 / 流動性順 / delay1 / v17 / 手数料0 / slip=0）。'
            '<b>上の表は毎回計算し直されるのでそちらが最新</b>。ここは判断の記録です。</p>'

            f'<div style="{_s}">① H とは（現行との違いは注文の種類だけ）</div>'
            f'<p style="{_p}">現行は前日終値に<b>逆指値</b>売り＝<b>下がってきたら</b>売る。'
            'H は前日終値−5ティックに<b>指値</b>売り＝<b>上がってきたら</b>／寄りが既に上なら売る。'
            '約定価格は現行がトリガー<b>以下</b>、H は指値<b>以上</b>。'
            '決済（損切0.1ATR上／利確1.0ATR下／引け成行）・銘柄選定・発注順は<b>完全に同一</b>。<br>'
            '効く理由は2つとも構造的。(a) 空売りは高く売るほど有利で、H は上でも当たる。'
            '(b) 約定の86%が寄りの板寄せなので<b>9:00から利確を狙える</b>（現行は11時に約定したら'
            'そこからしか走れない）。§18.32 の測定で、シグナル銘柄は翌日の日中に平均29bp下げると'
            '分かっている。下げるなら高いところで売るほうが幅を取れる。</p>'

            f'<div style="{_s}">② 現行との差（両期間で維持）</div>'
            f'<p style="{_p}">+95,546円/月・t=+3.88・95%CI +39,907〜+151,185・9/10勝。'
            '前半 t=+4.03 ／ 後半 t=+3.90 で<b>期間を割っても崩れない</b>。'
            '1件あたり +782円 は現行(+350円)の<b>2.2倍</b>。</p>'

            f'<div style="{_s}">③ E（翌朝の寄成）は不採用</div>'
            f'<p style="{_p}">対現行 t=+2.03 で全体でも有意でなく、<b>後半で失速</b>'
            '（前半 t=+3.77 → 後半 t=+0.51）。E は寄成なので<b>必ず建ててしまい</b>、'
            '上げ相場で刈られる。2026-07 は現行 +129,550 / H +156,425 に対し'
            '<b>E だけ −36,148</b>。トリガーは無料の相場フィルタでもあった。</p>'

            f'<div style="{_s}">④ 設定 = −5ティック・ザラ場込み</div>'
            f'<p style="{_p}">7通りを1回の実行で比較（設定ごとにレポートを回すと'
            '比較相手の現行まで動く。実測 2,235→1,817→1,947件）。'
            '<b>walk-forward でも11月以降ずっと −5tick ザラ場込が選ばれ続け</b>、'
            '固定最良の99.5%。10ヶ月を見てからの後知恵ではなく、1ヶ月見た時点で先頭だった。'
            '前半1位／後半1位。<br>'
            '寄指（板寄せのみ）との差は +94,324円/10ヶ月＝月+9,432円で CI の幅の中。'
            'ザラ場ぶんは +312円/件と薄く『高値が触れたら約定』の仮定に乗るが、'
            '<b>触れても約定しなければ建たないだけ</b>なので下振れはしない。</p>'

            f'<div style="{_s}">⑤ 発注順 = 流動性順のまま（変更なし）</div>'
            f'<p style="{_p}"><b>どの順序も期間で符号が反転</b>し、✓（前半後半とも同符号）が'
            '1つも付かなかった。walk-forward は 現行60% / E 100% / H 97%。'
            'BT降順は E/H で帯の外(下)（z=−3.21 / −4.36）だが、それも<b>後半だけ</b>で'
            '作られた差（前半は z≈0）。<br>'
            'このシムは <b>slip=0</b> なので流動性順の利点は映らない。帯の中なら下振れ'
            'しないので、<b>映らない部分（成行の滑り）で得をするほうを選ぶ</b>(§18.21)。'
            'H は決済の68%が損切り＝成行買い戻しなので、この論拠は現行より強く効く。</p>'

            f'<div style="{_s}">⑥ BT は使わない</div>'
            f'<p style="{_p}">発注順にも選定にも効かない（§18.12/§18.13/§18.21/§18.24 で4回否定）。'
            'BT降順は E/H でランダム12本すべてを下回った。'
            'BT30 はプールの床で、追加のフィルタはかけていない。</p>'

            f'<div style="{_s}">⑦ 累積マージは継続</div>'
            f'<p style="{_p}">選定からの経過月数で層別すると<b>単調劣化せず U字</b>で、'
            '7ヶ月〜がむしろ最良（H +1,144円/件）。3方式とも同じ形で全帯プラス。'
            '古い基準月を落とす根拠はない。<br>'
            '⚠ 経過月数と暦月は相関するので、時期の効果と分離できていない。'
            'ただし劣化しているなら右端が沈むはずで、沈んでいない以上'
            '「劣化していない」とは言える。</p>'

            f'<div style="{_s}">⑧ 残っている唯一の未知数 = 実運用の滑り</div>'
            f'<p style="{_p}">このシムは <b>slip=0</b>。H は決済の68%が損切り＝'
            '<b>成行の買い戻し</b>で、そこは板の厚さで滑る。§18.17 の実測でも滑りの99%が'
            '成行買い戻しに集中していた。<br>'
            '<b><code>.\\fills</code> を10営業日ぶん貯めれば直接測れる。</b>'
            '+782円/件 がどれだけ残るかは実測でしか分からない。'
            '一括切替はせず、小さめの予算で並走させること。</p>'

            f'<div style="{_s}">⑨ 実運用で変わること</div>'
            f'<p style="{_p}">発注 <code>python lss_budget_cap.py --entry-mode limit '
            '--limit-ticks -5 --budget-multiple 1.0</code><br>'
            '<b>予算倍率 2.0 → 1.0</b>。寄りで一斉に約定するので'
            '「約定を見ながら残りを取消す」over-subscribe が成り立たない'
            '（倍率2.0だと最大800万が同時に建つ）。<br>'
            'watcher は <b>実約定価格から</b>損切り・利確を計算し直す'
            '（板寄せの約定値は指値以上になるので、指値基準の損切りは約定値より下に来る）。<br>'
            '<code>--entry-cutoff</code> の議論は<b>消える</b>。寄りで建つか建たないかなので、'
            '「遅い時刻の約定が不利」という非対称が構造的に起きない。<br>'
            '⚠ <b>発注リスト（シグナルタブ）とレポートの発注ボタンは現行の逆指値のまま</b>。'
    ...
    try:
        # 調査ログは最後に読むものなので末尾。既定は折りたたみ。
        if _LSS_ORDER_MODE and _eh_sorted:
            _EH_CMP_HTML += _findings_html()
    except Exception as _fnce:
    ...
    #      F 5分足寄り+OCO / **G 翌寄りロング**(E の鏡像・棄却済み) /
    #      H 前日終値で指値売り
    #    新しい方式に **G を使い回さないこと**(棄却した負け方式と混ざる)。
    #    I は数字の1と紛れるので飛ばし、次は J。
    _EH_LBL = {"E": ("E 寄成", "#a78bfa", "#c4b5fd",
               ...
               # ★★ J = 実装版 / K = 理想版 (2026-08-16)。**同じ1回の実行**から
               #   作っているので、月別サマリーを並べればそのまま比較になる。
               #   決済設定(delay/sm/tm/ATR/上限)は両者まったく同じ。
               #   違うのは **母集団** と **09:00 に読める銘柄数** の2つだけ。
               "J": (f"J 実装版（選定あり・watch{_WATCH_CAP}件）",
                     "#facc15", "#fde68a",
                     "<b>いま kabu だけで作れる形</b>です。"
                     "前夜に銘柄選定(累積マージ)で候補を作り、"
                     f"そこから<b>流動性上位{_WATCH_CAP}件</b>だけを登録して"
                     "09:00 に始値を読み、"
                     f"ギャップ<b>{_EQ_TAB_GAP}</b>以上を合格として"
                     "<b>予算を均等割り</b>。"
               ...
               "K": (f"K 理想版（選定なし・watch無制限）", "#22d3ee", "#a5f3fc",
                     ...
                     "銘柄選定をせず(全ペアを対象)、"
                     "その日のシグナル<b>全部</b>の始値を 09:00 に読んで判定します。"
                     "決済設定は J とまったく同じで、違うのは"
                     "<b>母集団</b>と<b>読める銘柄数</b>だけ。"
    ...
    for _ehk in ("E", "H", "J", "K"):
        ...
        _eh_pane += (
            ...
            + (f'株数だけが違います（H は100株固定 / {_ehk} は '
               ...
               #    誤り**。方式で説明が真逆になるので分岐する。
               + ('09:00 に始値を見てギャップ判定 → その場で発注（約定は<b>始値</b>で近似'
                  '<b>合格件数が確定してから発注する</b>ので、'
                  '<b>予算 ÷ 約定数</b>で割り切れます（＝集中できる）。'
                  '<br>⚠ <b>不約定はありません</b>（確認してから出すので'
                  '発注枠を空振りに使わない）。'
                  '＝2026-08-15 のユーザー決定。08:5x に板を暖めておけば '
                  '09:00+十数秒で発注できるという前提）。'
                  '<br>⚠ 前提が崩れた場合の<b>保守側の下限</b>は、同じ表の '
                  '<b>「09:00確認→09:05成行(保守)」</b>の行です。'
                  '真の値はこの2つの間で、'
                  '<code>python measure_entry_decay.py</code>（1分足）で'
                  '帯を狭められます。確定は <code>.\\fills</code> の実約定。'
                  '<br>⚠ 09:00 に板（kabu <code>/board</code>）を読む自動化が要ります。'
                  '登録直後の初回は<b>48〜142秒</b>かかるので '
                  '<b>08:5x に登録して1回空読み</b>すること（ウォームなら41銘柄6秒）。'
                  '<br>⛔ kabu の<b>銘柄登録は50件が上限</b>で、超えると'
                  '<b>リクエストごと400で失敗</b>します（部分受理されない）。'
                  '候補が多い日は<b>前夜に流動性上位50件へ絞る</b>こと。'
                  '<br>★ <b>09:00 に寄らなかった銘柄は建てません</b>'
                  '（2026-08-16 決定）。09:00 に件数を確定させないとサイズを'
                  ...
                  '<br>★ <b>「判定」と「サイズ決定」は別物</b>です。'
                  '1分ポーリングで寄りを検知できれば<b>判定はどの銘柄でも'
                  '自分の始値でできます</b>（判定に件数は要りません）。'
                  '件数が要るのは<b>サイズ決定（予算÷件数）</b>だけです。'
                  'その上限を測るのが<b>「…即時」</b>の行で、'
                  '約定は各銘柄の自分の始値・サイズだけ段の配分で決めています。'
                  '<b>段の値で建てる版との差＝執行速度の価値</b>です'
                  '（そのままでは実装不可。100株だけ即建てて段で積み増す形なら'
                  '部分的に実現できます）。'
                  ...
                  '<br>⛔ 株数も<b>前夜に決めるしかない</b>のに、どれが約定するかは'
                  '09:00 まで分かりません。したがって予算は'
                  '<b>候補数（約定＋不約定）</b>で割ります'
                  '（約定数で割るのは<b>先読み</b>）。'
                  '<br>⛔ さらに<b>注文の総額そのものが予算を超えられません</b>'
                  '（委託保証金は発注時に要る）。候補が多い日は注文を出せない候補が'
                  '出ます（「枠切れ」）。どれを出すかは前夜に決めるのでギャップでは'
                  '並べられず、<b>流動性降順</b>で埋めています。'
...
<h2>取引明細</h2>
{'' if not _CAN_HIDE else '<p style="color:#64748b;font-size:0.72rem;margin:-6px 0 8px">見ないタブは既定で出していません（HTMLを軽くするため / 2026-08-15）。戻すには <code>set LSS_TAB_ALL=1</code>（全部・決済日順）／<code>LSS_TAB_ENTRY=1</code>（エントリー日別）／<code>LSS_TAB_TENKAN=1</code>（転換）／<code>LSS_TAB_EH_ALL=1</code>（H・Jの全取引）。<b>集計・検定の数字はこの設定に影響されません</b>。</p>'}
