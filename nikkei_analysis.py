...
<table>
  ...
  <tbody>{_poo_sym_rows}</tbody>
    ...
    def _preopen_day_html():
        ...
        def _band(n):
            """帰無: **日をランダムに5等分し、その中の最小群を落とす**。_NS 本。

            ⛔ 「同数をランダムに落とす」を帰無にしてはいけない(2026-08-13 に実際に
               間違えた)。こちらがやっているのは『5分位のうち最小を選んで落とす』
               で、**最小を選ぶ操作そのものが delta を押し上げる**。
               実測: 完全にランダムな日次損益でも、同数ランダム除外を帰無にすると
               z が平均 **+1.17** 出る。実際の8変数の z は +0.31〜+1.61 で、
               まるごとこのバイアスで説明がついてしまっていた。
               帰無側も同じ『5等分して最小を落とす』にすれば、選択バイアスが
               両側で相殺されて z が正しく 0 中心になる。
            """
            _o = []
            _sizes = [n] * 4 + [len(_days) - n * 4]
            for _sd in range(_NS):
                ...
                for _sz in _sizes:
                    if _sz <= 0:
                        continue
                    _gs.append(_rk[_i:_i + _sz])
                    _i += _sz
                _worst = min(_gs, key=lambda g: sum(_day[d][2] for d in g))
                _o.append(-sum(_day[d][2] for d in _worst))
            return _o
        ...
        return (
            ...
            f'合計がどう動くか</b>で、<b>日をランダムに5等分してその最小群を落とした帯</b>'
            f'({_NS}本)と比べています。|z|&lt;{_TC:.2f} は「測れていない」であって'
            f'「同じ」ではありません。<br>'
            f'⛔ 帰無を「同数をランダムに落とす」にしてはいけません。'
            f'こちらは<b>5分位のうち最小を選んで</b>落としているので、'
            f'その選択操作だけで z が押し上がります'
            f'(完全にランダムな損益でも z が平均 <b>+1.17</b> 出ると実測)。'
            f'帰無側も同じ『5等分して最小を落とす』にして相殺しています。<br>'
