...
def _run_one(win: list[str]) -> Path | None:
    ...
    if _htmls:
        ...
        if not args.no_open:
            # file:// URI(空白は%20・日本語も%エンコード)で既定ブラウザに渡す。as_uri() を
            # 使うのでパスの空白で切れない。シェル(cmd)を経由しないので % の誤展開も無い。
            # (既定ブラウザがEdgeならEdgeで開く。既定でなければ下の手動コマンドでEdge指定可)
            try:
                webbrowser.open(_hp.resolve().as_uri())
