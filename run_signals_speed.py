"""
run_signals_speed.py — 速度重視モード シグナル・損益レポート

run_signals_holdout_all.py と同一構造だが、以下の2点が異なる:
  1. score_speed_patch を適用 → BTスコアに目標達成速度ボーナス(最大+10点)
  2. CSV銘柄選定スコアに速度係数を追加 → 回転が速い銘柄が上位に来やすくなる

速度係数 (CSV選定時):
  speed_mult = max(0.5, 2.0 - avg_hold_days / 15)
    avg=1日  → ×1.93  (約2倍)
    avg=8日  → ×1.47
    avg=15日 → ×1.0   (通常と同じ)

BTスコアボーナス (score_speed_patch):
  speed_bonus = max(0, 1 - avg_target_days / 15) × 10点 (最大+10点)

使い方:
  python run_signals_speed.py
  python run_signals_speed.py --workers 8
  python run_signals_speed.py --no-browser
  python run_signals_speed.py --date 2026-06-09
  python run_signals_speed.py --days 180
"""
from __future__ import annotations

# ── 速度ボーナスパッチを最初に適用 ────────────────────────────────────────────
import score_speed_patch  # noqa: F401  check_signals_stop/breakout の calc_recommend_score を差し替え
...
def _composite_score_speed(r: dict) -> float:
    """速度係数込みの選定スコア。avg_hold_days が短いほど高評価。"""
    base = _float(r.get("total_test_pnl", 0)) * (1.0 + max(_float(r.get("sharpe", 0)), 0.0))
    avg_hold = _float(r.get("avg_hold_days", 15)) or 15.0
...
def _load_wl_from_csv(csv_path: Path, max_price: float, strategy: str,
    ...
    filtered.sort(key=_composite_score_speed, reverse=True)  # ← 速度係数込みでソート
...
# ── PNL_CONFIGS 構築 ──────────────────────────────────────────────────────────
print("=" * 65)
print(f"run_signals_speed: {TODAY}  ⚡ 速度重視モード")
print("=" * 65)
...
if _using_fallback:
    print("[INFO] WFスキャンCSVなし → 現行WATCHLISTを使用（速度ボーナスのみ適用）")
    _fb_cfg = {
        "label":   "現行WL con",
        "color":   "#3b82f6",
        "mode":    "conservative",
...
.ho-outer-btn.active { background:#0f172a; color:#f59e0b;
  border-bottom:2px solid #f59e0b; font-weight:700; }
...
.ho-period-btn.active { background:#f59e0b; color:#0f172a;
...
.sym-tab-btn.active { background:#b45309; border-color:#f59e0b; }
...
<title>⚡ 速度重視 シグナル・損益 {date_str}</title>
<style>
{_na.CSS}
{_extra_css}
</style>
...
<body>
<h1>⚡ 速度重視モード シグナル・損益レポート</h1>
<p class="subtitle">
  基準日: {date_str} &nbsp;|&nbsp;
  設定数: {len(_all_configs)}件 &nbsp;|&nbsp;
  BTスコア: 速度ボーナス最大+10点適用 &nbsp;|&nbsp;
  workers={_args.workers}
</p>
<p style="color:#f59e0b;font-size:0.82rem;margin-bottom:8px">
  ⚡ 速度重視モード:
  CSV選定スコアに速度係数(avg_hold_days短いほど上位)を適用。
  BTスコアに目標達成速度ボーナス最大+10点を加算。
</p>
...
<div id="ho-sym" class="ho-outer-pane">
  <p style="color:#94a3b8;font-size:0.82rem;margin:8px 0 0">
    本日シグナルが出た {len(_signal_stocks)} 銘柄の過去365日取引履歴（速度ボーナス込みBTスコア降順）
  </p>
...
out_path = Path(f"signals_speed_{date_str}.html")
