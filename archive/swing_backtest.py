...
# ── yfinance 日足取得 ─────────────────────────────────────────
def fetch_bars(start: str, end: str) -> list[dict]:
...
def report(trades: list[dict], equity: list[float]):
    W = 78
    ...
    if trades:
        ...
        for i, t in enumerate(trades, 1):
            ...
            print(
                f"  {i:>2}  "
                f"{t['entry_dt'].strftime('%Y-%m-%d')}({e_dow})  "
                f"{t['exit_dt'].strftime('%Y-%m-%d')}({x_dow})  "
                f"{t['hold_days']:>3}日  "
                f"{t['qty']:>4}株  "
    ...
    avg_hld = sum(t["hold_days"] for t in trades) / n if n else 0.0
    ...
    print(f"  平均保有日数         : {avg_hld:>13.1f} 日")
    ...
    if trades:
        ...
        for i, t in enumerate(trades, 1):
            ...
            sign = "+" if t["pnl"] >= 0 else ""
            print(f"  #{i:>2} {t['entry_dt'].strftime('%Y-%m-%d')}→"
...
if __name__ == "__main__":
    bars = fetch_bars(START_DATE, END_DATE)
    print(f"バックテスト実行中... ({len(bars)} 本)\n")
