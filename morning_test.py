...
if not args.no_kpaper:
    # ⛔⛔ **窓に入るまで起動しない**(2026-08-20 発見)。
    #   k_open_confirm は --execute のとき『発注の窓』
    #   [open_at-20分, poll_until+20分] = 既定 08:40〜09:30 の外なら
    #   **起動直後に sys.exit** する(2026-08-18 の時間外誤発注ガード)。
    #   ところがここには待ちが無く、手順2(気配ログ)が 08:45 まで回ることに
    #   暗黙に依存していた。--skip-preopen を付けた日や、気配ログが
    #   エラーで早く終わった日は、07:50 に起動 → 窓の外 → 即終了 →
    #   **その日は1件も発注されない**。morning_test は「失敗しましたが次に
    #   進みます」と出して watcher を起動するだけなので気づきにくい。
    #   → ウォームアップ時刻の1分前まで必ず待つ(窓の内側)。
    if not args.dry_run:
        _wait_until(_hm(args.k_warm_at) - _dt.timedelta(minutes=1),
                    "発注の窓(08:40〜)に入ってから起動する。"
                    "早く起動すると時間外ガードで即終了し、その日は発注ゼロになる")
    ...
    _res.append((("5. J発注" if args.execute else "5. K記録"), _run(
        ...
        _cmd5)))
    # ⛔⛔ **実発注のはずが1件も出なかった日を黙って流さない**(2026-08-20)。
    #   _run は「失敗しましたが次に進みます」と出して先へ行くので、
    #   その日は発注ゼロのまま watcher だけが立ち上がる = 気づきにくい。
    if args.execute and not _res[-1][1]:
        print(f"""
{'=' * 74}
⛔⛔ **手順5(J の実発注)が失敗しました。今日は1件も発注できていない可能性が高いです**
{'=' * 74}
  よくある原因:
   ・**発注の窓の外**で起動した(既定 08:40〜09:30)。手順2が早く終わると起きる
   ・トークンの取り合い(401) … 発注サーバ / 別の watcher が動いていないか
   ・手順0のシグナル収集が失敗して候補が無い
  → 上のログで k_open_confirm の終了理由を確認してください。
...
""", flush=True)
    ...
    if not args.dry_run:
        ...
        while True:
            ...
            #   その状態で watcher が落ちると **建玉があるのに再起動しない**。
            #   無駄な再起動のコスト(ログが増える)より、守られない建玉の
            #   コスト(2,200円/銘柄+青天井の逆行)のほうが桁違いに大きい。
            #   記録0件のときは回数だけ絞る(本当に何も無い日に40回回さない)。
            if _n_today == 0 and _tries >= 5:
                print("  ⚠ 発注記録0件のまま watcher が5回落ちました。"
                      "\n    建玉が本当に無いなら無害ですが、**kabuステーションで"
                      "建玉照会を1度見てください**", flush=True)
                _gave_up = True
                break
            if _tries >= 40:
        ...
        # ══════════════════════════════════════════════════════════════
        #  6b. 最終防衛線 — 監視をあきらめたら『引成』だけは置いて帰る
        # ══════════════════════════════════════════════════════════════
        # ⛔⛔ 2026-08-19 の実損は「無防備だったこと」ではなく
        #   **引け決済が飛んだこと**で出た(強制決済手数料 2,200円×8銘柄
        #   + 夜間ギャップの全リスク)。
        # ★ 引成(MOC)は **朝のうちに出しておいても大引けで約定する**。
        #   つまり引け決済に限れば、生きたプロセスは要らない。watcher を
        #   動かせないと分かった時点で置いてしまえば、最悪でも
        #   『その日の引けで閉じる』ところまでは保証される。
        # ⚠ 損切り逆指値は消える(建玉拘束のため)。それでも、閉じないより
        #   桁違いにマシ。デイトレ信用は持ち越せない建玉なので。
        if _gave_up:
            print(f"""
{'=' * 74}
▶ 6b. 最終防衛線 — 引け成行(MOC)を置きます
{'=' * 74}
  watcher を大引けまで維持できませんでした。**引け決済だけでも確実にする**
  ため、一般信用(デイトレ)の売建に引成の買戻しを出します。
  ・対象は **MarginTradeType=3(デイトレ)の売建のみ**。買建・制度信用・
    一般長期には触りません(多日保有を誤決済しないため)
  ・発注記録には依存しません(記録が壊れているときこそ動く保険なので)
  ⚠ 既存の損切り逆指値は取り消されます(建玉拘束の回避)。
{'=' * 74}""", flush=True)
            _run("6b. 引け成行の保険 (panic_close)",
                 _PY + ["panic_close.py", "--execute"] + _P)
            print("\n  ▶ 15:30 以降に kabuステーションの **注文照会**で"
                  "約定を必ず確認してください。", flush=True)
