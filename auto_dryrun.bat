...
echo [1/3] kabu STATION 起動待機中... >> "%logfile%"
echo [1/3] kabu STATION 起動待機中... (最大3分)
set retry=0
:wait_kabu
set /a retry+=1
...
if %errorlevel% equ 0 (
    echo [1/3] ✓ kabu STATION 接続OK >> "%logfile%"
    echo [1/3] ✓ kabu STATION 接続OK
    goto kabu_ready
)
if %retry% geq 18 (
    echo [1/3] ✗ kabu STATION 起動タイムアウト (3分超過) >> "%logfile%"
    echo [1/3] ✗ kabu STATION が起動していません。
...
:kabu_ready
echo [2/3] 接続確認完了 >> "%logfile%"

REM bot 起動 (--dry-run、本番環境のリアル価格使用)
echo [3/3] bot 起動 (ドライラン) >> "%logfile%"
echo [3/3] bot 起動 (ドライラン)
echo. >> "%logfile%"
...
REM 終了通知 (任意で30秒待機)
echo.
echo bot 終了しました。日次レポートはログファイルに保存。
echo   ログ: %logfile%
echo.
timeout /t 30 /nobreak > nul
