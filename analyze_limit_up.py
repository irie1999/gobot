...
def main() -> None:
    ...
    # ⛔⛔ **全部ショート表記の picks を黙って受け取らない**(2026-09-11)。
    #   analyze_gap_edge は `--side both` のときしか side 列を作っておらず、
    #   `--side long` の picks が **全部 side=+1** になっていた。
    #   このツールは side>0 で **ストップ高**、side<0 でストップ安 を見るので、
    #   ロングの行をショート表記で渡すと「ストップ安の日にストップ高の板」を
    #   照合することになり、**必ず該当0件**になる。
    #   §18.63 の「鏡像は決済できない日が 0件」はこれで説明がつく。
    #   ⚠ 片側だけの picks は正常なので **止めずに警告だけ**出す。
    _sd = sorted(set(df["side"].tolist()))
    if _sd == [1]:
        print("⚠ この picks は **全行ショート(side=+1)** です。")
        print("   鏡像(ロング)を含むはずなら、analyze_gap_edge を 2026-09-11 "
              "以降の版で作り直してください。")
        print("   それ以前の `--side long` / `--side both` の一部は side 列が"
              "壊れており、**ロングの行がストップ高で判定**されます。")
