...
for sym, ed, om in u[["symbol", "entry_date", "oos_month"]].itertuples(index=False):
    ...
    rec = {
        ...
        "H損切": None, "H目標": None,
        # D の終値 → D+1 の始値。J の合格判定(既定 +75bp)と同じ定義。
        "ギャップbp": (o1 - pc) / pc * 10_000,
...
# ── ギャップ帯別 (2026-08-25) ─────────────────────────────────────
# 問い: 「上げて寄った銘柄ほど、その日 下げるのか」
#
# ★ C を主に見ること。C は **バリアを1つも持たない**(寄りで売って引けで買い戻す
#   だけ)。sm / tm / delay / 資金均等 / 予算 / 発注順 が全部無いので、
#   **パラメータ選択のバイアスが構造的に入らない**。E は同じ母集団に現行の
#   OCO を掛けたもので、そちらは既にこの窓で選んだ設定を含む。
#
# ⚠ 見るのは **単調性だけ**。帯を7つ切れば偶然どれかは良く見えるので、
#   特定の帯を選ぶのは多重検定(§18.13)。単調でなければ軸として機能していない
#   (§18.31 の流動性・§18.38 の建値がそうだった)。
#
# ⛔ この母集団は --raw (lss バックテストの出力) 由来なので、
#   §18.50 の母集団バグを踏んだ生CSVで回すと『ギャップアップして戻らなかった日』
#   = ショートに有利な日 が抜けたまま測ることになる。必ず v20 以降の生CSVで。
_GAP_EDGES = [(-1e9, 0.0), (0.0, 25.0), (25.0, 50.0), (50.0, 75.0),
              (75.0, 100.0), (100.0, 150.0), (150.0, 1e9)]
_gcols = [c for c in ("C_翌寄り→翌引け", "E_翌寄り+OCO") if c in r.columns]
if "ギャップbp" in r.columns and _gcols:
    print(f"\n■ ギャップ帯別 — 「上げて寄った銘柄ほど、その日 下げるか」")
    print(f"  ★ C はバリア無し = パラメータ選択のバイアスが入らない。"
          f"見るのは **単調性** だけ(帯を選ぶのは多重検定)")
    _hdr = f"  {'ギャップ(bp)':<13}{'件数':>7}"
    for _gc in _gcols:
        _hdr += f"{_gc.split('_')[0] + ' 円/件':>13}{'bp':>7}{'t':>7}"
    print(_hdr)
    print("  " + "-" * (20 + 27 * len(_gcols)))
    for _lo, _hi in _GAP_EDGES:
        _sub = r[(r["ギャップbp"] >= _lo) & (r["ギャップbp"] < _hi)]
        if len(_sub) < 20:          # 20件未満の帯は出さない(読み違えのもと)
            continue
        _lbl = ("< 0" if _hi == 0.0 else
                f"{int(_lo)}〜" if _hi > 1e8 else f"{int(_lo)}〜{int(_hi)}")
        _line = f"  {_lbl:<13}{len(_sub):>7,}"
        for _gc in _gcols:
            _gv = _sub[_gc].dropna()
            if _gv.empty:
                _line += f"{'—':>13}{'—':>7}{'—':>7}"
                continue
            _gs = _sub.loc[_gv.index]
            _gbp = (_gv / (_gs["entry_p"] * a.qty) * 10_000).mean()
            _gdm = _gs.assign(_v=_gv).groupby("date")["_v"].mean()
            _gt = ((_gdm.mean() / (_gdm.std(ddof=1) / (len(_gdm) ** 0.5)))
                   if len(_gdm) > 1 else 0.0)
            _line += f"{_gv.mean():>+13,.0f}{_gbp:>+7.1f}{_gt:>+7.2f}"
        print(_line)
    print(f"  ※ t は日クラスタ頑健(同じ日の全銘柄はその日の方向を共有するので、"
          f"件数ベースの t は実効サンプルを誤認する / §18.13)")
