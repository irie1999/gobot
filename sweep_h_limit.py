...
for sym, dstr in sorted(pairs):
    ...
    for off in OFFS:
        ...
        if _auc:
            st["n_auction"] += 1
            st["pnl_auction"] += pnl
            # ⚠ 板寄せで **約定価格ちょうど** の注文は按分(比例配分)になり、
            #    需給が偏ると約定しないことがある。始値>指値 なら全量確実。
            #    寄指運用で唯一残る約定不確実性なので件数を出す。
            if abs(o1 - lim) < 1e-9:
                st["n_at_limit"] += 1
                st["pnl_at_limit"] += pnl
        else:
...
for sp in _splits:
    ...
    print()
    # ── 板寄せのうち『始値=指値ちょうど』= 按分リスクのある分 ──────────
    print(f"  {'指値':>6}{'板寄せ件':>10}{'うち始値=指値':>14}{'比率':>7}"
          f"{'その損益':>13}{'除いた板寄せ損益':>18}")
    for off in OFFS:
        s4 = stat[off][sp]
        na = int(s4["n_auction"])
        if not na:
            continue
        nl = int(s4["n_at_limit"])
        pl = s4["pnl_at_limit"]
        print(f"  {off:>+6g}{na:>10,}{nl:>14,}{(nl / na * 100):>6.0f}%"
              f"{pl:>+13,.0f}{(s4['pnl_auction'] - pl):>+18,.0f}"
              + ("  ←現行" if off == 0 else ""))
    print("  ※ 始値=指値ちょうどの注文は板寄せの**按分**で約定しないことがある。")
    print("     『除いた板寄せ損益』= 全量確実に約定する分だけの成績(最も保守的)。")
    print()
    print("  ※ 板寄せ = 寄り>=指値。始値>指値 なら**全量確実に約定**(現実と一致)。")
