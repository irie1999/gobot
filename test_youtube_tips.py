...
def test_curation() -> None:
    ...
    # 終盤の告知が分断されていても 1 ブロックに結合する (実字幕でよく起きる形)
    body29 = "".join(f"[{m}:00] アドバンテストは9700円割れで損切り 9600円台が支持帯 "
                     for m in range(29))
    frag = ("[29:43] ここからは勉強会のお知らせです "
            "[30:05] いつもご視聴ありがとうございます "
            "[30:31] 参加方法は公式LINEからお申し込みください "
            "[30:55] 多くの方に来ていただいております "
            "[31:19] 有料の講座もご用意しています "
            "[31:45] それではまた次回 ")
    pf = ex.analyze_promotion(body29 + frag, 1980)
    check("分断された終盤告知を 1 ブロックに結合する",
          pf["spans"] == [[1783, 1980]], pf["spans"])
    check("結合後の比率が上がる (約10%)", 0.09 < pf["time_ratio"] < 0.11, pf["time_ratio"])

    # 途中で本編に戻る場合は、その前までは結合しない
    frag2 = frag.replace("[30:55] 多くの方に来ていただいております ",
                         "[30:55] さて相場ですが 9200円割れは撤退です ")
    pf2 = ex.analyze_promotion(body29 + frag2, 1980)
    check("本編復帰を挟んだら手前は結合しない", len(pf2["spans"]) >= 2, pf2["spans"])
    check("本編復帰の後ろは末尾まで伸ばす", pf2["spans"][-1][1] == 1980, pf2["spans"])
