...
<p class="footnote" style="margin-top:16px">
  ※ 「安定型」= 安定性(全期間プラス率)が最も高い銘柄。「取引数型」= 取引回数が多く安定型・WR/PF型ではない銘柄。
    ...
    # ── ③ BTバンド別 銘柄詳細 HTML ───────────────────────────────────────────
    def _all_bands_detail_html() -> str:
        """全BTバンドの (銘柄, 戦略) 別損益表を全期間分生成する。
        バンド切替ボタンで表示を切り替える。
        """
        from collections import defaultdict
        type_colors = {"高WR": "#3b82f6", "高PF": "#f59e0b", "安定": "#10b981", "取引数": "#a855f7"}

        def _render_band_periods(lo: int, hi: int, lbl: str) -> str:
            all_period_html = ""
            for period_key in valid_periods:
                trades_p = all_results[period_key].get("combined_trades", [])
                band_trades = [t for t in trades_p if lo <= (t.get("rec_score") or 0) < hi]
                if not band_trades:
                    all_period_html += (
                        f'<h4 style="color:#94a3b8;margin-top:16px">'
                        f'{period_key}日 — データなし</h4>\n'
                    )
                    continue

                sym_map: dict = defaultdict(list)
                for t in band_trades:
                    sym_map[(t["symbol"], t.get("strategy", ""))].append(t)
                ...
                for r in rows_data:
                    ...
                    rows_html += (
                        ...
                        f'border-radius:3px;font-size:0.75rem">{r["bt_type"]}型</span></td>'
                        ...
                        f'<td style="text-align:center">{"✅" if r["pnl"] > 0 else "❌"}</td>'
                        f'</tr>\n'
                    )

                tp = total["pnl"]
                tc2 = "#4ade80" if tp >= 0 else "#f87171"
                ...
                all_period_html += f"""
<h4 style="margin-top:20px;font-weight:600">
  {period_key}日ホールドアウト — BT{lbl} ({len(band_trades)}件)
  <span style="font-size:0.82rem;font-weight:400;color:{tc2}">合計 {tp:+,.0f}円</span>
</h4>
<div style="margin-bottom:8px">{type_sub}</div>
<div style="overflow-x:auto">
<table>
  <thead><tr>
    <th style="text-align:left">銘柄</th><th>戦略</th><th>タイプ</th>
    <th style="text-align:right">BTスコア</th><th style="text-align:right">BT実績</th>
    <th style="text-align:right">取引数</th><th style="text-align:right">勝率</th>
    <th style="text-align:right">PF</th><th style="text-align:right">損益</th><th></th>
  </tr></thead>
  <tbody>{rows_html}</tbody>
</table>
</div>"""
            return all_period_html

        all_bands = BT_BANDS  # (90,101,"90-100"), (80,90,"80-89"), ...
        first_id = f"band_{all_bands[0][2].replace('-','_')}"

        btn_html = ""
        panes_html = ""
        for i, (lo, hi, lbl) in enumerate(all_bands):
            band_id = f"band_{lbl.replace('-','_')}"
            active_btn = "band-btn-active" if i == 0 else ""
            display = "block" if i == 0 else "none"
            btn_html += (
                f'<button class="band-btn {active_btn}" '
                f'onclick="switchBand(\'{band_id}\',this)">'
                f'BT{lbl}</button>\n'
            )
            panes_html += (
                f'<div id="{band_id}" class="band-pane" style="display:{display}">'
                f'{_render_band_periods(lo, hi, lbl)}'
                f'</div>\n'
            )
...
<h2>③ BTバンド別 銘柄詳細</h2>
<p class="footnote">
  BTスコア帯ごとに銘柄×戦略単位で損益を集計。損益降順ソート。<br>
...
<style>
  .band-btn {{
    ...
    padding:5px 12px; border-radius:4px; cursor:pointer; margin:3px 2px;
    font-size:0.82rem; transition:background 0.15s;
  }}
  .band-btn:hover {{ background:#2d3f55; color:#e2e8f0; }}
  .band-btn-active {{ background:#3b82f6 !important; color:#fff !important; border-color:#3b82f6 !important; }}
</style>
<div style="margin-bottom:16px">{btn_html}</div>
{panes_html}
<script>
function switchBand(id, btn) {{
  document.querySelectorAll('.band-pane').forEach(function(el) {{ el.style.display = 'none'; }});
  document.querySelectorAll('.band-btn').forEach(function(el) {{ el.classList.remove('band-btn-active'); }});
  document.getElementById(id).style.display = 'block';
  if (btn) btn.classList.add('band-btn-active');
}}
</script>"""

    band_detail_tab_html = _all_bands_detail_html()
    ...
    tab_btns  += '  <button class="tab-btn" data-tab="t_bands" onclick="switchTab(\'t_bands\')">③ 銘柄別詳細</button>\n'
    tab_panes += f'<div id="t_bands" class="tab-pane">{band_detail_tab_html}</div>\n'
    for p in valid_periods:
        tab_id = f"t_p{p}"
