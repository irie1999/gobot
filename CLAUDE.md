...
###### ⛔ 訂正 (2026-08-26 実測)。**「✅ が無い → 意味がない」は成立しない**

`verify_barrier_order_5m.py` / 14,644銘柄日 / 2024-07〜2026-08 /
`ret1≥1.753% & gap≥100bp`(= N の母集団を5分足で切り出したもの):

| 損切% | 利確% | 両方タッチ(日足で判別不能) | うち5分足でも不明 | **真実 − 悲観** |
|---:|---:|---:|---:|---:|
| 0.25 | 0.50 | **66.1%** | 44% | **+12.4bp** |
| 0.25 | 1.00 | 48.9% | 25% | +12.3bp |
| 0.50 | 1.00 | 39.9% | 20% | **+17.3bp** |
| 1.00 | 0.50 | 38.7% | 21% | **+31.2bp** |
| 1.00 | 2.00 | 12.0% | 8% | +9.4bp |
| | | | **範囲** | **+6.2 〜 +31.2bp** |

**悲観は真実より 6〜31bp 低く出る。戦略のエッジ自体(+11.8bp / §18.53)より大きい。**
狭いバリアだと日足の**3分の2**が判別不能なので当然。

したがって判定は **合格の向きにしか使えない**:

| 日足の結果 | 読み方 |
|---|---|
| ✅ +2bp 以上 | **信じてよい**。真実はもっと良い(安全側) |
| ⛔ ✅ なし | **「意味がない」の証明にはならない**。悲観バイアスで消えただけかもしれない |

⛔ **宣言を後から書き換えないこと。** 上の表の「✅ が1つも無い → 終わり」は
そのまま残す(結果を見てから緩めると『測ってから良かった方を採る』が再開する)。
変わるのは **不合格だったときの言い方だけ** —「棄却」ではなく
**「日足では測れなかった」**。次にやるなら 5分足がある2年で測り直すことになるが、
それは TEST を消費するので、やるなら1回だけ・条件を先に宣言してから。

⚠ 副産物: この母集団は **09:00 に寄っているのが 69.1% だけ**(09:03 が16.5%)。
§18.44 の「遅寄り2%」は watch50 の数字で、流動性で絞らないとこうなる。
N を watch なしで広げるなら遅寄りの扱いを決める必要がある。
