...
### 18.35b ⛔⛔ 寄り前の気配で判定する案 = **棄却** (2026-08-18 実測で決着)
...
| 案 | 死因 |
|---|---|
| 気配で前倒し判定して **watch上限50件を回避** | 反転率30% |
| 気配で 09:00 の36秒を短縮 | 同上 |

**09:00 の始値方式を維持する。候補は前夜に流動性上位50件へ絞る。**
...
`log_preopen_board.py` のログ収集自体は続けてよい(`.\mtest` に組み込み済み)。
予算を上げた後に watch の価値が跳ね上がった場合、**別ソース(楽天 MarketSpeed II
RSS 等)を検討する材料**になる。ただし kabu + 気配 の線は上記で閉じた。
