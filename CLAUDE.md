...
### 18.35 ☐ TODO(未着手): 寄り前の**気配値・板**で下がりそうか分かるか (2026-08-13 ユーザー依頼)
...
| **1銘柄の上限** | **200万**(=予算の50%) | #3b。合格1銘柄の日が月1.8日ある |
| 重複保有 | **許す**(1銘柄1件にしない) | #3。σ が下がらない |
| **損切り** | 約定値 **+0.1ATR** / **delay4**(20分後に武装) | #2 / delay は d0〜d8 で 月平均/σ ピーク |
| **利確** | 約定値 **−1.0ATR** | #1 |
...
| **2** | **損切 sm** | **0.1ATR** | lss継承→検証済 | ✅ **確定・変えない**(下記) | 済 |
...
| **6** | **価格帯** | 1,000〜6,000円 | ❌ lss継承 | ☐ 未実装 | — |
| **7** | **引け決済** | 15:20 MOC | ❌ lss継承 | ☐ 未実装。sameday5m の改修が要る(重い) | — |
| **8** | **銘柄選定・6戦略** | WF+累積マージ | ❌ lss継承 | ☐ 未実装。再スキャンが要る(最も重い) | — |
| **9** | **lss_proposal_2026-07.py が150件しかない** | — | 選定の不具合 | ☐ 未調査。他の月は621〜1,012件 | — |

⚠ #9 は設定ではなく**データの不具合**。8月は「初出が7月以前のペア」で動くので、
7月の選定が欠けていると **いま唯一の純OOS月(8月)の評価が薄くなる**。
§18.11 の残課題として記録済みだが未着手。次はこれを見る。

##### ★★ #3b 1銘柄の金額上限 = **200万** に決定 (2026-08-15)
...
判定基準は**測る前に宣言してある**(「損益の低下が月次σの半分より小さい範囲で
いちばんきつい上限。一番儲かる上限を選ばない」)。通るのは 200万と150万。

**200万を採る理由**: 150万は損益基準は通るが**主指標(月平均/σ)が 4.58→4.48 と落ちる**。
200万だけが 4.58→**4.83** と上がる。すでに 99%→50% と半減させた後の追加分に、
安定度を払う理由が薄い。

⚠ ツールが「— 測れていない」と出したのは ✅リスク低減の条件を σ −10%以上に
していて 200万が **−9%** で1ポイント届かなかったから。しきい値をまたいだだけで
中身は基準どおり通っている。

★ **副産物: 上限100万なら単元上限(10単元)に当たるのが0件になる。**
§18.38 で「株数で切るのは本来おかしい(1,000円株なら100万・4,000円株なら400万)」と
書いた問題は、金額上限を入れると自然に消える。200万でも 20件→13件 に減る。
...
##### ✅ #2 損切 sm = 0.1 のまま確定 (2026-08-15。delay4 の下で掃いた)
