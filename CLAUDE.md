...
1. **指数のキャッシュが銘柄と同じ日まで来ていること。** 指数が1日古いと
   その日の指数リターンが無く、`prev_z` が市場調整なしで計算されて
   `eligible_side` が誤ります (市場が2%下げた日に個別も2%下げた銘柄は残差
   ほぼゼロなのに、調整なしだと強い下げと判定される)。古ければ**止まります**。
   `.rsi2_cache/^N225.pkl` を消して取り直してください。
