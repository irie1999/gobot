...
##### ⛔⛔ K の実測値は無効 (2026-08-16)。**資金均等の分母が先読みだった**

**推奨タブ `H指値+50bp寄指d4sm0.5資金均等` は、2つの方式の都合の良い半分ずつを
混ぜていた。** §18.38 に記録した K の全数字(月平均 +454,112 / vs H +510,434 /
t=+14.56 / walk-forward 91.8% など)は**測り直しになる。**
...
`.\hvar` を1回。水準表で次の3行を比べる(すべて delay4 / sm0.5 / +50bp で揃う):

| 行 | 意味 |
|---|---|
| `H指値+50bp寄指d4sm0.5資金均等` | **指値 × 候補数割** = 実装できる形 |
| 同 `…約定数割` | 旧挙動。**この差が先読みの大きさ** |
| `H寄り確認+50bpd4sm0.5資金均等` | **確認 × 約定数割** = もう1つの実装できる形 |

判定は §18.36 のルール(walk-forward < 現状なら変えない / 前半・後半が同符号 /
1件あたりで見る)。**上2行の差が大きければ、§18.38 の K に関する結論は
すべて作り直し**になる。
