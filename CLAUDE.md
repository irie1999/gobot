...
### 18.27 ⛔ `compare_lss_rules --holdout-days` × delay1 が異常 (2026-08-08 未解決)
...
有力な仮説: **`stop_from = ei + 1` が日中バーの範囲外に出ると損切りが一度も
arm せず、実質ノーストップで引けまで持つ**。§18.1 の「損切りを引けまで持つ =
-21%(-405万) 論外」と符号・規模が整合する。

検証: `cmp_reason.py`(base と delay1 の決済理由内訳を比較)。
TRAIN窓で delay1 の `close`(引け決済)比率が base より大きく跳ねていれば仮説どおり。
...
**⛔ 解決するまで `compare_lss_rules --holdout-days` × delay 系の数字は使わないこと。**
