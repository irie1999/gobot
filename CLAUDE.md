...
`docs/nikkei_forecast_research.md` に調査メモと手法提案をまとめてある。
既存の逆指値シグナル運用 (§1-§15) とは独立したトラックで、
「日経平均の日中方向を予測して発注を止める / 絞る」ことを目的とする。

| ファイル | 役割 |
|---|---|
| `docs/nikkei_forecast_research.md` | 文献調査・手法の優先順位・落とし穴・N 向け設計 (§10) |
| `docs/preregistration_n225_veto.md` | **第2次検証の事前登録**。先物データ入手前に主判定を固定したもの |
| `n225_research.py` | Phase 0 評価ハーネス (walk-forward + ベースライン + 係数 + バケット表 + HAR-RV) |
| `n225_veto.py` | 日経日中上昇予測で N の発注を止める効果の検証 (6 段階の判定シーケンス) |
| `n225_intraday.py` | 日経225mini 1分足 (JPX) の形式検査と寄り前特徴量化 |

**押さえるべき前提 3 つ:**

1. **リターンは gap / day / c2c に分解して扱う。** 日経の c2c は大部分が gap
   (米国市場の翌朝反映) で、gap の的中率が高くても夜間先物が織り込み済み。
   取引価値があるのは `day` (始値→終値) と `gap_resid` (夜間先物との乖離) だけ。
2. **米国系列は日付インデックス上で必ず 1 日以上ラグさせる。**
   `^GSPC` の D 日足終値は翌 D+1 の 05:00-06:00 JST で、日経 D 日足終値 (15:00 JST)
   より「後」。日付で naive merge すると的中率が 70% 超に跳ね上がる (リーク)。
   `n225_research.build_panel` はカレンダー日グリッド上で shift(1) してこれを防いでいる。
3. **「休めばリスク指標は機械的に改善する」。** veto の効果は「veto なし」ではなく
   2 つの対照群と比較する。(a) 同じ日数をランダムに休んだ場合の分布、
   (b) **一律縮小** — 全日を (1−q) 倍にすると月平均÷σ と Sharpe は数学的に不変なので、
   選択的 veto がこれを上回ったら純粋に「日を選べている」ことになる。
   CVaR と 最大DD は露出に比例して縮むため、月平均で割ってから比較する。
4. **当日ギャップを使う設定は `at_open` であって「寄り前」ではない。**
   日経始値は 09:00:00 の板寄せで確定する。使ってよいのは
   「日経始値の受信 → 予測完了 → 最初の発注送信」の順序が実測で確認できる場合だけ。
   始値を待つなら、**veto するか否かに関わらず毎日待つ**ので、待機コストは
   建てた全日にかかる。`--wait-cost` と損益分岐待機コストで必ず検証すること。

**第2次検証を触る前に `docs/preregistration_n225_veto.md` を読むこと。**
主判定 (モデル・期間・警報率・合格条件) はデータ入手前に固定してあり、
**データを見た後に変更してはいけない**。変更する場合は変更履歴に理由を追記し、
結果を「探索的」として報告する。

押さえるべき事実:
- **08:45 の足は 2016-07-19 以降にしか存在しない。** 大阪取引所のデリバティブ
  日中立会が 08:45 開始になったのがこの日。それ以前は 09:00 開始。
- 08:45〜08:59 の先物だけで特徴量を作れば予測は 08:59:30 に完了でき、
  N の発注 (09:00:01〜) より前になる。**待機コストがゼロになる**のが
  1分足を買う最大の理由で、精度向上より価値が大きい。
- J-Quants DataCube は 2012 年以降なら月単位で購入でき、月額と年額の単価は同じ。
  **必要な月だけ買えばよい**。

どのスクリプトも `--selftest` でネットワーク無しの配管テストができる
(シグナル無し合成データでエッジが出ないこと / 既知シグナルを検出できることを検査)。
評価コードを触ったら必ず全部の `--selftest` を通すこと。

**やらない方が良いこと:**
- 水準予測を MAPE / RMSE / R² で評価すること (ナイーブ予測に負けているのに気づけない)
- 365 日ウィンドウで方向予測を検証すること (58% 未満のエッジは原理的に検出できない)
- veto の閾値・指標・情報境界を全部試して最良の 1 つを採用すること (過学習)
