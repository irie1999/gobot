...
1. **上位N の損益を見る**(生成済み。見るだけ)。同じ実行の中の3か所:
   - 水準(当月除く)の4行 → **円/件** と **月平均÷σ**。総額だけ見ない
   - 対応検定 `…上位N vs …資金均等` → CI がゼロをまたぐか
   - 擬似OOS → 前半・後半が同符号で 5/5勝 か

   予想: **上位8 ≈ 絞りなし**(中央値7件なので半分以上の日で発動しない)。
   上位3 は円/件が必ず上がる(件数が1/2以下)が**改善ではない**。σ は3.4倍に跳ねるはず。
...
| 前提 | 状態 |
|---|---|
| 09:00直後に発注できる(約定=**始値**で近似) | ユーザー方針で確定。**実測は未** |
