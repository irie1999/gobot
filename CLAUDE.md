...
* 先読みを除くと **ランダム順の方が +64,927 良い**。`--rank` 以外は同一条件
  (同じキャッシュ・同じ2,705注文・約定率84%)なので順序の効果だけを見ている。
* **毎朝BT降順で並べる作業には根拠が無い。** 18.12(勝率が全帯フラット)と整合。
* as-of の +188〜241円/件 は 18.20 の『信号のみ』TEST(+189〜+420円/件)と一致する。
