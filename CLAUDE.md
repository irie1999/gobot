...
#### ☐ 1つだけ残っている: 貸借区分(IssType) — これは選別軸ではない

```
貸借銘柄 … 制度信用の売建ができる  → N を実運用で建てられる
信用銘柄 … 制度信用の買いだけ      → **空売りできない**
```

⛔ **バックテストは信用銘柄も売っている。実運用には存在しないトレード。**
Codex の実測: 信用銘柄 **+29.5bp/件** vs 貸借銘柄 **+2.9bp/件**(10倍差)。
t=1.62 で「選別軸として不合格」としたが、**これは選別の問題ではなく
バックテストの水準が過大かどうかの問題**。

★ 2026-09-18 の実運用と向きが一致: 売建規制で建てられなかった3件が **-9,400円**
(建てられない銘柄が成績を押し上げる構造)。

`analyze_margin_days.py` に内訳を出すようにした(判定には使わない)。
**除外して良くなるかを測るのではない。除外した後が本当の水準。**
⚠ 一般信用(デイトレ)なら売建できる銘柄もあるので、実際の可否は証券会社による。
