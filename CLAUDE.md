...
**答え: しない。現行(終日不約定だけ転換)が最良。**
`analyze_tenkan_cutoff.py --days 240 --bt-min 40` (3,415シグナル / delay1 / 補正版fill):

| 締切 | lss | 転換 | 勝率 | PF | 総損益 | 現行比 |
|---|---|---|---|---|---|---|
| **締切なし(現行)** | 3072 | 343 | **53.9%** | **1.98** | **+4,029,306円** | — |
| 09:05 | 2382 | 1033 | 46.9% | 1.16 | +1,000,480円 | **−3,028,826円** |
| 09:10 | 2490 | 925 | 47.7% | 1.25 | +1,486,815円 | −2,542,492円 |
| 09:30 | 2685 | 730 | 49.6% | 1.45 | +2,365,282円 | −1,664,024円 |
| 11:00 | 2887 | 528 | 52.5% | 1.80 | +3,538,754円 | −490,553円 |
| 全部転換 | 0 | 3415 | 30.8% | 0.43 | **−5,006,014円** | −9,035,320円 |
...
09:09時点で実装できる唯一のルール=09:05締切 は +100万(現行 +403万に対し −303万)。
→ レポートの転換タブや `.\tenkan` の数字は **実績記録としては正しいが、
そのまま「これだけ稼げる戦略」として扱ってはいけない**。

**未検証の余地**: 転換のエントリーを遅らせる案(11:30買い→引け売り など)なら
不約定がほぼ確定してから入れるので look-ahead を減らせる。要検証。
