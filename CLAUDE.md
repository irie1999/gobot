...
**判定は `.\ehm` で行う。** 18.32 の t=+2.50 / +2.63 は使わないこと。
...
#### 実測 (`.\ehm` / 10ヶ月 2025-10〜2026-07 / 予算400万 / BT30 / 流動性順 / delay1 / v17 / 手数料0)
...
```
.\ehm                     # 365日窓。3方式の月別+対応検定まで一発
.\ehm --days 730
```
`ehm.bat` が `LSS_OOS_BUDGET_CSV` を **cmd の set** で渡す。⛔ **PowerShell の `set` は
環境変数を設定しない**(`Set-Variable` のエイリアス)ので、手打ちすると CSV が黙って出ない
(2026-08-10 に踏んだ)。手打ちするなら `$env:LSS_OOS_BUDGET_CSV = "..."`。

#### 既知の不具合

`.\ehm` 実行中に `SystemError: deallocated bytearray object has exported buffers` が出る。
E/H の並列データ読込中。**データ不足 636銘柄日**に影響している可能性があるので、
件数が実行ごとに変わらないか確認すること。
