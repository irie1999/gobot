...
| # | 内容 |
|---|---|
| ① | `backtest_limit_entry.py` 「発注中」の日付を **判定日(`fill_start_idx`)で固定**。データが伸びても動かない。最終バーのシグナル(判定日がまだ無い)は従来どおり |
| ② | `run_signals_holdout_all._BT_LOGIC_VER` `v18` → **`v19`**(v18キャッシュは不安定な日付を焼き付けている) |
| ③ | `audit_population.py --gap-bp`(既定75) — `pass_gap` は**その日の閾値**なので、閾値を変えた日をまたぐと比較にならない |

#### ⚠ 残っている『完全性』の穴(不安定ではないが欠けている)

| # | 内容 |
|---|---|
| (b) | **3営業日 触れずに期限切れ**した注文は記録が残らない → その銘柄日は永久に母集団に無い |
| (c) | lss が **2〜3日目に約定**した取引はその日付で記録される。ライブ J は**シグナル翌営業日しか見ない**ので、母集団に余分な日が入る |

**本筋の直し方は `eh_trades` が lss の記録に頼らず、シグナルから自前で (銘柄,日) を
作ること**(= ライブと同じ経路)。①はその前段の応急処置。
