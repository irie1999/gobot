...
```
1. .\daily              ← H の発注まで済ませる(いつもどおり)
2. .\mtest              ← 08:30 までに開始。09:00 過ぎまで自動で回る
3. .\watch              ← mtest が終わったら **すぐ**起動
```
...
⛔ **`.\watch` / 発注サーバと同時に走らせない**(kabu の有効トークンは1つ。401 の取り合い)。
⚠ 09:00 の測定は6秒ほど。**delay1 なので 09:00〜09:05 は元々損切りを置かない**ので、
   この6秒を失っても実害はない(利確が寄り直後6秒で 1.0ATR に届くことはまずない)。
...
⛔ **反転率を±2ポイントで測るには500〜1,000件 = 50件/日なら約1ヶ月。
   貯まる前に何度も覗かないこと**(実質 in-sample になる)。**「N日貯まるまで見ない」と
   先に決めてから始める。**

#### ▶ 積み残し (明日でなくてよい)

- `.\fills` を10営業日ぶん貯めて **実滑り**を確定(§18.37 の残る未知数)
- `.\hvar --days 730` でレジーム検証(いまは365日=13ヶ月)
- `k_open_confirm.py`(K の 09:00判定・株数計算・**dry-run のみ**の発注スクリプト)
  ⚠ ユーザー判断: **まず K は dry-run だけ** / watcher の対応は未決
