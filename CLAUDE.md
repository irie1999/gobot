...
#### ⑪ ⛔ 銘柄・戦略も選択軸にならない (2026-08-24)。選別軸はこれで**6回目**の否定

`analyze_symbol_edge.py`(新規)。**「銘柄ごとに差があるか」は問いとして間違っている**
—— 392銘柄あれば乱数でも必ず差は出る。測るべきは **その差が次の期間でも続くか**。

  ① 全期間の層別ばらつき(出すが「ほぼ意味がない」と明記)
  ② TRAIN の成績で分位 → **TEST の成績** / TRAIN↔TEST の順位相関
  ③ **帰無較正**: ラベルを『同じ日の中だけ』でシャッフル(日効果を保つ / §18.13)

##### ⛔ 銘柄 = **データ量の面から測定不能**(窓を伸ばしても届かない)

| | |
|---|---|
| 現状 | 904件 / **392銘柄** = **2.31件/銘柄**(6ヶ月) |
| 両側6件(計12件)に必要 | 約 **27ヶ月**ぶん |
| ⛔ 5分足の上限 | **25ヶ月**(2024-07 が最古 / 2年ローリング / §18.6) |
| 全データを使っても | 片側 **5.6件** < 6件 |

**両側6件以上ある銘柄は 0。**つまり『まだ分からない』ではなく
**『この戦略では銘柄を選択軸にできない』**が結論。J は +75bp のギャップで
絞るので同じ銘柄が繰り返し出ず(392銘柄に904件が薄く散る)、銘柄別の統計に
必要な反復回数が原理的に集まらない。

⛔ ①の表の上位(6235 オプトラン +631bp / 6件)・下位(3687 フィックスターズ
−140bp / 7件)は **6〜7件の平均**。ここから銘柄を選ぶのは §18.12 の BTスコアと
同じ失敗になる。

##### ⛔ 戦略 = 帰無のど真ん中

```
順位相関      実測 +0.600  / 帰無 +0.391 ± 0.432  → z=+0.48  両側p=0.785
分位スプレッド  実測 +17.2bp / 帰無 +19.1 ± 22.3   → z=-0.09  両側p=0.950
```

**★ 帰無の平均が +0.391 / +19.1bp。**識別力ゼロでもこの水準は普通に出る。
較正なしなら「相関+0.6・上位分位が+17bp 上」で**確実に誤判定していた**。

| 戦略 | TRAIN | TEST | 差 |
|---|---:|---:|---:|
| A7 | **+167.7**(1位) | +104.9(2位) | −62.8 |
| DON | +150.1(2位) | +102.6(3位) | −47.5 |
| MOM | +139.9(3位) | **+120.7(1位)** | −19.2 |
| RSI2 | +97.6(4位) | **+18.0(5位)** | **−79.6** |
| MACDTF | +85.6(**5位**) | +80.9(4位) | **−4.7** |

TRAIN 最下位の MACDTF が最も崩れず、4位の RSI2 が最も崩れた。
§18.36 の戦略別LOO・§18.20 の「TRAINの戦略順位はTESTで再現しない」と
**別手法で同じ答え**。→ **6戦略とも使う**(変更なし)。

##### ⚠ 副産物: 5戦略とも TEST で下がっている

差が全部マイナス(−4.7〜−79.6)。平均 TRAIN +128bp → TEST +85bp で**全体が3割落ちた**。
戦略の差ではなく **2026-06 以降のレジーム**(§18.20「水準は期間で大きく振れる」)。
**選別の話とは無関係だが、直近の水準が落ちていることは頭に入れておくこと。**
...
|---|---|
| §18.12 BTスコア | 全帯フラット |
| §18.13 15軸 × 5分位 = 78検定 | 候補ゼロ |
| §18.24 7軸 | 候補ゼロ |
| §18.31 流動性 | 3度目の棄却 |
| §18.38 建値(上限/下限) | 5回目 |
| **⑪ 銘柄 / 戦略** | **測定不能 / 帰無のど真ん中** |

§18.20 の結論(**「良い銘柄を選ぶ」という発想は捨てる。エッジは信号と機構にある**)を
また補強した。

##### ★ ツールの作法(合成データで3ケース検証済み)

| ケース | 順位相関 | 判定 |
|---|---|---|
| 銘柄・力あり | +0.607 (z=+3.55) | ✅ 正しく検出 |
| 銘柄・力なし | +0.270 (p=0.075) | ❌ 正しく棄却 |
| **戦略・力なし** | **+1.000** | ❌ スプレッドが帰無の中 (p=0.467) |

**3つ目が要点。5グループしかないと、中身が完全にランダムでも順位相関が
+1.000 になる**(5個の順序が偶然一致する確率 1/120)。だから
**順位相関とスプレッドの両方**が帰無の外でないと ✅ を出さない設計にした。
8グループ未満のときは「順位相関の検出力はほぼ無い」と明示する。
...
### 18.49 ★★★ 予算 300万 → 400万 の判断基準 (2026-08-24 に**先に**宣言)

⛔ **データが出てから基準を決めてはいけない**(§18.36 判定ルール / §18.39 の作法)。
実測 2日目の時点で書いた。**10営業日そろったら、この表だけを見て決める。**

#### なぜ判断が要るのか

| | |
|---|---|
| レポート(研究) | **400万**(`LSS_BUDGET_MAN=400`) |
| ライブ | **300万**(`.\jorder --budget 300`) |
| §18.9 の鉄則 | バックテストとライブは揃えるべき |
| §18.37 | 実スリッページ 10営業日が終わるまでサイズを上げない |

そして予算は**レバレッジそのもの**(§18.48 ⑥)。件数はギャップ +75bp だけで決まり、
予算は単元数しか動かさない。300→400 は **損益もσもテールも 1.33倍**、
1銘柄上限も 150万 → **200万** になる。月平均÷σ は変わらない。

#### 測っているもの = 「エントリーが始値からどれだけずれたか」

`slip_daily_log.csv` の **`平均エントリー滑り%`**(1% = 100bp / 符号は
「高く売れていればプラス」= ショートに有利)。
...
**2日で符号が反転している。** ショートは遅れて売るので、寄り後に上がれば有利・
下がれば不利 = **寄り後の数分の方向に賭けている**状態。ただし五分五分ではなく、
§18.44 の1分足実測(2,311件 / 13ヶ月)では **60〜71% が不利側**。
理由は単純で、J が選ぶのは「その日 下げる銘柄」だから、遅れることは
**自分が賭けた動きの序盤を取り逃すこと**を意味する。摩擦ではなく機会損失。

#### ⛔ 訂正: 「60件あるから標準誤差 ±4〜5bp」は誤り

**同日相関**(§18.13)。同じ日の全銘柄はその日の方向を共有するので、
実効サンプルは**取引件数ではなく営業日数**。日クラスタで見ると:

```
2日の日次sd ≒ 38bp → 10日の標準誤差 ≒ **±12bp**（95%CI は ±24bp 程度）
```

つまり 10日で決められるのは **「はっきり悪いか / 悪くないか」だけ**。
−30bp と −15bp の区別はつかない。⚠ 日次sd は2点からの当て推量なので、
**実際の10日から計算し直すこと**。

#### ★★ 判定表 (10営業日そろったら、これだけを見る)

**まず ① 運用ゲート。1つでも落ちたら統計を見るまでもなく据え置き。**

| # | ゲート | 合格条件 | 出所 |
|---|---|---|---|
| G1 | 持ち越し | **0回** | `.\fills` の持ち越し警告 |
| G2 | MOC 設置漏れ | **0回**(合算ログで全建玉ぶん出ている) | `.\jorder` 終了直前 |
| G3 | watcher 即死 | **0回** | `morning_test` の再起動ログ |
| G4 | 約定率 | **10日平均 ≥ 90%** | `約定率%` |
| G5 | 決済のズレ | **10日平均 ±15bp 以内** | `決済滑り/株` |

G1〜G3 は 2026-08-19 の事故(−36,800円)の再発検知。**統計の話ではないので妥協しない。**

**② エントリーの10日平均で決める**(日ごとの平均を10日ぶん平均 = 日クラスタ)。

| エントリー 10日平均 | 日クラスタ t | **判断** |
|---|---|---|
| **≥ −10bp** | \|t\| < 2 | ✅ **400万に上げる**。ノイズと区別できない = 織り込まなくてよい |
| **−10 〜 −25bp** | — | ⏸ **300万据え置き + さらに10営業日**。グレーゾーンは伸ばす |
| **≤ −25bp** | t ≤ −2 | ⛔ **上げない**。③へ |
| **≤ −25bp** | \|t\| < 2 | ⏸ **据え置き + さらに10営業日**（振れが大きいだけかもしれない） |

**③ ⛔ が出たときにやること(この順で)**

| 順 | やること | 判断材料 |
|---|---|---|
| 1 | **板読み秒と bp の関係を見る** | 秒が長い日ほど悪い → **遅れが原因**。速くする余地あり |
| 2 | 秒が短いのに悪いなら | **09:00確認の固有コスト**。速くしても取り戻せない |
| 3 | 固有コスト確定なら | **エッジを −30bp 引いて評価し直す**(約90bp → 約60bp = 1/3減) |
| 4 | それでもプラスなら | 300万のまま継続。**上げるのは別の根拠(σ・資本効率)が要る** |

⛔ **前夜指値に戻す案は検討しない**。§18.40 で fill率19.8%・10ヶ月 −231万 と却下済み。

#### ★ 前倒し条件 (10日待たずに上げてよい)

**6営業日目**の時点で、次を**すべて**満たすなら前倒し可:

- G1〜G5 が全部 合格
- エントリーの6日平均が **±10bp 以内**
- **6日とも符号が同じ日が4日以下**(＝一方向に偏っていない)

#### ⛔ 中止条件 (即座に300万へ戻す / または停止)

400万に上げた**後**でも、次が出たら戻す:

- G1〜G3 が **1回でも**発生
- 1日の損失が **−15万円** を超えた(400万での日次σ ≒ 2.8万 の5σ超 = 想定外の事象)
- 1銘柄の建玉が **200万を超えた**(上限が効いていない = §18.48 ⑦③ と同じ実装バグ)

#### ⚠ この10日で分かること / 分からないこと

| | |
|---|---|
| ✅ 分かる | エントリーが**はっきり不利か**どうか / 運用機構が安定しているか |
| ❌ 分からない | **戦略が儲かるか**。日次σ ≒ 2万(300万)なので10日の合計は σ≒6.3万。 月次で t=2 に届くには **9ヶ月**(§18.15) |
| ❌ 分からない | 不利の**正確な大きさ**(CI ±24bp) |

**「10日プラスだったから上げる」は根拠にならない。** 見るのは損益ではなく
エントリーのbpと運用ゲートだけ。

#### 記録

判断した日に、**何を見てどう決めたか**をここに追記すること(§18.38 の作法)。
10日ぶんの `平均エントリー滑り%` を表で残す。

#### ✅ 決定 (2026-08-24)

**10営業日そろうまで 300万で運用。予算の議論はそれまでしない**(ユーザー判断)。
レポートは400万のまま(揃えるために 300↔400 を行き来させるほうが事故のもと。
`.\fills` は株数不一致を検知して1株あたり(bp)で比較するので、滑りの測定は
この不一致のままで正しく取れる)。

進捗: **2 / 10 営業日**(08-21 +22.7bp / 08-24 −30.9bp)。

⛔ 訂正(2026-08-24): 「08-24 は板読み 6.1秒だったのに −30.9bp」は**誤り**。
その 6.1秒は **08-21** の記録(§18.48 ①)で、**08-24 の板読み秒は記録が無い**。
したがって「速いのに不利 = 09:00確認の固有コスト」という推論は現時点では成り立たない。
確かなのは『読み取りが速いと分かっている唯一の日(08-21)は +22.7bp で有利だった』だけ(n=1)。

⚠ 板読み秒は `slip_daily_log.csv` に**列が無い**。後から遡って取れないので、
§18.49 ③ の1番目(板読み秒 × bp)をやるなら**10日が終わる前に**足す必要がある。

---

### 18.50 ⛔⛔ 過去の結果が実行するたびに変わっていた (2026-08-25 原因特定・修正)

**ユーザー指摘「昨日のテストにはセブンもあった気がします。なぜ消えている？」から発覚。**
同じ日を測り直すとバックテストの取引が入れ替わる。10ヶ月の成績もこの影響を受ける。

#### 機構

J タブは **自前でシグナルを探していない**。lss(逆指値)バックテストが作った
(銘柄, 日) の一覧を借りて「もし J のやり方で建てたら」を計算し直しているだけ
(`eh_trades.py:275` = `for t in list(trades or []) + list(nofills or [])`)。

そして lss の記録の作られ方は3通りで、**2つが不安定**だった:

| lss 注文の運命 | 記録される日付 | 安定 |
|---|---|---|
| 逆指値に触れて約定 | **触れた日**(本物) | ✅ |
| **まだ触れていない** | **実行時の最終バー** `entry_dt=df.index[-1]` | ⛔ **回すたびに動く** |
| **3営業日 触れず期限切れ** | **記録なし** `continue # 期限切れ → 破棄` | ⛔ **永久に消える** |

#### 実例 3382 セブン＆アイ / 2026-08-24

```
08-21(金) 終値 2,030.5 で A7 シグナル → lss は「2,030.5 まで下がったら売る」注文
08-24(月) 始値 2,049.5(+93.6bp) / **安値 2,031.5** → 逆指値に **1円届かず** 未発動
```

| 実行日 | 最終バー | 「発注中」の日付 | (3382, 08-24) は |
|---|---|---|---|
| 08-24 夕方 | 08-24 | **08-24** | ✅ 母集団に入り J が取引を作った |
| 08-25 夕方 | 08-25 | **08-25** | ⛔ **消えた** |

**ライブは 09:00 の始値で判定するので普通に約定している**(実損益 +60円)。
壊れていたのは**採点側だけ**で、発注側は3日とも正しく動いていた。

#### ★ 実測 — 抜け率が古い日ほど単調に増える (`audit_population.py`)
...
**予算でもデータでも相場でも説明できない**(日付の古さとしか相関しない)。
J の日(08-24/25)では **BT投入 306万/313万** = 予算400万に90万の余裕があり、
**予算落ちではない**。「逆(BTのみ)」は0件なのでギャップ判定の食い違いも無い。

#### 08-21(J 初運用日 +16,870円)は J タブに **0件**

`lss_trades.csv`(生)には 113件あるのに `lss_trades_K.csv` は 0件。
`check_day_population.py` で判定を再現すると **5分足なし/分割ガード/価格ATR異常
は全部0件**、90銘柄すべてが「ギャップ不足」。データは無傷。

⚠ ただし **08-21 のライブ閾値は +50bp**(+75bp への変更は 08-21〜22 / §18.48 ④)。
現行の +75bp で揃えると、ライブの7件のうち該当は2件(4661 +168.1bp / 2212 +78.4bp)で、
**その2件とも母集団に無い**。つまり 0件のうち**バグで消えたのは2件**、残り4件は
閾値変更による正しい不合格。

#### 修正 (2026-08-25) — **3段階かかった。1段目・2段目だけでは効かない**

| # | 内容 |
|---|---|
| ① | `backtest_limit_entry` 「発注中」の日付を **判定日(`fill_start_idx`)で固定**。データが伸びても動かない |
| ② | ⛔ **①だけでは1件も直らなかった**。`nikkei_analysis:10544` が『発注中は全部 最終バーの日付』を前提に **最新シグナル日だけに畳んで**いた(実測 94件→19件)。日付が正しくなった以上 畳む理由が無いので撤去し、条件を「**判定日がもう来ている**(シグナル日 < 最終バー)」だけにした |
| ③ | ⛔ ①②でも**期限切れは戻らない**。`if i > po["expire_idx"]: continue # 破棄` で何も残らないため。**期限切れも判定日で1件だけ記録**するようにした(`expired_orders`) |
| ④ | `_BT_LOGIC_VER` `v18` → `v19` → **`v20`** |
| ⑤ | `audit_population.py --gap-bp`(既定75) — `pass_gap` は**その日の閾値**なので、閾値を変えた日をまたぐと比較にならない |

★ **教訓: 母集団の欠落は「エンジンが記録しない」と「レポートが捨てる」の2箇所で起きる。
片方だけ直しても数字は1ミリも動かない**(実測: v19 の再構築後、抜け率 22% が完全に同一だった)。
直したら必ず `audit_population.py` で抜け率が下がったことを確認すること。

#### ⚠ 残っている穴 (不安定ではないが、ライブと完全一致ではない)

| # | 内容 |
|---|---|
| (c) | lss が **2〜3日目に約定**した取引はその日付で記録される。ライブ J は**シグナル翌営業日しか見ない**ので、母集団に余分な日が入る |

**本筋の直し方は `eh_trades` が lss の記録に頼らず、シグナルから自前で (銘柄,日) を
作ること**(= ライブと同じ経路)。①〜③はその前段。

#### ⛔ 損益の向きは**未確定**

理屈では「抜けるのは その日ほとんど下げなかった銘柄日 = 利確に届かないのに損切りには
当たる = マイナス寄り」だが、**これは `1.0ATR% > gap%` のときだけ成立**する。
実際 9279(08-25 / +77.2bp)は**利確に達しているのに抜けている**(低ボラで
利確ラインが前日終値より上)。
...
| 切り方 | 件数 | 抜けの1件あたり |
|---|---:|---:|
| 08-21 を +50bp(当日の閾値)で | 3 | **−413円** |
| 08-21 を +75bp(現行)で揃える | 2 | **+1,315円** |
| 08-24/25 の推定(日足) | 3 | +875円(最大1件を除くと) |

**n=2〜4 で1件が総額の92%を占める。10営業日 貯めるまで結論を出さないこと**(§18.49 の作法)。

#### 使うツール

```
python audit_population.py --since 2026-08-21 --gap-bp 75 --estimate
python check_day_population.py --date 2026-08-21 --compare 2026-08-20
```

⚠ どちらも既存CSVを読むだけ。発注経路には触らない。

#### ⚠ 副産物: 5分足の先頭バーが 09:05 で **90%**

`check_day_population` の実測(08-20/08-21): 09:05 が 89〜92% / 09:00 が 8〜9%。
§18.32 の記録「先頭バーが09:00でない日は **13%**」と**7倍ずれている**。
エントリー価格は日足の始値を使うので直接の影響は無いが、**損切り遅延の起点**が
09:00 でなく 09:05 になる(コード上は「09:00が無い日は delay を1本前倒し」で
補正済み)。記録と実態のズレなので要確認。

---

### 18.51 ⛔⛔⛔ J を止める (2026-08-25 確定)。**エッジの94%は母集団のバイアスだった**

**§18.50 の修正を入れてローリングで測り直したら、バックテスト上のエッジがほぼ消えた。**
ユーザー判断で **J の実運用を停止**。以下は結論と、次に何かを作るときの出発点。

#### 何が起きたか (同じコマンド・同じ窓・修正の有無だけが違う)

| | 修正前 | **修正後** |
|---|---:|---:|
| 予算内の取引(365日) | 1,379件 | **2,028件**(+47%) |
| **月平均** | **+645,245** | **+40,605**(**−94%**) |
| 月次σ | 128,715 | 154,432 |
| **月平均÷σ** | **5.01** | **+0.26** (t≈0.82 = ゼロと区別できない) |
| 資本効率 | 19.84% | **1.15%** |
| **1件あたり(bp / 180日窓)** | **+124.6bp** | **+15.1bp**(**−88%**) |

戻ってきた649件は **「ギャップアップして前日終値まで戻らなかった日」= ショートが負ける日**。
**負ける側だけが系統的に抜けていたので成績が良く見えていた。**

#### ✅ 閉じたことが確認できたもの

| | 証拠 |
|---|---|
| 母集団の (c)「D+2/D+3 約定」 | **`days_to_fill = 0` が 9,467/9,467 (100%)**。`ENTRY_EXPIRE = 0`(翌営業日のみ有効・繰越なし)なので**構造的に存在しない** |
| as-of BT | `[BT出所] 今日のスコア=0件 (0.0%)` |
| per-symbol START_DATES | `全3508件に初出基準月の翌月1日` |
| 選定の TRAIN 期間 | `scan_lss_universe:281` = `fd <= base_end` |
| 手数料 | `FEE_PCT_ONE_WAY = env(既定0)` = 実口座と一致 |
| 同バーの決済 | **損切り優先**(`sameday5m_firsttouch:126`) = 保守側 |
| 決済モデル v16 | 1分足 2,703件と突合 **±260円/件**(§18.22) |
| 価格フィルタ | 「窓内で一度でも範囲内」= 包含方向 |

#### ★★ 残っている『有利に働く要素』の全リスト (2026-08-25 監査)

**A. 測定できているもの**

| # | 要素 | 大きさ |
|---|---|---|
| A1 | **スプレッド = 0**(`_INTRADAY_5M_SLIP=0.0`) | **6.6〜13.2bp** = エッジ +15.1bp の **44〜87%** |
| A2 | **執行が瞬時**(約定=始値) | 1分 −15.8 / 3分 −29.4 / 5分 −36.6bp。**60〜71%が不利側**(§18.44) |
| A3 | 予算 レポート400万 vs ライブ300万 | 月平均 **×0.75** |
| A4 | watch50 の顔ぶれがライブと違う | **10%**(2/20 実測) |
| A5 | 5分足の09:00欠落 | −8.7bp(180日) / +9.5bp(365日) = **符号が窓で反転=ノイズ** |

**A1+A2 だけで 22〜29bp。エッジ +15.1bp を超える。**

**B. 方向は分かるが未測定 (全部 有利側)**

| # | 要素 |
|---|---|
| B1 | **パラメータ7つがこの窓で選ばれている**(delay4/sm0.5/tm1.0/+75bp/ATR14/上限50%/watch50)。**目的関数が94%バイアスだった** |
| B2 | **★ 方式そのものがこの窓で選ばれている**。lss → E → H → J と 08-10〜16 の1週間で4回乗り換え = **巨大な多重検定**。パラメータより桁違いに大きい |
| B3 | 選定(cumul) = TRAIN で勝ったペアを選ぶ → 平均回帰。§18.38 #8 で「選定なしのほうが良い」と実測済み |
| B4 | 流動性が **今日の120日平均**(`_liquidity_of`)。watch50 の切り方に効く(63%の日で上限) |
| B5 | 空売り不可175件が今日のリスト (5.5%) |
| B6 | 生存バイアス(今日の上場銘柄) |
| B7 | 貸株在庫が常にある前提 |
| B8 | 決算・特別気配・IPO を全部建てる |
| B9 | 証拠金の制約なし(1銘柄 最大 予算の50%) |
| B10 | 監査ボードで50行以上を見ている(何か選べば必ず良く見える) |
...
月平均 +40,605(レポート400万)
  A3 ×0.75            → +30,454
  A1 スプレッド        → エッジ 15.1bp → **1.9〜8.5bp**(−44〜87%)
  A2 執行遅延          → さらにマイナス
  A4 / B1〜B10         → **11項目すべて未測定・すべて有利側**
────────────────────────────────
残り                   → **ゼロ、またはマイナス**
```

#### ★ 再開の条件 (データが出る前に宣言する / §18.49 の作法)

**3つ全部を通ったときだけ再開する。**

1. **測定経路を直す**
   - `eh_trades` が lss の記録を借りるのをやめ、**シグナルから自前で (銘柄, 判定日) を作る**(= ライブと同じ経路)。A4 と B4 が同時に消える
   - 流動性を **as-of 化**(その日までの120日)
2. **ローリングOOS**(`run_oos_folds` + `sim_oos_budget`)で **月平均÷σ > 1.0**
   - 同じ窓で掃くと B1/B2 を再生産する。**掃くのをやめる**のが唯一の対策
3. **1件あたりが 呼値往復(6.6〜13.2bp)を明確に上回る**

#### ⛔ やってはいけないこと

- **監査ボードの ✅ を追いかける**。基準が 0.26(ほぼゼロ)なので**何を試しても基準超になる**。
  実際 2026-08-25 のボードでは ✅ が10個以上出たが、その中身は **上位N絞り**(§18.38 で2回棄却)と
  **建値**(§18.13/18.24/18.31/18.38/18.48⑧g で**5回**「識別力なし」)だった
- **方式をもう一度乗り換える**(B2 をさらに悪化させる)

#### 止め方

J は**同日決済**なので、**`.\jorder` を回さなければそれで終了**。建玉も含み損も残らない。

#### 実績 (3営業日 / 参考)

| 日 | 件数 | 実損益 | エントリー |
|---|---:|---:|---:|
| 2026-08-21 | 7 | +16,870 | +22.7bp |
| 2026-08-24 | 5 | +16,690 | −30.9bp |
| 2026-08-25 | 7 | +1,020 | +11.8bp |
| **計** | **19** | **+34,580** | 平均 +1.2bp |

⚠ 3日・19件。**何の証拠にもならない**。運用機構(約定率100% / MOC設置 / 決済モデル)が
正しく動くことは確認できた ＝ **次に何かを作るときの土台としては有効**。

#### ★ 今日いちばん価値があったこと

**ユーザーが「過去の成績がいいのも、測定結果が変わっているだけではないの？」と疑ったこと。**
それが無ければ、94%のバイアスに気づかないまま資金を増やしていた。

**数字が良く見えたら、まず『測っている対象が実際に建てるものと一致しているか』を疑う。**
`audit_population.py` / `check_day_population.py` はそのために作った。**どんな戦略でも使える。**

---

### 18.52 ⛔⛔⛔ ギャップアップの地力 = 不合格。**そしてシグナルは寄与ゼロだった** (2026-08-25)

`analyze_gap_edge.py --workers 8 --split 2026-02-01`
**1,540銘柄 / 625,329銘柄日 / 544営業日 / lss を1ミリも通らない母集団**

#### ★ このツールが安全な理由 (以後この形で測る)

| | これまで | **本ツール** |
|---|---|---|
| 母集団 | lss のバックテストが作る(約定した日だけ) | **シグナル列 `entry_sig` から直接**。`run_limit_backtest` を呼ばない |
| パラメータ | sm/tm/delay/資金均等/予算/発注順/選定 | **ゼロ**(寄りで売って引けで買い戻すだけ) |
| 合格ライン | 測ってから決める | **測る前にコードへ埋め込み**(`PASS_BP` 等) |
| 対照群 | 無し | **シグナルが出ていない日 506,547銘柄日** |
| 帰無較正 | 無し / 日をまたぐシャッフル | **同じ日の中で帯ラベルをシャッフル**(日効果を保つ) |
| ホールドアウト | 下限しか動かない(§18.25 の事故) | **上限で切る** |

⛔ `scan_lss_universe._scan_symbol` も `reason in ("発注中","保有中")` を捨てている。
   **§18.20 の「信号の寄与 +100〜145円/件」も lss が約定した日だけの測定**だった。
   流用してはいけない。
...
|---|---|---:|---:|---|
| ① | bp/件 ≥ 15.0 | **+2.7** | **+2.9** | ⛔ |
| ② | 日クラスタ t ≥ 2.0 | **+0.44** | **+0.15** | ⛔ |
| ④ | 単調 ρ ≥ 0.7 | +0.96 | +0.86 | ✅ |
| ⑤ | 対照 \|t\| < 2.0 | — | t=+0.28 | ✅ |
| ⑤ | 帰無95%点超え | — | +22.2 vs +2.0bp | ✅ |

#### ★★ 決定的な発見: 対照群が同じ形をしている = **シグナルは何も足していない**

bp/件:

| ギャップ | 全期間 シグナル/対照 | TRAIN シグナル/対照 | **TEST シグナル/対照** |
|---|---|---|---|
| < 0 | −3.7 / +0.6 | −4.7 / −0.1 | −1.2 / +2.9 |
| 0〜25 | +2.0 / −2.7 | +1.0 / −3.5 | +5.7 / +0.4 |
| 25〜50 | +4.4 / +0.8 | +5.6 / +0.0 | +0.5 / +3.7 |
| 50〜75 | +4.7 / +5.3 | +4.4 / +4.9 | +5.6 / **+6.5** |
| 75〜100 | +10.3 / +8.4 | +11.9 / +7.6 | +6.5 / **+10.4** |
| 100〜150 | +13.1 / +11.5 | +16.3 / +10.6 | +6.1 / **+13.7** |
| 150〜 | +18.4 / +2.0 | +26.5 / −0.2 | +7.7 / +5.5 |

**TEST では対照のほうが良い帯が3つある。** 単調性(ρ=+0.96/+0.86)も帰無超え(+22.2 vs
+2.0bp)も、**シグナルなしの銘柄で同じように出る**。

→ エッジがあるとすればそれは「ギャップアップ後の寄りのオーバーシュート」という
   **市場全体の現象**で、6戦略のシグナルとは無関係。
   **選別軸に識別力なしの7回目**(§18.12/13/24/31/38/48⑪ に続く)。今回は
   シグナルそのものが対象なので最も重い。

#### ⛔ 帯を選んでも合格しない

TRAIN と TEST の**両方**で ①bp≥15 を満たす帯は**ひとつも無い**:

| 帯 | TRAIN | TEST | 落ちる側 |
|---|---:|---:|---|
| シグナル 150〜 | +26.5 | **+7.7** | TEST |
| シグナル 100〜150 | +16.3 | **+6.1** | TEST(t も +3.56→**+0.71**) |
| 対照 100〜150 | **+10.6** | +13.7 | TRAIN |
| 対照 75〜100 | **+7.6** | +10.4 | TRAIN |

#### ★ 執行コストで完全に閉じる

最良の帯(TEST 対照 100〜150)でも **+13.7bp**。これに対し:

| | bp |
|---|---:|
| **実測のエントリー遅延**(2026-08-24 / 5件) | **−30.9** |
| §18.44 の1分足実測(3分相当) | −29.4 |
| 呼値往復(2〜4ティック) | −6.6〜−13.2 |

**倍以上の赤字。** C は寄りと引けで2回叩くので呼値も2回分。
①を 15bp に置いたのはこれを見越してのことで、**どの帯もどの窓でも届いていない**。

#### ⚠ 設計ミス (記録として)

①②を**全帯合計**で判定する設計にしたが、仮説は「ギャップアップした銘柄」なので
閾値を事前に決めておくべきだった。ただし上表のとおり**閾値をどこに置いても
TRAIN/TEST 両方は通らない**ので判定は変わらない。

#### 残った確かなもの (2つ)

1. **ギャップアップ後の日中反落は実在する**(帰無 +2.0bp に対し実測 +22.2bp = 10倍)。
   ただし **+10〜18bp** で執行コストに届かない。
2. **シグナルは寄与していない。** 「良い銘柄を選ぶ」方向に時間を使う理由がまた1つ減った。

#### ★ これで尽きたもの

**6戦略のシグナル × 同日決済** という組み合わせは、方向(ロング/ショート)・
エントリー方式(逆指値/寄成/指値/09:00確認)・決済(sm/tm/delay)・選定・予算・発注順を
すべて掃いた末に、**素の地力が +2.7bp / t=+0.44** だった。
建て方を工夫しても執行コストを超えない。

---

### 18.53 ★★ ギャップアップの日中反落は**実在する**。ただし①未達で不合格 (2026-08-25 第3回)

`analyze_gap_edge.py --workers 8 --days 4200 --min-gap-bp 100 --split 2020-09-01,2024-03-01`
**1,540銘柄 / 2,671,679銘柄日 / 2,841営業日 / 11.6年 / lss を1ミリも通らない母集団**
...
| 閾値 | **100bp 固定。スイープしない**(第1回・第2回で見た表から決めた値) |
| 母集団 | シグナル不問(2回とも「シグナルの寄与ゼロ」/ §18.52) |
| 執行 | 前夜に寄付指値 → **板寄せ約定 = 呼値を払わない** → 引けMOC = **3.3bp** |
| 検証 | **2015-02〜2020-08 (5.5年)。一度も見ていない** |

#### 判定 = **不合格**。ただし ①だけ、あと 2.5bp

| # | 条件 | 実測 | |
|---|---|---:|---|
| ① | bp/件 ≥ 9.9 (執行3.3bp×3) | **+7.4** | ⛔ |
| ② | 日クラスタ t ≥ 2.0 | **+4.31** | ✅ |
| ④ | 前半・後半とも プラス | +4.9 / +9.4 | ✅ |
| ⑤ | 帰無95%点超え | +10.0 vs −2.7 | ✅ |

#### ★★ 現象そのものは実在が確定した

| 期間 | 件数 | bp/件 | 日クラスタ t |
|---|---:|---:|---:|
| **2015-02〜2020-08(未使用・判定対象)** | 167,954 | **+7.4** | **+4.31** |
| 2020-09〜2024-02 | 121,865 | +7.0 | +4.77 |
| 2024-03〜2026-08 | 99,005 | +9.4 | +5.47 |
| **全期間 11.6年** | **388,824** | **+7.8** | **+8.16** |

**3期間すべてで +7.0〜9.4bp / t=+4.3〜5.5。** 一度も見ていない5.5年が、既見の6年と
ほぼ同じ値。§18.12〜18.52 で **7回否定した選別軸**とは質が違う。

⚠ ただし帯別では 100〜150 が一貫して良く(+7.5 / +8.0 / +12.0)、150〜 は落ちる
(+7.2 / +6.1 / +6.9)。極端なギャップは本物のニュース(決算・上方修正・TOB)で
続伸するため。**帯を選ぶのは多重検定なのでやらない。**

#### 不合格の中身 = 「効果が無い」ではなく「安全マージンが足りない」

```
効果                      +7.4bp   (= +222円/件 / 建値30万)
執行(板寄せ0 + 引けMOC呼値) −3.3bp   (=  −99円/件)
────────────────────────────────
残り                      +4.1bp   (= +123円/件)
合格ライン                 9.9bp
```

①を執行コストの3倍に置いたのは **呼値は下限で実スプレッドはそれ以上**だから。
2.2倍では足りない、と測る前に決めた。**下げない。**

1日13件 / 予算400万なら **月 +3.2万円**。実スプレッドが呼値の2倍なら消える。

#### ★ 「否定された」ものとの区別

| | 状態 |
|---|---|
| §18.18 LDT / §18.19 日中ドリフト / §18.26b 転換 | **否定された**(符号がマイナスかゼロ) |
| §18.12〜18.52 の選別軸7種 | **否定された**(帰無の中) |
| **本節のギャップ現象** | **実在する。ただし安全基準に届かない** |

①は「バックテストの bp がこれだけあれば実執行でも残るだろう」という**代理指標**。
本当に知りたいのは「実執行で +123円/件 が残るか」で、**バックテストでは永遠に
分からない**。→ バックテストでの探索はここで終わり。

#### ⛔ この検証で踏んだ事故 — データが無いのに『不合格』と出た

第3回の初回実行は 判定窓が **1,374営業日 / 1,770銘柄日 = 1日1.3銘柄**(他窓は
212〜1,169銘柄/日)で、中身が空なのに ①②が落ちて「不合格」と表示された。

原因: **`fetch` に `min_start_date` を渡していなかった。**
`backtest_limit_entry.py:456` の else 枝は、`min_start_date is None` のとき
**キャッシュの開始日を一切見ずに返す**(見るのは最新バー日付だけ)。
`--days` をいくら伸ばしてもダウンロード時にしか効かず、キャッシュが2020年始まり
ならそのまま使われる。

対策(実装済み):
1. `min_start_date` を渡す。足りなければ再ダウンロード(`--no-refetch` で従来挙動)
2. **密度ガード** — 判定窓の「銘柄/日」が最も濃い窓の10%未満なら判定を
   **測定不能**にし、①②が落ちても不合格と呼ばない(`--min-density`)
3. 測定不能は **試行回数に数えない**

★ **作法: 窓を分けたら、まず『銘柄/日』を見る。**数字を読む前に、そこにデータが
入っているかを確認すること。**空の母集団は必ず「効果なし」に見える。**

#### 試行記録

`gap_edge_trials.csv` に自動追記。**通算3回**(第1回=シグナル限定 / 第2回=閾値なし /
第3回=本節)。3回試すと帰無でも14%はどれか通るので、合格が出たときは必ず併記する。

---

### 18.54 ★★★ 選別軸が初めて見つかった — **前日リターン Q5**(2026-08-25 第4回)

**TRAIN で掃き、TEST で1回だけ確かめる**という正しい手順を初めて実装して得た結果。
§18.12〜18.53 で **7回**「選別軸に識別力なし」と結論してきたが、**8回目で出た**。

#### ★ そもそも私(Claude)が禁止しすぎていた

「パラメータを足して救わない」という文言が、次の2つを一緒に禁止していた:

| | 判定 |
|---|---|
| 同じデータで測ってから良かった方を採る | ⛔ 8月に方式を4回乗り換えたのはこれ |
| **TRAIN で探索し、TEST で検証する** | ✅ **統計の標準手順。禁止する理由はない** |

ユーザー指摘(2026-08-25「trainの期間に、それを行い、検証すればいいと思いますが」)で
後者を解禁し、`--explore` / `--confirm` を実装した。

#### 手順 (これが本体。結果より作法が重要)

```
# ① TRAIN(最も古い窓)だけで軸を掃く。**TEST は集計すらしない**
python analyze_gap_edge.py --workers 8 --days 4200 --min-gap-bp 100 \
                           --split 2020-09-01 --explore

# ② 出た候補を **1候補につき1回だけ** TEST で確かめる
python analyze_gap_edge.py ... --confirm ret1:Q5
```

- 探索モードは **TEST を画面にも出さない**(誤って見ないためのガード)
- 分位の境界は **TRAIN で決めて TEST に当てはめる**(TEST の分布で切り直したら再探索)
- 帰無較正は **実測と同じ『最良分位を選ぶ』操作**を掛ける(最大を選ぶだけで z は
  平均+1ずれる / §18.34b)

#### TRAIN の探索結果 (11軸 × 5分位 / 167,954銘柄日)

| 軸 | 最良 | bp | t | 帰無95% | 判定 |
|---|---|---:|---:|---:|---|
| **売買代金20日平均** | Q1 | **+16.7** | +8.71 | +10.3 | ✅ 候補 |
| **前日リターン%** | Q5 | +15.5 | +3.01 | +12.0 | ✅ 候補 |
| 連続上昇日数 | Q3 | +14.6 | +3.81 | +8.8 | ✅ 候補 |
| 建値 | Q5 | +11.5 | +3.30 | +9.7 | ✅ 候補 |
| ATR%(ボラ) | Q2 | +10.0 | +5.59 | +12.9 | — |
| 20日レンジ位置 / 5日 / 20日リターン / 出来高比 / ギャップbp | — | — | — | — | — |

候補4個(帰無の期待0.6個)。**2つの系統に分かれる**:

| 系統 | 軸 | 実運用 |
|---|---|---|
| 薄さ・値がさ | liq:Q1 / entry_p:Q5 | ⛔ **執行コストと正面衝突**(このツールは呼値3.3bp固定) |
| **過熱** | **ret1:Q5** / up_streak:Q3 | ✅ 流動性と無関係 |

→ **実運用できないものを検証しても意味がない**ので ret1:Q5 を選んだ。
   up_streak:Q3 は同じ「過熱」仮説なので試さない(TEST の使用回数を節約)。

#### ★★ TEST の結果 = **4条件すべて合格**

**仮説: 前日に +1.75% 以上上げた銘柄が、翌日さらに +100bp 以上ギャップアップして
寄ったら空売りし、引けで買い戻す。**

| 分位(境界はTRAINで決定) | TEST 件数 | TEST bp | TEST t | TRAIN bp |
|---|---:|---:|---:|---:|
| Q1 | 45,240 | +0.6 | +3.13 | +1.7 |
| Q2 | 47,401 | +6.6 | +5.16 | +4.2 |
| Q3 | 42,959 | +5.5 | +6.04 | +8.6 |
| Q4 | 47,473 | +10.0 | +3.98 | +6.9 |
| **Q5 (前日 ≥ +1.753%)** | **37,797** | **+19.4** | **+4.70** | **+15.5** |

**TRAIN も TEST もほぼ単調で Q5 が突出。**

| # | 条件 | 実測 | |
|---|---|---:|---|
| ① | TEST bp ≥ 9.9 | **+19.4** | ✅ |
| ② | TEST 日クラスタ t ≥ 2.0 | **+4.70** | ✅ |
| ③ | TEST で絞らない場合を上回る | **+11.3** | ✅ |
| ④ | **TRAIN でも効いている** | TRAIN +8.1 / TEST +11.3 | ✅ |

#### ⛔ 先に否定された候補 — ATR%Q1 (④が効いた実例)

`--explore` を飛ばして直接 `--confirm atr_pct:Q1` を実行したところ ①〜③が通った
(TEST +11.9bp / t=+8.70)。しかし後から TRAIN を掃いたら:

* TRAIN の最良は **Q1 ではなく Q2**
* その Q2 でさえ **帰無の中**(+10.0 vs 帰無95% +12.9)
* TRAIN での改善 **+0.2bp** に対し TEST だけ +3.8bp

→ **TEST 期間のノイズ。t=+8.70 でも、である。**
§18.13 の『TRAIN で効いていないものを候補にしない』が実際に効いた。
このとき ③ が「TEST で絞らない場合を上回る」だけで TRAIN を見ていなかったので、
**④を追加した**(合格の後に基準を足した形になったのは事実。ただし §18.13 に
既に書いてある作法を実装し忘れていただけ)。
...
執行(板寄せ0+引けMOC)   −3.3bp
────────────────────────────
残り                 +16.1bp  = +483円/件 (建値30万)
```

第3回(絞らない +4.1bp / +123円/件)の **約4倍**。候補は **25.9件/日**(TEST) /
24.9件/日(TRAIN)で安定。
...
| 実スプレッド | 残り | 円/件 |
|---|---:|---:|
| 呼値どおり 3.3bp | +16.1 | +483 |
| 呼値の3倍 10bp | +9.4 | +282 |
| 呼値の5倍 16bp | +3.4 | +102 |

第3回は呼値が少しでも増えれば消えたが、**今回は3倍でも生き残る**。

#### 多重検定

11軸 × 5分位 = **55通り**から選んで TEST を2回使った(atr_pct:Q1 → ret1:Q5)。
t=+4.70 は 55倍しても p<0.001 で生き残る。そして **④を満たしている**ので
期間依存ではない。

⛔ **残り2候補(liq:Q1 / up_streak:Q3 / entry_p:Q5)は試さない。**
試すたびに TEST が既見になる。

#### ⚠ 残る懸念

1. **実執行コストは未検証**(§18.53 と同じ)。ただしマージンが4倍あるので耐性が違う。
2. **候補が予算を超える**。25.9件/日 に対し予算400万で建てられるのは13件。
   選び方を探すとまた多重検定になるので、**流動性降順**(§18.21 / §18.24 で
   「発注順は何にしても区別できない」)で埋めるのが筋。
3. **好材料銘柄の踏み上げ**。前日+1.75%→さらにギャップアップ = 好材料が出た銘柄。
   §18.53 で「150〜 は落ちる(本物のニュース=続伸)」と出ている。同日決済なので
   損失は当日の値幅で頭打ちだが、ストップ高に張り付くと逃げられない。

#### ⛔ この検証で直したツールのバグ2件

| # | 内容 |
|---|---|
| 1 | **曜日の帰無較正が効いていなかった**。同じ日の全銘柄は同じ曜日なので、日の中で分位ラベルをシャッフルしても何も変わらない = 帰無 == 実測。実測/帰無中央/帰無95% が3つとも +21.7 になって発覚。日の中で値が一定の軸は『較正不能』として候補から外す |
| 2 | `--confirm` が **TRAIN での効きを見ていなかった**(上記 ATR% の件)。④を追加 |

#### ★ レポートに新方式Nのタブを追加 (2026-08-25)

`.\dailyfast --days 365` の損益タブに **「★ 新方式N 前日%+ギャップ」** が出る。
既存の J/K/L と同じ `_dup_toggle_html`(月別サマリー + 日別アコーディオン + 明細)
を通すので見た目は揃っている。独自ブロックは「条件 / 1件あたりbp / 50件の壁 /
月別の候補数」だけ。

⛔ **母集団が J/K/L と違う**。あちらは `eh_trades`(lss のバックテストが作る
   銘柄日)、こちらは **全銘柄日を日足から直接**。5分足も lss も通らない。

シミュの順番が実運用そのもの:
  ① 前夜の候補(前日リターン) → ② **watch上限50件**(流動性降順)
  → ③ 09:00 のギャップ判定 → ④ 予算(ギャップ降順に充当)

env: `LSS_NEWGAP_TAB` / `_RET1`(1.753) / `_BP`(100) / `_WATCH`(50) /
     `_BUDGET`(400) / `_WORKERS`(8)

##### ★★ 実測 (365日窓 / 13ヶ月 / 予算400万)

| | |
|---|---:|
| 合計 | **+1,549,342円** / 1,993件 / **+24.5bp** |
| 月平均 | **+119,180円** / 月次σ 213,506円 / **月平均÷σ 0.56** |
| 勝日 | 135/252 |

`analyze_gap_edge.py` の TEST(+89,364円 / 0.59)と整合。**別経路・別実装で
同じ水準**が出たので、実装は正しく動いている。

##### ⛔⛔ 50件の壁が最大のボトルネック

| | |
|---|---:|
| 前夜の候補 | 中央値 **120件/日** / 最大 **957件** |
| 候補が50件を超えた日 | **206/252日 (82%)** |
| **取り逃した**(ギャップ条件は満たしていたのに板を読めなかった) | **5,473件** |
| 建てた | 1,993件 → 取り逃しは **その275%** |

**建てた数の2.75倍を取り逃している。** kabu の登録上限50件(§18.44)を外せれば
理論上3倍以上になるが、§18.44/§18.45 で「バッチ回しは速くならない」
「299銘柄で3.6分」と棄却済み。**別ソース(楽天 RSS 等)が要る。**

##### ⛔ この実装で踏んだバグ2件

| # | |
|---|---|
| 1 | **銘柄コードを変換していなかった**。`available_local_symbols()` は 5分足のファイル名 = J-Quants の5桁コード(末尾0)を返す。`21200` をそのまま yfinance に投げて全1,540銘柄が `possibly delisted` で失敗。`analyze_gap_edge._jq_to_yf` と同じ変換(`21200`→`2120.T`)が要る |
| 2 | 独自形式で描いてしまい「他のtabと形式が異なりすぎている」。既存の `_dup_toggle_html` に載せ替えた |

---

### 18.55 ⛔ §18.51 の訂正 — 並べすぎた (2026-08-25 ユーザー指摘)

**J が崩れた明らかな原因は 母集団バグ(§18.50) ただ1つ。**
§18.51 で B1〜B10 として並べた「有利に働く要素」のうち、実際に効いていたのは
これだけだった。それ以外は **OOS のルールに則って除去済み**だった:

| | 状態 |
|---|---|
| as-of BT(§18.11) | ✅ 除去済み |
| per-symbol START_DATES(§18.29) | ✅ 適用済み |
| ローリングOOS(§18.23) | ✅ 実施済み |
| walk-forward 判定(§18.36) | ✅ **手順は正しい** |

⚠ 唯一残るのは「その walk-forward 自身が**汚染データの上で走っていた**」こと。
§18.36 で「設定は全部変えない」と結論したのも、94%がバイアスの数字を見ての判断。
**手順の問題ではなく入力の問題。**

★ そして **J を止めた理由を取り違えないこと**。「手順が悪かった」のではなく、
**直したらエッジが無かった**(月平均 +40,605 / 月平均÷σ 0.26 / t≈0.82)。

#### ★ 作法: 原因を並べ立てない

数字が崩れたとき、思いつく限りの「有利に働く要素」を列挙するのは**分析ではない**。
実際に効いていたものを1つ特定し、**残りは『検証済み』と『未検証』に分けて**示すこと。
並べ立てると、正しく除去できていた手順まで疑わしく見え、判断が止まる。

#### ☐ 進行中: 損切り/利確・2次元・運用パラメータのスイープ (2026-08-26 実行予定)

**すべて TRAIN(2015-02〜2020-08) のみ。TEST は1回も使わない。**

★ 作法: **「効果がある」の主張には TEST が要るが、「効果がない」の確認は
TRAIN で完結する**(TRAIN で効かないものが TEST で効くことは期待できない / §18.13)。

```
python analyze_gap_edge.py --workers 8 --days 4200 --min-gap-bp 100 --split 2020-09-01 --sweep-barrier
python analyze_gap_edge.py --workers 8 --days 4200 --min-gap-bp 100 --split 2020-09-01 --sweep-grid
python analyze_gap_edge.py --workers 8 --days 4200 --min-gap-bp 100 --split 2020-09-01 --sweep-ops
```
...
| 日足 TRAIN の結果 | 次 |
|---|---|
| **✅ が1つも無い** | **「意味がない」で確定。終わり**。5分足は不要 |
| ✅ はあるが差が **+2bp 未満** | 誤差。終わり |
| ✅ が **+2bp 以上** かつ件数が大きく減らない | **5分足で検証に進む** |

+2bp は呼値往復(3.3bp)の6割。それ未満なら実運用では消える。

⚠ 日足の判定は **バリアに有利**(両方触れた日は損切り優先=悲観だが、約定は
ラインちょうど=楽観)。**ここで勝てないなら正確に測っても勝たない。**

###### 5分足で検証: 悲観バイアスは **小さい** (2026-08-26 実測)

`verify_barrier_order_5m.py` / 14,644銘柄日 / 2024-07〜2026-08 /
`ret1≥1.753% & gap≥100bp`。**幅は ATR倍**(ATR% は日中レンジ中央値 3.05% で代理)。

| 損切ATR | 利確ATR | 両方タッチ(日足で判別不能) | **真実 − 悲観** |
|---:|---:|---:|---:|
| 0.1 | **0.0** | **0 (0.0%)** | **+0.0bp** |
| 0.1 | 1.0 | 1,667 (11.4%) | +3.3bp |
| 0.3 | 1.0 | 964 (6.6%) | +4.7bp |
| 0.5 | 1.0 | 619 (4.2%) | +4.9bp |
| 0.1 | 2.0 | 307 (2.1%) | +0.3bp |
| | | **範囲** | **+0.0 〜 +4.9bp** |

**悲観バイアスは +0〜4.9bp。日足の判定はほぼそのまま読んでよい。**
とくに **利確なし(tm0)は両方タッチが構造的に0なのでバイアスも0**。

日足スイープ側の実測(両方タッチ 5% / sm0.1 tm1.0)とオーダーが一致する。

##### ⛔⛔ この節を書くときに1度 間違えた。同じ轍を踏まないこと

最初の測定は幅を **%** で取っていた(0.25〜1.0% / 0.5〜2.0%)。日足スイープは
**ATR倍**なので、利確が **3倍狭かった**。その結果:

| | 両方タッチ | 真実−悲観 |
|---|---:|---:|
| ⛔ 誤(% で測定) | **49%** | **+6 〜 +31bp** |
| ✅ 正(ATR倍で測定) | **5〜11%** | **+0 〜 +4.9bp** |

**一桁 過大評価**した。そして「エッジ(+11.8bp)より大きいので、日足の判定は
合格の向きにしか使えない」という**誤った訂正**を CLAUDE.md に書いた(撤回済み)。

★ **作法: 2つの測定を突き合わせる前に、まず単位が同じか確かめる。**
  片方が ATR倍・片方が % のような食い違いは、値がもっともらしいので気づけない。
  気づけたのは日足スイープの「両方触れた 5%」と自分の 49% を並べたときだった。
  **必ず『同じ量』を両方から出して照合すること。**

##### ⛔ なぜ日足で測るのか — 分足は TRAIN に無い

| | 期間 |
|---|---|
| 日足(yfinance) | **2015-02〜(11.6年)** |
| **5分足 / 1分足(J-Quants)** | **2024-07〜(2年)**。分足アドオンは全プラン2年ローリング(§18.6) |

**TRAIN(2015-2020)には5分足が1日も無い。** だから日足で測るしかない。
日足の限界(高値/安値の順序が分からない)には **損切り優先(悲観)** で対処する
(`sameday5m_firsttouch` と同じ / §18.51 C)。`--stop-slip-pct 0.005` で約定も悲観にできる。

##### 5分足に進む場合の設計 (先に決めておく)

| | |
|---|---|
| パラメータ | **日足 TRAIN で決定済みのものをそのまま使う**(5分足で探索しない) |
| 5分足の役割 | **検証のみ**(順序と約定価格を正確に測る / §18.22 で1分足2,703件と突合済み) |
| コスト | **TEST を1回消費**(5分足がある2年は TEST 期間内) |

⛔ **5分足で探索を始めた瞬間に多重検定が再開する。** そこは固定する。

##### ⛔ delay は測れない

分足が要るが TRAIN に無い。そして **N はバリアが無いので delay の概念自体が無い**。
①で損切りが「意味なし」と出れば delay も不要。**順序として①が先。**

---

### ★★★ 結果 (2026-08-26)。**バリアは不採用。宿題はすべて終わった**

`analyze_gap_edge --days 4200 --min-gap-bp 100 --split 2020-09-01` の3本 + 滑り版。
TRAIN(2015-02〜2020-08 / 167,880銘柄日 / 1,347営業日)のみ。**TEST は1回も使っていない。**

#### ① 損切り/利確 = **不採用**。滑り0.5%で ✅ が全滅

現行(バリアなし) = **+175円/件**。現行との差(円/件):

| 損切ATR | tm0.0 | tm1.0 | tm2.0 | | 滑り0.5%: tm0.0 | tm1.0 |
|---:|---:|---:|---:|---|---:|---:|
| 0.1 | **+190 ✅** | +152 ✅ | +184 ✅ | | **−628** | −667 |
| 0.3 | +104 ✅ | +52 ✅ | +97 ✅ | | −476 | −528 |
| 0.5 | +57 ✅ | +5 ✅ | +50 ✅ | | −332 | −384 |
| 1.0 | +7 ✅ | −37 | +0 ✅ | | −124 | −168 |
| 2.0 | −10 | −43 | −15 | | −32 | −65 |

**滑りを入れると24セル全部マイナス。事前宣言「✅ が1つも無い → 意味がないで確定」に該当。**

##### なぜ構造的に無理か — 分岐点は **2.6ティック**

2つの実行の差だけから逆算できる(建値は sweep-ops の 投入162万/日 ÷ 7.18件/日 ÷ 100株 = **2,257円**):

```
滑り0.5% のコスト = 190 −(−628) = 818円/件
  検算 0.005 × 2,257 × 100株 × 発動率77% = 869円  ✅ ほぼ一致
利得が消える滑り  = 190 ÷ (818/0.5) = **0.116% = 2.6円 = 2.6ティック**
```

* **狭い損切り(sm0.1)** … 効果はあるが **77%が発動**するので滑りに極端に敏感。2〜3ティックで消える
* **広い損切り(sm2.0)** … 滑りに鈍感だが **滑り0%でも効果がない**(−10)

**この非対称は幅を変えても解けない。** 損切りの成行買い戻しは価格が上に走っている
瞬間なので、平均2.6ティックに収まる見込みは無い(§18.17 の実測は無保護窓明けで
−1,430円/件 の事例)。

⚠ **悲観バイアスは言い訳にならない。** 5分足で実測して +0〜4.9bp、
利確なし(tm0)なら **0**(両方タッチが構造的に0)。−628円/件 = −27.8bp は埋まらない。

##### → **delay も不要**(宣言どおり。①が「意味なし」なら delay の出番が無い)

#### ② 前日リターン × ギャップ = **現行のまま**

現行 `ret1 ≥ 1.753% × gap ≥ 100bp` = **+15.5bp / 24.3件/日 / t=+3.02**

* **bp は厳しくするほど単調に上がる**(3.5% × gap300 = +54.1bp)が **2.0件/日**しか出ない
* **t は緩いほど高い**(0.000 × gap50 = +4.07 / 122件/日)
* 現行はその中間。どちらの端にも寄っていない = **動かす根拠が無い**

⚠ bp が高い升は件数が予算(1日十数件)を下回るので月の総額は落ちる。
**bp と 件/日 は必ずセットで見る。**

#### ③ 運用パラメータ = **全部そのまま**

現行(watch50 / 予算400万 / 上限なし) = +3,339,673円 / 9,670件 / +345円/件 / **月 +49,587円**

| つまみ | 結果 |
|---|---|
| **watch** | 50 → 無制限で 月 +49,587 → **+87,267 (+76%)**。⛔ **kabu の登録上限50件**で実装不可(§18.44/§18.45) |
| **予算** | 400→600万で 月 **+5,611**、600万で頭打ち。レバレッジ = リスク許容度の宣言(§18.38 #3b) |
| **1日の件数上限** | 20 が 月 +280 だけ上 = 誤差 |
| **同一銘柄 1日1回** | 差 **±0** |

唯一の改善余地が watch で、それは kabu では届かない。

#### ★ N の確定仕様

| | |
|---|---|
| 前夜 | 前日リターン **≥ +1.753%** / 建値 1,000〜6,000円 / 流動性上位 **50件** |
| 09:00 | 始値が前日終値 **+100bp 以上**なら空売り |
| **決済** | **引け。損切りも利確も置かない**(バリア不採用) |
| サイズ | 予算400万 / 1日の件数上限なし / 同一銘柄の制限なし |

⚠ **損切りを置かないので、ストップ高に張り付くと逃げられない**(ショート)。
ただし §18.53/§18.54 の +19.4bp は**元からバリアなしで測った数字**なので、
新しく増えたリスクではない。同日決済なので損失は当日の値幅で頭打ち。

#### ★ 月いくらか (2026-08-26)

⛔ **バックテストの bp は執行コストを差し引いていない。** `EXEC_BP` は
   合格ラインを作るのに使うだけで、`+345円/件 = +15.3bp` は **グロス**。

| | 月額 | 根拠 |
|---|---:|---|
| TRAIN (2015-02〜2020-08 / 67ヶ月) | **+49,587円** | 今日の `--sweep-ops`。watch50 + 予算400万 + バリアなし = 確定仕様そのもの |
| TEST (2020-09〜2026-08 / 72ヶ月) | **+89,364円** | §18.54 の記録。σ 150,395 / t=+5.04 / プラス月 54/72 |

**TEST が TRAIN の1.8倍。エッジが強くなったのではなく期間差**(§18.20 と同じ形)。

##### 執行コストを引く

建値は 投入162万/日 ÷ 7.18件/日 ÷ 100株 = **2,257円**。呼値1円 = **4.4bp**。

**エントリー遅延は N 自身の母集団で実測した**(5分足 14,644銘柄日):

| 遅れ | 中央 | 平均 | 始値より不利 |
|---|---:|---:|---:|
| 5分後 | −10.4bp | −10.4bp | 53% |
| 10分後 | −12.9bp | −9.4bp | 54% |
| 15分後 | −12.4bp | −8.4bp | 53% |

**5分で −10.4bp、そこから頭打ち。** §18.44 の J 母集団(5分 −36.6bp)の**3分の1以下**で、
N のほうがずっと執行に強い。§18.44 の凹型(1分が5分の43%)で按分すると
**36秒(§18.44 の板読み実測)で ≒ −3bp**。

| 前提 | 残る bp | 残る% | TRAIN基準 | TEST基準 |
|---|---:|---:|---:|---:|
| 保守(成行・片道1ティック −4.4) | +7.9 | 51% | +25,482 | +45,923 |
| 中庸(保護指値で半分 −2.2) | +10.1 | 66% | +32,668 | +58,874 |
| 楽観(板寄せ並み −0) | +12.3 | 80% | +39,855 | +71,825 |

**→ 現実的には 月 +2.5〜6万円(予算400万 = 月利 0.6〜1.5%)。**

##### ⚠ 数字を見るときの注意

1. **単月では普通にマイナスになる。** TEST の月次σ 150,395 は月平均の **1.7倍**。
   プラス月は 54/72 = **75%**
2. **1日で最大 −167,143円**(予算400万・バリアなし・2024-2026の実測)。
   資本の **4.2%**。損切りを置いていないので、これが構造的な上限
3. **稼働率40%**(投入162万/日 ÷ 予算400万)。資金は余っているが
   **watch50 が律速**で買う対象がない(§18.55 ③)
4. ⛔ **シミュは 09:00確認の機構(gapで並べて予算充当)なのに、執行コストは
   auction(呼値なし・遅延なし)の前提**。上の控除はその食い違いを手で埋めたもの

#### 残る未検証 = 執行コストだけ

バックテストでやることは無くなった。**前向きに小ロットで実約定を測る段階**
(§18.53 の「バックテストでの探索はここで終わり」に到達した)。
実測は `.\norder`(1円も発注しない板記録)+ 引け後 `python n_paper.py --close`。

#### 試行記録

`--sweep-*` は判定を出さないので試行に数えない(閾値を宣言して仮説を測ったのは
§18.53 の1回 + §18.54 の `--confirm` 2回)。

---

### 18.56 ★★ 鏡像(ギャップダウンを買う)と両建て (2026-08-26〜27)

**N の対策を8系統掃いて全滅したあと、ユーザーの「何かしら考えてほしい」から出た案。
これだけが生き残った。**

#### ① まず: N 単体の対策は8系統すべて否定 (2026-08-26)

| 軸 | 結果 |
|---|---|
| その日の合格件数 | 相関 −0.028。ワースト8日中5日が1〜7件の日 |
| 建値 | 相関 −0.013 |
| **決済時刻** | 5分 +10.3 → 11:05 +10.5 → **引け +17.8bp**。**早いほど悪い** |
| **寄り5分での撤退** | **逆効果**(−100bp逆行組はその後 **+18bp 戻る**) |
| 損切り / 利確 (ATR倍) | 滑り0.5%で24セル全部マイナス(§18.55) |
| **固定円の損切り** | 下記② |
| **ギャップ上限150bp** | 月 +49,587 → **+21,075(43%)**。件数も円/件も落ちる |
| 寄り前の市場変数19軸 / 銘柄属性15軸・7軸 | 候補ゼロ(§18.34b / §18.13 / §18.24) |

★ ギャップ上限が効かない理由: §18.53 の帯別表では 150bp〜 が劣位だったが、
あれは `ret1` フィルタなし。**「前日も上げて、さらに大きくギャップアップ」=
過熱の極みが一番効く**ので、`ret1` と組み合わせると劣位が消える。

#### ② 固定円の損切り = **利益減少**。ただしテールは買える

5分足 subset 13,908銘柄日(2024-07〜2026-08)。**平均はどの水準でも下がる**:

| 損切り | 発動率 | 損益/件 | 現行との差 | 1tick滑り | 2tick |
|---|---:|---:|---:|---:|---:|
| なし(現行) | 0% | **+486円** | — | +486 | +486 |
| −5,000円 | 33% | +451 | −35 | +353 | +255 |
| **−10,000円** | 15% | +425 | **−61** | +380 | +336 |
| −20,000円 | 4% | +428 | −57 | +415 | +402 |
| −30,000円 | 1% | +451 | −35 | +447 | +442 |

| | 最悪の1件 | 最悪の日(予算400万換算) |
|---|---:|---:|
| なし | **−76,000円** | **−201,977円** |
| −10,000円 | **−10,000円** | −137,490円(**32%改善のみ**) |

**1件の最悪は7分の1になるが、最悪の日は32%しか改善しない。**
固定1万円は建値27万の3.7%で日中の値幅として普通に起きるため、
**悪い日は全ポジションが同時に止まる**。→ 採否はリスク許容度の宣言であって
成績の最適化ではない(§18.38 #3b と同じ性質)。

⚠ 測定期間が TEST 寄り(2024-07〜)なので、**設定を選び直す根拠にはしない**。

#### ⛔ ③ 5分足 subset に分割汚染 732件(5%)。値幅ガードでは捕まらない

固定円損切りの初回測定で **最悪1件 −695,000円**(建値27万で−257%)が出て発覚。
§18.27 の再発。日足は分割を遡及調整するが **5分足(J-Quants)は未調整**なので、
日足で 1,000〜6,000円 の銘柄でも 5分足では 5万円の日がある(実測 `58010` = 56,480円)。

**値幅30%ガードでは捕まらない。建値の帯で切るのが実効的。**
`verify_barrier_order_5m.py --price-band 1000,6000`(既定)。

#### ★★ ④ 鏡像 = 前日下げ × ギャップダウンを **買う**

符号を全部反転しただけ。`analyze_gap_edge.py --side long`。

| TRAIN 2015-02〜2020-08 / 予算400万 / watch50 | 件数 | 円/件 | 月換算 | 投入/日 |
|---|---:|---:|---:|---:|
| N(ショート) | 9,670 | +345 | **+49,587** | 162万 |
| **鏡像(ロング)** | 7,979 | **+402** | **+47,839** | **131万** |

**総額はほぼ同じ、1件あたりは鏡像が上。** 私(Claude)は「ロングは無理」と
予想して外した。

##### ⛔ §18.18/§18.19 とは矛盾しない

* §18.18 の LDT は **トリガー付きロング** = 上に抜けたのを**追う**。
  鏡像は下に飛んだのを**拾う**。逆向きの操作
* §18.19 は「**日中ドリフトは存在しない**(intraday t=0.59)」。
  ドリフトが無いなら効果は過剰反応の巻き戻ししか有り得ず、**それは本質的に対称**。
  つまり **§18.19 がこの結果を予測していた**。誤っていたのは私の予想だけ

##### ★ 実運用は鏡像の方が楽

| | ショート | **ロング** |
|---|---|---|
| 貸株在庫 | **必要**(§18.51 の未測定リスク②) | **不要** |
| 逆日歩・空売り価格規制 | あり | **なし** |

#### ★★ ⑤ 両建て(`--side both`) — 1つの予算・1つの watch50 を共有

⛔ **200万ずつ分けてはいない。400万を1つの財布として、両側まぜて
|ギャップ|降順に埋める**(片側だけの日はその側が400万まで使う)。

| TRAIN | 件数 | 円/件 | 月換算 | 日次σ |
|---|---:|---:|---:|---:|
| ショート分 | 6,083 | +464 | +41,861 | 22,544 |
| ロング分 | 5,129 | +609 | +46,351 | 24,638 |
| **合計** | **11,212** | **+530** | **+88,212** | **32,352** |

* **相関 −0.062** / σ 47,183 → 32,352(**−31%**)
* **月平均÷σ: 片側 0.42 → 両建て 0.61(+45%)**

##### ⛔⛔ σ削減の内訳 — 「独立」と「逆相関」は別物

```
単純合算        47,183
ρ=0(独立)なら   33,392   ← 28pt はここ
実測(ρ=-0.062)  32,352   ← 逆相関で追加できたのは 3pt だけ
```

**「鏡像が N の負けを打ち消す」ではない。独立な2本目を足したから √ で
足されて下がっただけ。** ヘッジではなく分散。

構造的な理由: **強く上げた日はギャップダウン銘柄がそもそも少ない**ので、
鏡像は建てる玉が無く助けようがない。だから相関がゼロに張り付く。
**両方とも負ける日は普通に起きる。**

⚠ 逆に言えば、この効果は当てはめではなく**算数**なので期間が変わっても消えない。
CLAUDE.md に並ぶ「掃いたらヌル」(選別軸7回・発注順3回・寄り前19変数)とは質が違う。

##### 予算競合は 36% 押し出すが、利益は 9.5% しか減らない

| | 単独で回すと | 両建て |
|---|---:|---:|
| 件数 | 17,649 | **11,212(−36%)** |
| 月換算 | +97,426(単純和) | **+88,212(−9.5%)** |
| 円/件 ショート | +345 | **+464** |
| 円/件 ロング | +402 | **+609** |

**|ギャップ|降順で埋めているので、切られるのは弱いトレード。予算制約が選別として
働いている。** 稼働率48%なのに36%落ちるのは矛盾ではない(候補ゼロの日と
400万が埋まる日の平均だから)。

#### ⑥ 発注順と予算配分 (`--sweep-ops` に同梱)

**ランダム12シードの帯**を作ってから判定する(§18.24)。|z| ≥ t(11)=2.20 で帯の外。
前半/後半それぞれ同じ半期のランダム4本と比べて符号が揃うかも出す。

* 発注順: |ギャップ|降順(現行) / 売買代金降順 / 建値の安い順 / |ギャップ|昇順
* 予算配分(両建てのみ): 1つの財布(現行) / 側ごとに半額 / 合格数で按分 / 交互
  ⚠ **投入/日 を必ず見る。** 財布を分けると片側に候補が無い日に半分が遊ぶ

⛔ **帯の幅がゼロなら「予算が効いていないので比較できません」と出して判定を止める。**
押し出しが起きないと並べ替えても結果が変わらず z だけ発散する
(検算で `z = -33,085,121,677,298` が出た)。

#### ★★★ ⑦ TEST の合格条件 — **2026-08-27 に TEST を1度も見ずに決定**

`analyze_gap_edge.py --side both --confirm-both`。条件は `_BOTH_PASS` に
焼き込んであり実行時に変更できない。**5つ全部**を満たしたときだけ合格。

| # | 条件 |
|---|---|
| ① | 両建ての **月平均÷σ が N単独を上回る** |
| ② | 両建ての月次 **t ≥ 2.0** |
| ③ | **鏡像 単体**も 月平均>0 かつ t ≥ 1.0（片側が死んでいるのに合成で誤魔化すのを防ぐ）|
| ④ | 日次相関 **≤ +0.30**（独立性。強い正相関ならレバレッジと同じ）|
| ⑤ | TEST を半分に割って **両方の半期**で ① が成立 |

⛔ **TEST は1候補につき1回きり**(§18.54 で ATR%Q1 を消費した前例)。
今回使うのは **「両建て」1候補**。鏡像単体は別候補にしない。
**落ちたら『相場が特殊だった』と言わないこと。基準を緩めて再判定しないこと。**

#### ⛔ ⑧ 実装で踏んだ重大バグ (どれも「黙って誤った結論を出す」形)

| # | 内容 |
|---|---|
| 1 | **鏡像の複製が TRAIN にしか掛かっていなかった。** TEST 側はショートだけになり、ロング0件・両建て=ショート単独。そのまま実データで回していたら「不合格」と出て **TEST を1回無駄にしていた**。『鏡像単体 t=0.00』という有り得ない値で気づいた |
| 2 | 運用シミュが `--sweep-ops` の中に埋まっていて TEST から呼べなかった → `_make_ops_sim()` に切り出し、**TRAIN と TEST で同じ関数**を使う(§18.48 ⑧d の教訓) |
| 3 | `--max-gap-bp` が `_ops_sim` / `--sweep-grid` に届かず **1円も効いていなかった**(`_pool_of` を通らない経路)。ヘッダも件数も正しく出ていたのでたちが悪い |
| 4 | ATR倍 と % を取り違えて、悲観バイアスを **一桁 過大評価**(両方タッチ率 5% ↔ 49%)。CLAUDE.md に誤った訂正を書いて撤回した |
...
### 18.57 ★ 朝のペーパー記録 = `.\norder`(N / 鏡像 / J を1回の板読みで)

⛔ **1円も発注しない。** `n_paper.py` に発注メソッドは1つも存在せず、
`n_open_confirm.py` は `send_sell` / `send_moc` を物理削除し `--execute` を
起動時に `SystemExit` + `args.execute = False` で固定してある(AST で検査済み)。
`k_open_confirm.py --collect` も `sys.exit(0)` で終わり発注コードに到達しない。

#### 手順

```powershell
.\norder                                    # 08:40 までに起動。あとは自動
python n_paper.py --close --budget 400      # 引け後 15:40 以降。1コマンドで全部
```

`--close` は ①3方式の合格銘柄 → ②発注シーケンス → ③ペーパー損益 を順に出す。
`--sequence` 単体なら引け前でも①②だけ見られる(CSV を読むだけ / kabu 不要)。

| 方式 | 前夜 | 09:00 | 上位 |
|---|---|---|---|
| **N** | 前日 ≥ +1.753% | 始値 ≥ 前日終値 **+100bp** → 売り | 売買代金 50件 |
| **鏡像** | 前日 ≤ −1.753% | 始値 ≤ 前日終値 **−100bp** → 買い | 売買代金 50件 |
| J | 提案ファイル | 始値 ≥ +75bp | 売買代金 50件 |

#### ★ 50件の壁の正しい理解

**壁は実在する**(kabu の登録上限は同時50件。51件以上はリクエストごと 400 で失敗)。
ローテーションで超えられるが**時間と 429 のコストを払う**。

ただし **`OpeningPrice` は寄れば動かない**ので、150銘柄を数分かけて読んでも
**『どれが選ばれるか』は正しく出る**。速さが要るのは実発注のときだけ。
→ **3方式で50件を分け合うのではなく、それぞれ50件**(2026-08-26 ユーザー指示)。
読む対象はその和集合(最大150 = 3バッチ)に絞る。全候補を読むと429を招く(§18.48 ⑦)。

#### ★ 発注順はライブとバックテストで一段違う

| | 並べ方 |
|---|---|
| バックテスト | **その日を丸ごと** \|ギャップ\|降順 |
| **ライブ** | **①寄った時刻のグループ順 → ②グループ内で \|ギャップ\|降順** |

ライブは全部が寄るのを待たない(§18.38: 待つと1分で −15.8bp 逃げる)ので、
**遅く寄った大ギャップ銘柄が、先に寄った小ギャップに枠を取られる**。
`--sequence` はこの差を検出して理由つきで警告する。**実装の誤りではなく
待たない方針の代償。**

#### ⛔ 2026-08-27 の朝に見つけた不具合

| # | 内容 |
|---|---|
| 1 | **`k_signals` の `prev_close` 列が空**(fieldnames にはあるが値を書いていない)。n_paper の merge-j が `prev_close > 0` で絞っていたため **J 固有の43銘柄が1件も読まれず**、N/鏡像と重なった24件だけになっていた → 自前のスキャン結果から補う |
| 2 | `_read_all` が **毎周 全候補を読み直していた**。50件を超えるとバッチ切替のたびに登録し直しが走り、2026-08-18 に入れた最適化が無効化される → **寄った銘柄を次の周から外す** |
| 3 | 「候補 62件」と出ていたが実際は **107件**。読む対象に絞ったあとを数えていた |

★ 作法: **絞ったあとの数を『候補』と表示しない。** 絞る前の件数を別に持つこと。

---

### 18.58 ⛔⛔ マイナス月を抑える手段は **4系統すべて否定** (2026-08-27 確定)

**N の月別マイナス（プラス月 54/72 = 75%）を減らせるかを、方向を変えて4通り掃いた。
どれも帰無の中か、TEST で崩れた。マイナス月は N の構造そのものと確定。**

| 系統 | 何を試したか | 結果 |
|---|---|---|
| **① 両建て**(N + 鏡像) | 1つの予算・1つの watch50 を共有 | **TEST ⑤で不合格**(下記) |
| **② 使い分けルール** | 相場の状態で N/鏡像を切り替え。**288通り**(軸6 × 閾値4 × 組合せ12)+ 帰無200本 | **p=0.360** |
| **③ サイジング・件数上限** | 100株固定/金額均等/上限/ATR均等 × 件数上限 0,20,13,8,5 | **無効**(下記) |
| **④ 寄り前の外部指標** | 先物ギャップ/VIX/S&P500/為替/日経 × 5分位 = **40通り** + 帰無200本 | **p=0.280** |

**月平均÷σ 0.43〜0.48 なら負月は理論上 25〜32%。実測 18/72 = 25% は理論値より少ない。**
削るものが元から無い。

---

#### ★★ 18.53/18.54 を19年窓で追認 (TRAIN 2007-06〜2015-12)

`--days 7000 --split 2016-01-01` / 2,082営業日 / 964,476銘柄日。**リーマン(2008)と震災(2011)を含む。**

| 窓 | 月平均 | 月次σ | 月平均÷σ | t | プラス月 |
|---|---:|---:|---:|---:|---|
| **2007-2015** | +42,539 | 98,364 | **0.43** | **+4.39** | 62/103 |
| 2015-2020 | +49,587 | — | 0.42 | — | — |
| 2020-2026 | +89,364 | 150,395 | 0.48 | +5.04 | 54/72 |

**3窓とも 0.42〜0.48。** これまでで最も長い確認。

2008年の月別: 01 −122,954 / 08 −91,976 / 10 −69,574 / 11 −47,306 / 12 −126,671。
**破綻はしないが儲かってもいない。** 2011-03(震災)は **+111,492 でプラス**(同日決済は暴落に強い)。

発注順も3窓とも同じ結論: |ギャップ|降順 z=+2.00(帯の縁) / **|ギャップ|昇順 z=−3.70(帯の外)**
= **ギャップの大きさには本物の情報がある**が、降順の優位そのものは弱い。

⚠ 銘柄リストは**今日 上場している1,540銘柄**。19年窓ほど生存バイアスが効く。
⚠ 1件あたり **+315円**(直近13ヶ月の +802円 の4割)。**水準は期間で大きく振れる**(§18.20)。

---

#### ① 両建て = 不採用。**σ削減はヘッジではなく分散だった**

TRAIN: 月 +88,074 / 日次σ 32,363 / 日次相関 **−0.062** / 月平均÷σ 0.64

```
単純合算        47,183
ρ=0(独立)なら   33,392   ← σ削減31%のうち **28pt はここ**
実測(ρ=-0.062)  32,352   ← 逆相関で追加できたのは **3pt だけ**
```

**「鏡像が N の負けを打ち消す」ではない。独立な2本目を足したから √ で足されただけ。**
構造的な理由: **強く上げた日はギャップダウン銘柄がそもそも少ない**ので、鏡像は建てる玉が無く
助けようがない。だから相関がゼロに張り付く。**両方とも負ける日は普通に起きる**(120日 / 26%)。

N が負けた466日の実測: N平均 −13,474 / 同日のロング +2,255 = **穴埋め率 17%**。
**最悪5日のうち4日は片側が0件。**

TEST(72ヶ月)で ①②③④ は通ったが **⑤(両半期とも N単独を上回る)で不合格**
(後半 0.48 vs N単独 0.49)。理由は **月次相関 +0.266**(日次は −0.055)。
**月次σ を決めるのは月次の相関。日次相関がゼロでも月次では効かない。**

⛔ TEST は1候補につき1回。**基準を緩めて再判定しないこと**(宣言済み)。

---

#### ③ サイジングは **100株の床**で動けない

**100株が最小単位なので、サイジングは「大きくする」方向にしか動けない。**
実測で **ATR均等・上限30万・上限40万 は 100株固定と小数点2桁まで同一**だった。

件数上限を絞ると **負月 18/72 → 24/72 に増え、最悪月も悪化**。分散が減るだけ。

---

#### ⛔⛔ ④ は **無効**。乱数で測っていた (2026-08-27 夜に発覚)

`--sweep-market` を作ったとき、コンテナが yfinance に出られないので
**乱数を返すダミーを書き、それを本物の `preopen_market.py` に上書きして
コミットしていた**(`8fdd34a`)。

証拠は出力そのものにあった:
```
使える指標 **5本**: fut_gap, n225_ret, sp500_ret, usdjpy_ret, vix
```
ダミーが返すのは正確にこの5つ。本物は **19変数**(NASDAQ / SOX / 米10年債 /
DAX / ユーロSTOXX50 / KOSPI / ドル指数 / WTI / 金 / S&P500先物 など)。
`5変数 × 4閾値 × 2方向 = 40通り` も一致する。

**→ p=0.280 は乱数を掃いた結果で、何の意味もない。** 本物を復元済み。
今後ダミーは `_dummy_for_container/` に隔離する。

★ **作法: 検算用のダミーを、本物と同じ名前でリポジトリ直下に置かない。**
  出力の「使える指標 N本」のような**件数の表示は、取り違えを検出する装置**に
  なる。5本と19本の差は見れば分かったのに、見ていなかった。

⚠ §18.34b(lss / 19変数 / 113営業日 / 候補ゼロ)は**ダミーを作る前**の測定なので
  無傷。ただし「N でも同じ結論」は**言えない状態に戻った**。

#### (以下は無効な測定の記録) ④ 寄り前の外部指標 (40通り / TRAIN 1,237日 / TEST 1,374日)

| | 実測最良 | 帰無中央 | 帰無95%点 | 基準 |
|---|---:|---:|---:|---:|
| 月平均÷σ | **0.531** | 0.507 | **0.586** | 0.443 |

**p=0.280。** TRAIN 上位2つは TEST で崩れる:

| ルール | TRAIN ÷σ | TEST ÷σ | TEST 負月 |
|---|---:|---:|---:|
| fut_gap 80% 高い日を捨てる | **0.53**(1位) | 0.42 | 21 |
| usdjpy_ret 40% 低い日を捨てる | 0.52 | 0.35 | 23 |
| **基準(全部建てる)** | 0.44 | **0.48** | **18** |

**帰無較正と TRAIN→TEST の両方が同じ答えを出した。**

§18.34b(lss / **113営業日** / 19変数 / 候補ゼロ)を **25倍のデータ(2,843営業日)で追認**。
「相場の状態でその日を降りる」は N でも作れない。

---

#### ★ 損切りの現在地 — 「否定」と「保険料」を混同しないこと

| 種類 | 判定 |
|---|---|
| **ATR倍の損切り/利確**(sm/tm) | ⛔ **否定**。滑り0.5%で **24セル全部マイナス**(§18.55) |
| **固定円の損切り**(1万円など) | ⚠ **否定ではない**。利益を減らす代わりにテールを買う(§18.56 ②) |

固定1万円(5分足 subset / 2024-07〜):

| | 平均 | 最悪の1件 | 最悪の日(予算400万換算) |
|---|---:|---:|---:|
| なし(現行) | +486円/件 | **−76,000円** | **−201,977円** |
| −10,000円 | +425円/件(**−61**) | **−10,000円** | −137,490円(**32%改善のみ**) |

**保険料は約 −61円/件 ≒ 月 −9,300円**(153件/月)。月平均 +42,000〜89,000 の **10〜20%**。
買えるのは「1件の最悪が7分の1」だけで、**最悪の日は32%しか改善しない**
(固定1万円は建値27万の3.7% = 日中の値幅として普通に起きるので、悪い日は全ポジションが同時に止まる)。

**→ 成績の最適化ではなくリスク許容度の宣言**(§18.38 #3b と同じ性質)。
⚠ 測定期間が TEST 寄り(2024-07〜)なので、**この数字で設定を選び直さないこと**。

---

#### ★ 残っている未検証 = **執行コストだけ**

バックテストでやることは尽きた。`.\norder` の日次記録を貯めて実約定を測る段階(§18.55)。

| 項目 | 状態 |
|---|---|
| **執行コスト**(実約定 vs 始値) | ⛔ **唯一の未知数**。`.\norder` を貯める |
| watch50 の壁 | 外せれば +63% だが **kabu では不可**(§18.44/§18.45)。実装課題であって検証課題ではない |
| 予算 | 400万 → 1,200万 で月 +7,449 だけ(頭打ち)。レバレッジ = リスク許容度 |
| 1件あたりの水準 | 期間で 315〜802円 と振れる。**監視するだけ** |

---

#### ★★ 利確は **完全に閉じた** (2026-08-27 / 損切りなし × 利確8段)

`--sweep-barrier --sm-list 0 --tm-list 0,0.3,0.5,0.75,1.0,1.5,2.0,3.0`
TRAIN 2015-02〜2020-08 / 167,788銘柄日 / 1,346営業日 / 現行(バリアなし) = **+175円/件**

| tm | 0.3 | 0.5 | 0.75 | 1.0 | 1.5 | 2.0 | 3.0 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 現行との差(円/件) | **−43** | −26 | −24 | −22 | −10 | −4 | **+2** |

**きれいに単調で、利確を遠ざけるほどゼロ(=利確なし)に近づく。**
tm3.0 の +2円/件は **0.09bp**(執行コスト3.3bp の3%)で、3ATR ≒ 9%下なのでほぼ到達しない
= **「何もしていない」の別名**。tm→∞ で現行に収束する形そのもの。

★ **利確は指値なので滑らない**(損切りは成行で滑る)。つまり利確だけは滑りモデルの議論を
  一切通らずに判定できる。**最も有利な条件で測ってこの結果**なので、これで確定。

#### ★ バリアは全滅。すべて同じことを言っている

| | 結果 |
|---|---|
| 利確(8段) | **単調に害**。緩めるほどゼロに近づく |
| 損切り ATR倍(5段 × tm4段) | 滑り0.5%で24セル全滅。分岐点 **2.6ティック**(§18.55) |
| 損切り 固定円 | 保険料(−61円/件でテールを買う。§18.56 ②) |
| **delay** | **対象が存在しない**(損切りを置かないので遅らせるものが無い) |
| 決済時刻(5分〜引け) | 5分 +10.3 → 11:05 +10.5 → **引け +17.8bp**。早いほど悪い |

**→ N は寄りで建てて引けまで何もしないのが最良。**
エッジが「一日かけて下げ続けること」なので、**価格でも時間でも介入すれば削るだけ**。

☐ 未実施は **トレーリングストップ**1つだけ。ただし ①日足では測れない(順序不明 / §18.6)
  ②5分足は2年分しかない ③**損切り系なので滑りに殺される構造は同じ**。
  上の5つが全部同じ方向を指している以上、これだけ違う答えを出す理由は無い。

**★ バックテストでできることは尽きた。残る未知数は執行コストだけ。**

---

### 18.59 ⛔ 選別軸の再探索と「件数を増やす」方向 — **どちらも不採用** (2026-08-27 夜)

**§18.58 で「バックテストは尽きた」と書いたが、正確には『私(Claude)が TEST を節約するため
早く閉じた扉』が3つ残っていた。** 全部 TRAIN で開け直した結果、採用に至ったものはゼロ。

#### ⛔ ① これまでの `--explore` は ret1 を掛けずに回していた

`_pool_of` に ret1 フィルタが無く、探索はいつも「ギャップ≥100bp の全部」の中で行われていた。
つまり **見つかったのは『ret1 が最強』までで、その先(残差)を一度も見ていなかった**。
`--min-ret1` を追加して開け直した(TRAIN 33,554銘柄日 = ちょうど全体の1/5)。

| 候補 | 判定 |
|---|---|
| ret1 Q5 | ⛔ **条件そのもの**。ret1≥1.753 の中でさらに上位20% = 再発見 |
| gap_bp Q5 | ⛔ 帰無95%を **+0.1bp** しか超えない(+23.9 vs +23.8)。`--sweep-grid` で掃き済み |
| liq Q1 | ⛔ **非単調**(26.3 / 14.6 / 11.2 / **8.4** / 16.3 の U字)+ 執行と衝突 |
| **up_streak Q3** | ✅ 3群で単調(+14.3 → +18.5 → +23.0) / t=+2.66 / 帰無 +19.9 超え |

軸を3つ追加(`ret2` / `ret3` / `gap_hi_bp` = 前日**高値**からのギャップ)したが、
**過熱の窓は1日(ret1)が最良**で 2日・3日は帰無の中。ギャップの定義変更も帰無の中。

#### ★ ② 「絞る」ではなく「増やす」— N は件数不足だった

`up_streak Q3` をフィルタに使うと **件数が91%減る**(26.3 → 2.4件/日)。
N は **稼働率40%**(投入162万/日 ÷ 予算400万)で資金が余っているので、**絞る方向は必ず負ける**。
そこで `--sweep-relax` を作り、**閾値を下げて足せるか**を掃いた:

```
建てる = (ret1≥1.753 かつ gap≥100)  ∪  (AXIS≥閾値 かつ gap≥緩めた閾値)
```

⛔ **追加分は2種類あってコストが違う。混ぜて判定してはいけない**:

| | 内容 | コスト |
|---|---|---|
| **(A)** | ret1≥1.753 だが gap が届かず落ちていた | **前夜の候補プールに既にいる = タダ** |
| **(B)** | ret1<1.753 だが AXIS が強い | **プールが増える → 流動性順で現行を押し出す** |

**(B) は全升が現行 +15.5bp を下回る**(最良 +11.4bp)= 入れ替わるだけで悪化。**却下**。

#### ⛔ ③ そして up_streak は効いていなかった(コントロール行で判明)

`--relax-axis-list` に **0 =「条件なし」のコントロール行**を足したら:

| (A) bp/件 | gap25 | gap50 | gap60 | gap75 | gap90 |
|---|---:|---:|---:|---:|---:|
| **条件なし** | +6.8 | +9.7 | +10.5 | +11.1 | **+15.8** |
| up_streak≥2 | +7.4 | +11.1 | +13.0 | +13.0 | +17.8 |

**≥2 の上乗せは gap90 で +2.0bp、代わりに件数が半減**(2.9 → 1.4件/日)。gap25 では +0.6bp。
**効いていたのは軸ではなく『gap 閾値を下げたこと』だった。**

★ **作法: 分位や閾値の表には必ず『条件なし』の行を置く。**
  無いと「軸が効いた」のか「別の条件を緩めただけ」なのか判別できない。
  ret1≥1.753 は前日が上げ = up_streak≥1 が常に成立するので、既定の 2..6 には
  **コントロールが存在しなかった**。

#### ★★ ④ 本命だった「gap 閾値を下げる」も **レバレッジ**だった

`--sweep-grid` は bp/件 だけを見て「現行のまま」と結論していたが、**ops sim(watch50+予算)を
一度も通していなかった**。通したら月換算は **単調に増えた**:

| gap | 件/日 | 円/件 | 月平均 | 月次σ | **÷σ** | 差/月 | 対応t | **執行後 差/月** |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 25 | 12.5 | +261 | +66,146 | 139,189 | 0.48 | +16,224 | +1.81 | **+8,180** |
| 50 | 10.8 | +294 | +64,329 | 130,209 | 0.49 | +14,407 | +1.98 | **+8,895** |
| 75 | 9.0 | +305 | +55,277 | 121,587 | 0.45 | +5,355 | +1.10 | +3,993 |
| 90 | 7.9 | +352 | +55,916 | 113,952 | 0.49 | +5,994 | **+2.25** | **+4,802** |
| **100 ★本番** | 7.1 | +346 | +49,922 | 112,601 | **0.44** | — | — | — |
| 125 | 5.6 | +402 | +45,425 | 95,135 | 0.48 | −4,497 | −0.93 | — |
| 150 | 4.4 | +402 | +35,563 | 90,546 | 0.39 | −14,359 | −2.14 | — |

**÷σ が 0.39〜0.49 でフラット。** gap100 → gap25 は 月平均 ×1.33 / σ ×1.24 = **ほぼ比例
= 予算を上げるのと同じ**。

そして **執行コスト(3.3bp/件 ≒ 74.5円/件)は件数に比例する**ので、増やす方向には必ず不利。
引くと **唯一 t=2 を超えていた gap90 も +2.25 → ≈+1.8 に落ちる**。
しかも gap75 が t=+1.10 と**非単調**なので gap90 の +2.25 自体がノイズ。

→ **不採用。gap 閾値は 100bp のまま。**

★ **作法: 件数が変わる比較では、必ず執行コストを引いてから判定する。**
  引く前の差だけを見ると、**件数を増やす方向が常に良く見える**。列にした。

#### ⚠ ⑤ 発注順で初めて「帯の外」が出たが、期間依存

| 発注順 | z | 前半 / 後半 |
|---|---:|---|
| \|ギャップ\|降順 ★現行 | +1.62 | +360,391 / −34,807 — |
| **売買代金 降順** | **+3.12 帯の外** | **+938,358 / −246,512 —** |
| 建値 安い順 | −2.48 帯の外 | −197,452 / −480,259 ✓ |

§18.21/§18.24/§18.31 の3回では一度も出なかった水準だが、**前半と後半で符号が逆**なので
§18.36 判定ルール2 に照らして不採用。ツールが正しく落とした。

#### 追加した機能(今後の再検証用)

| | |
|---|---|
| `--min-ret1` | ret1 を固定した残差を探索する |
| `ret2` / `ret3` / `gap_hi_bp` | 過熱の窓・ギャップの定義を掃く軸 |
| `--sweep-relax` | 絞るのではなく**増やす**方向。(A)/(B) 分解 + コントロール行 |
| `--ops-gap-list` / `--ops-gap-ref` | ギャップ閾値を **ops sim(watch50+予算)** に通す |
| ギャップ表の `÷σ` / `対応t` / `執行後 差/月` | 総額だけで選ぶ罠を防ぐ |
| 引数の事前検証 | 10分走ってから `ValueError` で落ちるのを防ぐ(PowerShell の `]` 混入) |

#### ★ 結論(§18.58 から変わらず)

**N は現行設定のまま。バックテストでできることは尽きた。残る未知数は執行コストだけ。**
毎日 `.\norder` → 引け後 `python n_paper.py --close --budget 400` を貯める。

---

### 18.60 ★★★ 大負けの日は「相場」だが**寄り前には読めない** (2026-08-28 確定)

**ユーザーの直感「明らかにそういう相場だから事前に分かるはず」を、順番を追って測った。
関係は実在した。ただし事後にしか分からない。**

`analyze_gap_edge.py --days 4200 --min-gap-bp 100 --split 2020-09-01 --tail-diag --tail-nulls 2000`
TRAIN 2015-02〜2020-08 / 1,237営業日 / 日次パネル(1日1行)

#### ★ 測る順番を変えたのが要点

⛔ §18.34b も §18.58④ も「19変数を掃いて候補ゼロ」で終わり、**なぜ出ないのかが
分からなかった**。今回は先に **予測可能性の上限**を測った:

```
① 事後: N の日次損益は「同日の相場」でどこまで説明できるか   ← 天井
②      寄り前変数は その「同日の相場」をどこまで読めるか
③ 事前: 寄り前変数で N の損益を直接読めるか
①×② が ③ の上限。①が小さければ ③ で何が出ても多重検定の産物と判定できる。
```
...
| 変数 | β(円/1%) | R² | t |
|---|---:|---:|---:|
| 日経 当日%(前日終値→終値) | −8,078 | 0.124 | −13.20 |
| **日経 当日の日中%(始値→終値) ★N の保有区間** | **−14,945** | **0.235** | **−19.49** |
| 日経 当日の寄りギャップ% | +894 | 0.000 | +0.77 |

**大負けの日(124日)の81%が「日経が日中プラスの日」**(全日49%)。
その日の日経日中% は 平均 **+0.71%**(全日 +0.01%)。

⚠ 試運転(200銘柄)では R² 0.091 だった。**母集団を絞ると過小に出る**ので、
  この種の測定は必ず全銘柄で行うこと。

#### ② しかし寄り前には読めない — R² 0.045

寄り前変数 → 同日の日経日中% の最良は **先物-現物ギャップ の R² 0.045**。

```
①×② = 0.235 × 0.045 = **0.0105**  ← ③で出せる R² の天井
```
...
| | |
|---|---:|
| TRAIN in-sample R² | 0.0593 |
| TRAIN「予測が下位20%の日は建てない」 | +3,362,998 → +4,042,350 (**+679,352**) |
| **TEST out-of-sample R²** | **−0.0644** |
| **TEST 同じルール** | +4,727,162 → +2,249,526 (**−2,477,636**) / 降りた日 480日 |

**OOS R² がマイナス = 平均を使うより悪い。** 予測力が無いどころか逆に作用する。

#### ③-a / ③-b も否定

41本(外部18 / 絶対値17 / **N自身の履歴6**)× 5分位。両方の帰無を超えたのは **4本**
(帰無の期待2.1本)だが、**試運転(200銘柄)の5本とほとんど重ならない**:

| | 両方の帰無を超えた変数 |
|---|---|
| 試運転(200銘柄) | VIX水準 / 日経前日% / \|日経前日%\| / \|KOSPI\| / ドル指数 |
| 本番(1,540銘柄) | NASDAQ / S&P500 / VIX変化% / \|VIX変化%\| |

**母集団を変えただけで顔ぶれが入れ替わる = ノイズ。**
2変数の交互作用(820ペア / 3,177セル)も帰無の中。

★ **N 自身の履歴6本(前日損益/5日/20日/20日σ/勝日率/連続マイナス日数)も全滅。**
  外部データが要らず確実に寄り前に確定する、いちばん筋の良い候補だったが効かない。

#### ★ 結論

| 問い | 答え |
|---|---|
| 大負け日は相場か | **はい(81%が日経プラス日 / R² 0.235)** |
| 寄り前に読めるか | ⛔ **いいえ**(OOS R² −0.064 / TEST −247万) |

**関係は実在するが、事後にしか分からない。予測で降りる案はここで閉じる。**

---

### 18.61 ⛔ 同日ヘッジ = **棄却** (2026-08-28 実測)

**§18.60 ① の副産物。N の市場感応度は lss の4倍ある。**

| | R² | §18.30 の判定 |
|---|---:|---|
| lss | 0.063 | σ削減3.2% → **ヘッジは棄却** |
| **N** | **0.235** | **前提が違う。測り直す価値がある** |

**これは予測ではない。** 寄りで日経先物を買い、引けで売るだけ。**当日の相場を当てる
...
σ削減 = 1 − √(1−0.235) = **12.5%**
月平均÷σ  0.43 → 0.49 (+14%)
必要枚数 = 14,945円/1% ÷ (日経マイクロ1枚 ≒ 4,200円/1%) ≒ **3.5枚**
```

⚠ 未確認: ①TEST での β と R² ②先物の手数料・スプレッド ③kabu の先物対応
④β が期間で安定するか(§18.30 では lss の月別βが**2ヶ月符号が逆**だった)

⛔ **判定は必ず TRAIN/TEST を分けて。** そして σ が本当に下がったかを
月平均÷σ で見ること(総額は変わらないのが正常)。

---

#### ★ 実測 (2026-08-28) — **σ削減は本物だが、コストが2.4倍足りない**

`analyze_gap_edge.py --days 4200 --min-gap-bp 100 --split 2020-09-01 --hedge`

##### ① σ削減 6.1% は TRAIN/TEST で完全に一致 = **本物**

| | β | 比率0 → 1.0 のσ | 削減 |
|---|---:|---|---:|
| TRAIN (2015-2020 / 67ヶ月) | −14,945円/1% (R² 0.235) | 112,619 → 105,785 | **6.1%** |
| TEST (2020-2026 / 72ヶ月) | −15,171円/1% | 139,450 → 131,012 | **6.1%** |

**β も σ削減率も一致。** ここは疑いようがない。

##### ⛔ ② TEST の「+27%改善」は σ ではなく **日経ロングの儲け**
...
TEST は **月平均が +66,746 → +79,554 と19%増えている**。σ を削るだけなら平均は動かない。
逆算すると **TEST 期間の日経 日中% 平均 = +0.053%/日**(TRAIN は **+0.006%** ≒ ゼロ)。

**TEST 期間は日経が日中に上げ続けていたので、先物を買ったこと自体が儲かっただけ。**
0.48→0.61 のうち σ削減の寄与は 0.48→0.51 で **4分の1**、残りは日経ロング。
**再現を期待できるのは TRAIN の +4.5% のほう。**

##### ⛔⛔ ③ 現実のコストで TRAIN は単調に悪化

| コスト4.4bp / TRAIN | 月平均 | 月次σ | ÷σ |
|---:|---:|---:|---:|
| **0.00 ★現行** | +49,948 | 112,619 | **0.44** |
| 0.50 | +44,656 | 107,721 | 0.41 |
| 1.00 | +39,363 | 105,556 | 0.37 |
| 1.25 | +36,716 | 105,560 | 0.35 |

**最良比率: TRAIN = 0.00(ヘッジしない) / TEST = 0.75〜1.25。完全に食い違う**
= §18.36 判定ルール2 で不合格。

##### 損益分岐 = **1.83bp**。現実は約4.4bp

2つの実行(1.0bp / 4.4bp)から正確に逆算:
```
コスト 1bp あたり  月 2,759円
÷σ 0.44 を維持できる上限 = **1.83bp**
日経マイクロ先物の現実     = 約 4.4bp
  (建玉 149万 = マイクロ3.6枚 / 呼値5円×指数10 = 50円/枚 の往復 + 手数料)
```

**2.4倍足りない。** しかもミニ先物(1枚420万)では2.8倍のオーバーヘッジになるので、
**より安い商品に逃げる道もない**。

#### ★ 結論

**σ を 6% 削るために利益を 21% 払うことになる。棄却。**

§18.30 で lss のヘッジを棄却したのと**理由は違うが結論は同じ**
(あちらは R²=0.063 と効果が小さすぎ / こちらは効果に対してコストが大きすぎ)。

⚠ ただし **σ削減 6.1% と β≈−15,000円/1% は実在する**ので、
将来 先物のコストが下がる / 建玉が大きくなって bp が下がる ようなら再検討の余地はある
(建玉が3倍になれば同じ円コストでも bp は3分の1)。**予算を大きく増やしたときは測り直すこと。**

---

### 18.62 ⛔ ポートフォリオ損切り = 棄却 (2026-08-28)。**悪い日は戻す**

**ユーザー発案。§18.60 で見つけた構造(大負け日の81%は相場)を正しく狙った案で、
個別損切りが失敗した理由も回避できていた。それでも負けた。理由が新しい。**

`analyze_gap_edge --dump-picks picks.csv` → `analyze_portfolio_stop.py`
469営業日(5分足のある 2024-07〜2026-08) / 自己検算 400円/日

#### なぜ試す価値があったか

これまで否定したのは **個別の損切り**:
- ATR倍 … 滑り0.5%で24セル全滅。分岐点 2.6ティック (§18.55)
- 固定円 −10,000円/件 … 保険料。平均 −61円/件 (§18.56 ②)

個別が負けるのは **銘柄固有のノイズで刈られる**から。
**合計で判定すればノイズは打ち消し合い**(12銘柄なら √12)、相場全体の動きだけが残る。
そして §18.60 で「大負け日の81%は日経が日中プラス(R² 0.235)」と測れていた。
ヘッジ(§18.61)と違い **発火した日しかコストを払わない**のも構造的な利点。

#### ① TRAIN では全水準が悪化

| TRAIN 234日 | 合計 | 月平均÷σ | 発火 |
|---|---:|---:|---:|
| **損切りなし ★** | +877,600 | **0.56** | — |
| −20,000円 | +646,085 | 0.47 | 65 |
| −30,000円 | +689,571 | 0.44 | 34 |
| −50,000円 | +705,709 | 0.39 | 15 |

(−80,000 の 0.57 は **発火1回**なので実質「損切りなし」と同じ)

#### ② TEST では逆に改善 = **正反対**

| TEST 235日 | 合計 | 月平均÷σ | 発火 |
|---|---:|---:|---:|
| 損切りなし ★ | +1,845,611 | 0.75 | — |
| **−30,000円** | +1,846,534 | **0.91** | 63 |
| −50,000円 | +1,593,429 | 0.90 | 41 |

**TRAIN で最悪の −30,000 が TEST では最良。** §18.36 判定ルール2 で不合格。

#### ★★ ③ 決定的 — 降りたほうが損

| 水準 | 発火 | 降りた損益 | 引けまで | 差 | 正解率 |
|---|---:|---:|---:|---:|---:|
| −20,000円 | 154 | −4,019,701 | −3,157,100 | **−862,601** | 45% |
| −30,000円 | 97 | −3,449,856 | −3,262,750 | **−187,106** | 57% |
| −50,000円 | 56 | −3,125,472 | −2,701,400 | **−424,072** | 46% |
| −80,000円 | 14 | −1,201,996 | −1,125,600 | **−76,396** | 50% |

**全水準で差がマイナス。正解率 45〜57% = コイン投げ。**
−30,000 の 57% が最高だが差は −187,106 =「勝つ回数は多いが負けるとき大きく負ける」。

#### ★ 答え

> **悪い日は最後まで悪いのか、それとも戻すのか?** → **戻す。**

§18.55 の「早く降りるほど悪い」(5分 +10.3bp → 引け +17.8bp)が
**ポートフォリオ単位でも成立**した。N のエッジは『一日かけて下げ続けること』なので、
**含み損が膨らんだ日ほどその後に戻しており**、途中で降りると回復ぶんを捨てる。

⛔ これで **損切りは個別・合計・水準・トレーリングの全形式が否定**された。
   N は寄りで建てて引けまで何もしないのが最良(§18.55 の結論を強化)。

#### ⛔ このツールで踏んだバグ2件(どちらも出力の異常で気づけた)

| # | 内容 |
|---|---|
| 1 | **株式分割の汚染を通していた**(§18.27)。5分足は分割未調整・日足は遡及調整なので、分割銘柄では5分足が日足の2〜15倍になり、ショートの含み損が爆発。予算400万に対し **−1億9,500万** という数字が出た。`picks.csv` の `d1_close` と突き合わせて弾く |
| 2 | 汚染銘柄が1つあると **その日を丸ごと捨てて**いたので 482日 → 193日 と6割が消えた。**その銘柄だけ外す**のが正しい(基準にも同じ銘柄を外すので比較は公平) |
| 3 | `✅` を `ratio > base * 1.10` で判定し、**base がマイナスのとき不等号が反転**。発火0回の行に ✅ が付いた。**差**で比べる形に修正 |

★ **作法: ツール自身に『合っているはずの等式』を持たせる。**
  ここでは「**経路の最終値の合計 = その日の損益の合計**」(最後の5分足の終値 ≒ 日足の終値)。
  平均誤差を毎回印字するようにしたので、同種の食い違いは必ず検出できる
  (汚染時 数万円 → 修正後 **400円/日**)。

---

### 18.63 ★★ ストップ高で決済できない日 = **月0.7件・全部ショート**。鏡像はゼロ (2026-08-28)

N は **損切りも利確も置かない**(§18.55)ので、決済が成立しない唯一のケースが
**引けがストップ値に張り付いたとき**。決済できないと持ち越しになり、§18.46 の事故
(8建玉 / −36,800円)と同じ形になる。`analyze_limit_up.py --picks picks.csv` で数えた。

⛔ **「寄りがストップ高」は危険ではない**(板寄せで約定済み。31件・+6,984円/件)。
   危険なのは **建てた後に飛んで引けまで張り付く**ケースだけ。

#### 実測 (20,848件 / 2,611営業日 / 11.6年)

| | 件数 | 頻度 | バックテスト上の損益 |
|---|---:|---:|---:|
| **N(ショート)** | **91** | 0.436% = **月0.7件** | −1,560,700円 (−17,151円/件) |
| ~~**鏡像(ロング)**~~ | ~~**0**~~ | ~~**0.000%**~~ | ⛔ **下記のとおり誤り** |
| TRAIN → TEST | 21 → 70 | 0.0170 → **0.0509件/日 (3倍)** | 直近ほど増えている |

#### ⛔⛔ 訂正 (2026-09-11): 「鏡像ゼロ」は **バグの産物**だった

`analyze_gap_edge.py --side long` が **明細を全部ショートと記録していた**
(`side` 列が `--side both` のときしか作られず、`getattr(_r,"side",1)` が +1 を
返していた)。損益は `_SIDE` で反転するので**金額は合い、方向だけが黙って逆**になる。
`analyze_limit_up.py` は side>0 でストップ高・side<0 でストップ安を見るので、
**ロングをショート表記で渡すと必ず該当0件**になる。`cfaf37f` で修正済み。

再測定 (5.5年 / 10,780件 / 1,313営業日):

| | 件数 | 頻度 | バックテスト上の損益 |
|---|---:|---:|---:|
| N(ショート) | 33 | 0.54% | −21,661円/件 |
| **鏡像(ロング)** | **16** | **0.34%** | **−11,960円/件** |

**ゼロではない。ただし頻度は4割少なく、1件あたりの損失も半分。**
向き(上への暴走のほうが起きやすい)は正しかったが、程度が違う。

⚠ TEST 0.0638件/日 vs TRAIN 0.0119件/日 で **直近が5倍**。引けがストップ値を
**超えている**2件は分割汚染(§18.27)の疑いがあるので、数え直すときは除くこと。

★ 教訓: **片側だけを渡した出力で「該当0件」が出たら、まず向きの記録を疑う。**
「構造的にゼロ」という説明がもっともらしく書けてしまったので、3週間気づかなかった。

#### ⛔ ギャップ上限では防げない

上位10件の建値ギャップは **+135bp(8278) 〜 +630bp(3778)** に散らばる。8278 は
何の変哲もない N 候補。§18.55 で `--max-gap-bp 150` を**損益**理由で棄却したが、
**テールリスクの理由でも切れない**。予防手段は無い。手当は「引けの成行が約定
しなかったら翌朝すぐ閉じる」だけ。
...
**99%の取引はストップまで5%以上の余裕がある。** 制限値幅は株価比 14〜30% なので、
**分割の遡及調整(§18.27)で帯を1つ間違えても結論は動かない**(近づくのは裾の0.5%だけ)。
引けがストップ値を**超えている** 9件 は汚染で、②に入るべきものが漏れた側 = 91件は過小。

#### コスト

⛔ **−1,560,700円は すでにバックテストの損益に入っている**(引けで決済できた前提)。
上乗せされるのは **強制決済手数料 2,200円/件 + 引け→翌朝の逆行**だけ。
後者は §18.46 の −2,400円/件(普通の銘柄8件)を仮置きしていたが、ストップ高で
買い気配のまま引けた銘柄に当てはまる保証がないので、`picks` に **D+2 の始値**を
足して実測に置き換えられるようにした(`--dump-picks` を作り直すと ③ が実測になる)。
仮置きのままなら 4,600円/件 × 91件 = **月 3,206円**(TRAIN月平均の6.5%)。

⚠ 終値がストップ値で**ついている** = 板寄せは成立しているので、比例配分で買い戻せた
可能性がある。**91件は決済できなかった恐れのある上限**で、実数はこれより少ない。

#### ✅ ガードの不一致は無かった(私(Claude)の前回の指摘は誤り)

| | ライブ(`n_paper`) | バックテスト | |
|---|---|---|---|
| N / 鏡像 | 上限なし | `_newgap_sim` 上限なし | ✅ |
| J | +300bp 超は見送り | `eh_trades(gap_guard=0.03)` | ✅ |

`n_paper.py:828` に「**方式ごとに違う。** J は +300bp 超を見送るが N は上限を
持たない。ここを揃えるとどちらかがバックテストと食い違う」と既にコメントがあった。

★ **作法: 「不一致がある」と言う前に、両側のコードを開いて確認する。**

---

### 18.64 ⛔ 業種は選別軸にならない (2026-09-06 確定)。**この経路は閉じる**

**33業種 × 4通りの使い方を掃いて、1つも通らなかった。** 選別軸の否定は9回目
(§18.12 BTスコア / §18.13 15軸78検定 / §18.24 7軸 / §18.31 流動性 / §18.38 建値 /
§18.48⑧g 母集団を帯にした確認 / §18.48⑪ 銘柄・戦略 / §18.52 シグナルそのもの)。

```
python analyze_gap_edge.py --days 4200 --split 2020-09-01 --min-ret1 1.753 \
  --min-gap-bp 100 --min-price 1000 --max-price 6000 --sector-scan --seeds 1000
```
TRAIN 2015-03〜2020-08 (1,361営業日 / 33,477銘柄日) / TEST 2020-09〜2026-09

#### ① 半導体28銘柄を除外 = 根拠なし

| | 半導体 | その他 | 差 | 帰無95% | p |
|---|---:|---:|---:|---:|---:|
| **TRAIN** | **+3.2bp**(日t +0.34) | +16.0 | +12.7 | +17.2 | 0.135 |
| TEST | **+23.3bp** | +19.3 | −4.0 | +10.9 | 0.813 |

**TRAIN でプラス**なので除外は純損。TEST では半導体のほうが**良い**。
「大負け日に半導体が目立つ」のは 5分足の2年窓がほぼ半導体・AI相場だったことの反映。

#### ② 33業種の総当たり = 帰無帯の中、TEST で反転

| | 件数 | bp/件 | 日t |
|---|---:|---:|---:|
| **ガラス・土石製品**(TRAIN 最悪) | 535 | **−13.1** | −0.68 |
| 帰無(同日シャッフルで最悪を選ぶ) 中央 / 5%点 | — | −5.7 / **−18.0** | — |
| **同じ業種を TEST で固定** | 554 | **+27.5** | **+2.26** |

**帯の中(−13.1 > −18.0)で、TEST では全体(+19.3)より良い。** TRAIN 単体でも日t −0.68。

⚠ 別セッションの独立実行は 5%点 −18.2bp(こちらは −18.0)。**seed 違いで ±0.2bp** =
帰無較正はこの規模で安定している。結論は両方とも「帯の中」で一致。

#### ④ 最悪業種を除外して予算シミュ = 測れていない

| | 除外なし | 除外後 | 差 | 月あたり |
|---|---:|---:|---:|---:|
| TRAIN | +3,393,798 | +3,492,401 | +98,603 | **+1,555円** |
| TEST | +4,861,926 | +4,889,995 | +28,069 | **+395円** |

月次σ ≒ 12.6万円に対して **1.2% / 0.3%**。TRAIN がプラスなのは自明
(bp が最も低い業種を抜いたので)。意味があるのは TEST の +395円 で、実質ゼロ。

#### ⑤ 同業種の同日集中に上限 = **平均では**1つも勝たない

| 上限 | TRAIN | TEST |
|---|---:|---:|
| 2件まで | **−6,925円/月** | −9,192円/月 |
| 3件まで | **−6,451円/月** | +1,252円/月 |
| 4件まで | **−1,378円/月** | +425円/月 |

TRAIN は3水準とも損。TEST でプラスに転じる2つもσの1%。
**同じ上限が両方で良くないと固定できない**(§18.36)。

⚠⚠ **ただしこれは平均損益の話で、⑤の判定にはなっていない**(2026-09-06 ユーザー指摘)。
上限は **大負け対策(保険)** として提案されたもので、平均を少し犠牲にしても
**CVaR・最悪日・MaxDD が十分軽くなるなら成立する**。上の表にはその3つが無い。
→ ツールに左裾の列を追加した(下記)。**⑤ は未判定のまま**。

#### ★ 構造的な理由 — 除外系は N では**平均について**原理的に不利

**N の稼働率は約40%**(§18.55)。予算400万に対し投入は中央値162万しかないので、
**除外や上限で空いた枠を埋める代わりの銘柄がいない**。したがって除外の利得は
『抜いた集団の bp がマイナスであること』だけで、プラスなら純損になる。

⛔ **この論法は左裾には効かない。** 件数を減らせば裾は必ず軽くなるので、
「平均が下がるから不採用」とは言えない。⑤で見るべきは
**平均の低下に対して裾の改善が割に合うか**であって、平均の符号ではない。

#### データ品質は良好だった(言い訳が使えない)

年別の業種判明率 **2015〜2020 すべて100%**、全体の不明 0.1%(18件)。
⚠ ただし **今の上場一覧を遡及適用**しているので、上場廃止銘柄の欠落と
業種変更の未反映は残る(探索用と最初に宣言済み)。

#### ⛔ この検証で踏んだツールのバグ2件

| # | 内容 |
|---|---|
| 1 | **予算シミュのフィルタが効いていなかった**(`b4be106`)。`_ops_sim` が `_src = _src_all if a.pool == "all" else _pool_df` で、既定 `--pool all` では**渡したフィルタ後のフレームが無視される**。535件を除外しても差が **+0** だった。元フレーム側を絞ってプールを作り直す形に修正 + 「除外しても1円も動かない」ときは警告を出す |
| 2 | **`--min-ret1` を渡した実行を「★ 未使用(判定対象)」と表示していた**(`29bdb1e`)。この閾値(1.753)は §18.54 の `--explore` が**その窓で**見つけた値なので in-sample。ラベル・合格メッセージ・試行記録の3箇所を直し、**通算回数から除外**するようにした(数えると独立試行が水増しされ多重検定の補正が狂う) |

★ 作法: **「差が 0 だった」を「効果がゼロ」と読まない。** まずフィルタが
効いているかを確かめる。動かないほうが異常。

#### ★★ 結論 — **2つの問いに分ける**(2026-09-06 ユーザー指摘で訂正)

| 問い | 状態 |
|---|---|
| **業種で期待値を改善できるか**(①②④) | ⛔ **終了**。半導体も最悪業種も除外しない。33業種の除外ルールを作らない |
| **同業種集中で左裾を軽くできるか**(⑤) | ⛔ **終了**(下記)。ランダムに同数落とすほうが裾が軽い |

★ 経緯: 当初「業種経路は閉じる」と①②④だけで書いたのは**書きすぎだった**。
①②④は期待値の検定で、⑤は保険の検定。**平均が下がることは⑤の不合格理由にならない**
(2026-09-06 ユーザー指摘)。左裾を測り直した結果、⑤も別の理由で落ちた。
**両方の問いを別々に検定したうえで、業種経路は閉じる。**

#### ⛔⛔ ⑤ の実測 (2026-09-06) = **不合格。業種はランダムに落とすより裾が重い**

単調性は両窓とも ✅ だった。等価σ縮小も TRAIN4件 / TEST4件 / TEST3件 が ✅。
**しかしランダム帯が全部 逆向きだった。**

| 窓 | 上限 | 業種で落とす CVaR5% | ランダムに同数落とす 中央 | 差 |
|---|---:|---:|---:|---:|
| TRAIN | 4件 | −58,278 | −53,452 | **−4,826** |
| TRAIN | 3件 | −57,901 | −53,229 | **−4,672** |
| TRAIN | 2件 | −54,629 | −49,089 | **−5,540** |
| TEST | 4件 | −72,855 | −66,557 | **−6,298** |
| TEST | 3件 | −68,607 | −66,376 | −2,231 |
| TEST | 2件 | −66,214 | −61,247 | **−4,967** |

**6件すべてで業種のほうが裾が重い**(z = −1.04 〜 −3.84 / 5件が z≤−2)。
= 裾が軽くなったのは『件数が減ったから』で、**業種という基準は逆に効いている**。
同じ件数をサイコロで落とすほうが良い保険になる。

★ 決定打がもう1つ: **TEST の上限4件は 最悪日が −146,350 で基準と1円も変わらない**。
最悪の1日に同業種5件以上が無かった = **保険が本番で発動していない**。

#### ⛔ 判定ロジックの設計ミス(2026-09-06 に踏んだ)

ランダム帯の合否を `abs(z) >= 2.0` の**両側**で書いていたため、
上の6件が全部「帯の外」= 合格に見える表示になっていた。CVaR は負の値なので
**z<0 は「ランダムより重い」= 不合格**。片側(`z >= +2.0`)に直し、
`✅ ランダムより軽い` / `⛔ ランダムより重い` / `帯の中` と方向を明記するようにした。
最悪日の帯も出していなかったので追加。

★ **作法: 帯との比較は必ず片側で書き、どちらに外れたかをラベルに出す。**
  「帯の外」だけでは合格と不合格を区別できない。

#### ☐ (参考) ⑤ を再検証するときの手順(判定は**回す前に宣言済み**)

```
python analyze_gap_edge.py --days 4200 --split 2020-09-01 --min-ret1 1.753 \
  --min-gap-bp 100 --min-price 1000 --max-price 6000 --sector-scan --seeds 1000
```
⑤ が TRAIN/TEST 別に 月平均 / 月次σ / ÷σ / **日次CVaR5% / 最悪日 / MaxDD** /
**同数をランダムに落とした帯** / **等価σ縮小の月平均** を出す
(帯の本数は `--sector-cap-seeds`、既定12。1本ごとに予算シミュを回すので重い)。

**★ 事前宣言(2026-09-06。結果を見て動かさない):**
1. **両窓で なし→4→3→2 と単調に裾が改善しなければ終了**
2. **等価σ縮小の月平均を上回らなければ**、それはレバレッジを落としただけ(§18.38 #3b)
3. **ランダムに同数落とした帯の外**でなければ、業種で落としたからではない(§18.24)
4. ⛔ **結果を見て 2/3/4件 から選ぶことはできない**

3つとも満たす上限が両窓で一致したときだけ候補。1つでも欠ければ **⑤も終了**。

⚠ 検算の必要なもの: `_tail5` の CVaR5%・最悪日・MaxDD は合成系列4本で確認済み
(MaxDD は**曲線の先頭に 0 を置く**。置かないと初日から下げ続けた場合に
1日目の損失が山として扱われ過小になる — 実際に検算で見つけた)。

#### 残る負け対策

1. **契約データが要るもの** — 決算翌日 / 貸借区分 / 空売り残高。
   ~~J-Quants は `sectors33` は通るが `statements`・`master` が 403(契約範囲外)~~
   ⛔⛔ **訂正 (2026-09-22): この1行は誤り。→ §18.79**
   Standard で `statements` も `master` も取れる。403 の原因は**契約期間
   (2016-09-23以降)より前を要求したこと**。したがって「決算翌日フラグは
   契約が無いので諦めた」という判断も無効。
2. **執行改善** — 契約もデータも要らない。**`.\nexec --go` の実約定**でしか測れない
   (⛔ `.\norder` は発注しない板・ペーパー観測。実約定の営業日数に数えない)

---

### 18.65 ⛔ 大負け対策は尽きた (2026-09-06)。**残るのはサイズ調整だけ**

**1日で4系統を測り、全部落ちた。** 他AI2つ(ChatGPT / 別Claude)の提案も含めて掃いた。

#### ① 前夜の候補数で『その日を建てない』(`--sweep-cands`) = 不合格

軸3本(n_cand / n_cand_ma5 / n_cand50) × 5分位。**3条件すべて落ちた**:

| 条件 | 結果 |
|---|---|
| TRAIN と TEST で同じ分位が最悪 | ⛔ 3軸とも入れ替わる(Q2→Q1 / Q5→Q4 / Q2→Q1) |
| ランダムに同数の日を落とすより軽い | ⛔ 帯の外に出た2つは**どちらも逆側**(ランダムより重い) |
| 帰無較正 95%点超え | ⛔ p=0.056(実測 0.539 / 帰無95%点 **0.540**) — 0.001差 |

★★ **最悪日は候補数の Q4(上から2番目)にいた。** 両端ではない。
```
TRAIN 最悪日 −139,533 → 変わったのは n_cand Q4 と ma5 Q3 だけ
TEST  最悪日 −146,350 → 変わったのは n_cand Q4 と ma5 Q5 だけ
```
「多い日ほど危ない」も「少ない日ほど危ない」も成り立たない = **順序構造が無い**。
TRAIN/TEST で順位が入れ替わったのは偶然ではなくこれが理由。

⚠ `n_cand_ma5 Q5` だけ TRAIN で z=+4.93(帯の外・良い側)、CVaR も両窓17〜18%改善。
  だが TEST で月平均が半減(+66,602→+31,774)し ÷σ も基準割れ。TRAIN で効いて
  TEST で崩れる典型。**帯だけで判定していたら通していた。**

⚠ `n_cand50` は50超の日が73.6%あるため Q3/Q4 が**0日**、Q5が926日(69%)に縮退。
  5検定のうち実質1つしか機能していない(整数の軸は分位の境界が重なる)。

#### ② サイジングの平準化(`--sweep-size`) = 不合格。**前提が偽だった**

⛔⛔ **まず 4本中3本が基準の複製だった**(別Claudeの指摘。検算で確認):
```python
_lot = a.qty                                        # = 100
_lot = min(_lot, max(a.qty, int(...) * a.qty))      # min(100, ≥100) = 必ず100
```
`cap30 / cap40 / atr3000 / atr5000` は 100株固定と**定義上まったく同じ**。
ATR も ATR×100 > size_arg なら int(...)=0 → max で 100。建値1,000〜6,000円・
ATR 2〜3% ではほぼ全件そうなる。→ cap を「1単元が上限超なら建てない」に修正。

⛔ **`equal`(予算÷件数)は平準化ではなくレバレッジ**。1件の日に1,700株まで膨らみ
投入が 162万→350万 になる。平均もσも同率で伸びるので ÷σ が動かず、
それを見て「サイジングは無効」と結論していた。→ `level`(目標を万円で固定)を新設。

**★ 平準化は物理的に成功した。しかし効かなかった。**

| サイジング | 床に当たった | 投入CV | 投入/日 | **÷σ** | 効率% |
|---|---:|---:|---:|---:|---:|
| 100株固定 ★現行 | 0 (0.0%) | **0.72** | 160万 | **0.46** | 3.22 |
| 平準化 120万/日 | 9,411 (97.7%) | 0.56 | 176万 | 0.44 | 3.26 |
| 金額均等(予算÷件数) | 4,549 (47.4%) | **0.09** | — | — | — |
| 金額均等 上限50万 | 4,549 (47.4%) | 0.50 | 218万 | 0.44 | 2.81 |

**投入CV は 0.72 → 0.09 まで落ちた**(平準化は成立)。**それでも ÷σ は改善しない。**

理由: σ ∝ 1/√(1+CV²) は **各日の1円あたりリターンが独立同分布**を仮定している。
実測では違う。平準化は候補が少ない日に多く張る操作で、その日の効率が低い:
```
σ削減 9.3%(CV 0.72→0.50) × 投入増 1.36 = σ 1.23倍
月平均 51,421 → 61,058 = 1.19倍
÷σ = 0.46 × (1.19/1.23) = 0.44   ← 実測と一致
```
**理論は正しく、前提が偽だった。**

⚠ 件数上限5件が TRAIN で ÷σ 0.54〜0.55 と目立つが、発注順が |ギャップ|降順なので
  「上限5件」=「ギャップ上位5件だけ」= §18.53 の再発見。TEST では全部 基準割れ。

#### ③ 同日ヘッジ(`--hedge`) = 不合格。**TESTの改善は日経のドリフトだった**

★ **β は両窓でほぼ同一**: TRAIN −15,010円/日経+1% / TEST −15,087 (0.5%差) / R² 0.238。
  §18.30 で lss の月別βが2ヶ月符号反転したのとは違う。**関係は実在して安定**。

リスク削減も両窓で出る: σ −6.0〜6.6% / CVaR 9.0〜12.5%改善 / 最悪日 8.7〜13.9%改善。

**しかし月平均の動きが逆:** TRAIN **−10,763円/月** / TEST **+2,956円/月**。
```
ヘッジコスト = 15,010 × 100 × 4.4bp = 660円/日 (両窓で同じ)
TRAIN 日次 −570 → 日経の日中ドリフト = +0.006%/日
TEST  日次 +156 → 日経の日中ドリフト = **+0.054%/日**(年率+13%)
```
**TEST は日経が日中に上げ続けていたので、先物を買ったこと自体が儲かっただけ。**
§18.61 に記録した現象が数字で裏付けられた。§18.19 で日中ドリフトは長期ゼロ(t=0.59)
なので当てにできない。**ドリフトをゼロと置くと両窓とも落ちる**:
```
TEST 比率1.0  月平均 66,602 − 660×18.9日 = 54,120
              等価CVaR縮小 = 66,602 × (68,685/74,610) = 61,313
              54,120 < 61,313 → 落ちる
```

★ 天井が R² から決まる: β を完全に消すと σ ×√(1−0.238)=0.873 → 同σまで戻して **+14.5%**。
  先物コストは日次平均の **23.9%**。**コストが天井の1.6倍**なので原理的に勝てない。
  逆算すると **2.7bp を切れば黒字**。ミニ/ETF・指値執行・引成手仕舞いは未測定。

#### ④ 銘柄の組合せで β を圧縮(`--beta-scan` / `--beta-run`) = 不合格

別Claudeの最有力提案。「予測せず曝露だけ下げる」ので9回ヌルの選別軸とは種類が違い、
先物の660円/日も払わない。**事前チェックの2条件は通った**:

| | TRAIN | TEST |
|---|---|---|
| 50%残しで β比 ≤0.80 | 0.68 ✅ | 0.53 ✅ |
| β分位の bp/件 が単調に増えていない | 山型(Q3最良) ✅ | むしろ減少(Q1が27.6bpで最良) ✅ |

⛔ **しかし β の持続性が無い**。重ならない窓どうしの相関:
```
TRAIN +0.274   TEST +0.455
```
過去120日のβが次の120日のβを言い当てる度合いがこれ。実効的な削減はこの分だけ
減衰するので、50%残しの天井 +7.2%/+9.8% は **実効 +2.0%/+4.9%** に縮む。

**本番(TRAIN のみ / 日ごとに β 下位を残す / 予算シミュ):**

| 残す | 月平均 | ÷σ | CVaR | **最悪日** | ①等価3つ | ②ランダム帯 |
|---|---:|---:|---:|---:|---|---|
| 90% | +47,278 | 0.46 | −60,436 | **−139,533** | σ のみ | ÷σ +1.46 / CVaR −0.20 帯の中 |
| 75% | +41,270 | 0.45 | −57,413 | **−139,533** | — | ÷σ +1.66 / CVaR **+2.48** |
| 50% | +31,600 | 0.38 | −55,776 | **−139,533** | — | 帯の中 |

★★ **最悪日が3行とも1円も動かない。** §18.64 ⑤(同業種上限)とまったく同じ形 =
**保険が本番で発動していない**。別の軸で2回同じ落ち方を見た。

75% だけ CVaR が帯の外(良い側)に出たが、月平均が −20% 落ちて等価σ +41,455 に対し
41,270。**σ の観点でぴったり「0.8倍で取引した」**と同じ。

#### ⛔ この日に踏んだ設計ミス(全部 記録して直した)

| # | 内容 |
|---|---|
| 1 | **ランダム帯の判定を両側 `abs(z)>=2` で書いていた**。CVaR は負なので z<0 は「ランダムより裾が重い」= 不合格なのに『帯の外』と表示。§18.64 ⑤ の6件すべてがこれだった → 片側 + 方向を明記 |
| 2 | **事前チェックに相関を合格条件として入れていなかった**。ツールは「⛔ 0.3未満」と出すのに判定に含めず、形式的に通してしまった。後から足すのはゴールポストを動かすので、効果量の補正として扱った |
| 3 | **事前チェックなのに TEST も印字した**。この仮説について TEST が既見になった |
| 4 | **2窓のβ比較で重なる窓(120日 vs 250日)を使った**。直近を共有するので**推定ノイズまで相関し、真のβ差ゼロでも相関0.76** が出る(合成データで確認) → 重ならない1つ前の窓に変更 |
| 5 | **`--beta-scan` を `import ble` より前に置いて NameError**。しかも except で飲んで「日経を取れません」とデータの問題に見せていた |
| 6 | **投入/日 の分母がブロックごとに違った**(全営業日 vs 取引のあった日)。90%残しで248万 と出て --sweep-size の160万と並べられなかった |
...
| 入口 | 状態 |
|---|---|
| **① その日を建てない** | 外部変数19本(§18.34b) / 合格件数(§18.58) / 同日発注数(§18.13) / 日内上位N(§18.24) / **前夜の候補数(本節①)** = **5回ヌル** |
| **② 途中で降りる** | 損切りATR倍24セル(§18.55) / 固定円(§18.56) / ポートフォリオ(§18.62) / 決済時刻(§18.55) / 寄り5分撤退 = **全滅** |
| **③ 建てる対象を選ぶ** | 選別軸 **9回ヌル**(§18.12/13/24/31/38/48⑪/52/64) + **β圧縮(本節④)** + **銘柄そのもの(§18.68)** |
| ④ 曝露を打ち消す | 先物ヘッジ(本節③) = コストが天井の1.6倍 |
| ⑤ サイズを変える | ✅ **これだけ効く。ただし最適化ではなくリスク許容度の宣言**(§18.38 #3b) |

**「利益を落とさずに左裾だけ削る方法は無い」が確定した。**
裾を軽くするだけなら取引を減らせばいつでもできるが、それには**選別能力**が要り、
選別軸は10回ヌルだった。

⚠ 唯一 数字の上で残っているのは **先物コストを 2.7bp 以下にできるか**だけ
(ミニ/ETF・指値執行・引成手仕舞いは未測定)。ただし天井は +14.5% で、
執行改善(遅延1分で −10.4bp / エッジ +15.7bp = **エッジの最大66%**)のほうが1桁大きい。

**→ 研究資源は `.\nexec --go` の実約定の蓄積に寄せる。**

---

### 18.66 ★★★ 実約定の打ち切り条件 (2026-09-06 に**データが揃う前**に宣言)

**現時点 10件 / 件数加重エントリー滑り −34.0bp / 損益分岐 −15.0bp。
既に分岐の2倍悪い状態で、あと20件貯める。**
30件たまってから基準を決めると、その時点の数字を見て基準が動く。だから先に書く。
...
| 数える | **`.\nexec --go` の実約定だけ** |
| 数えない | ⛔ `.\norder`(発注しない板・ペーパー観測) |
| 指標 | **件数加重のエントリー滑り(bp)** = (実売値 − その日の始値) / 始値 |
| 除外 | 板記録が壊れている日(下記の品質チェックに落ちた日) |

⛔ **板記録の品質チェック**(2026-09-03 の事例)。1周の所要が 0.0秒 や
銘柄数が普段の 1/6 の日は、板の深さが取れておらず寄り値だけが入っている疑い。
**判定に入れる前に必ず確認する。** 実際 9/3 の2件を抜くと点推定が
−34.0 → −41.0bp と動いた。

#### 判定点 = **20件 かつ 8営業日** の両方に達した時点

⛔ 件数だけだと 13件建つ日が2日で届き、**日クラスタの実効サンプルが2**になる。
日数だけだと薄い日が続いて件数が足りない。**両方**を条件にする。
判定は営業日ごとの件数加重平均を出し、その**日次の平均と標準誤差**で見る。

#### 閾値(事前宣言・動かさない)

グロスのエッジは判定窓(TRAIN 2015-03〜2020-08)で **+15.7bp/件**。
呼値は建値 ~2,257円 で1ティック=1円=4.4bp、エントリー片道のみ
(決済は引けMOC = オークションなのでスプレッドを払わない)。
→ **エントリー遅延に使える予算は 15.7 − 4.4 ≈ 11.3bp**。

| 件数加重エントリー滑り | 判断 |
|---|---|
| **≥ −6bp** | ✅ 続行。エッジの半分以上が残る |
| −6 〜 −11bp | ⚠ 続行して30件まで。ネットはほぼゼロ |
| **≤ −11bp** | ⛔ **09:00確認+成行/保護指値 を棄却** |

⚠ 現時点の −34.0bp は既にこの3倍悪い。**通る見込みは高くない**。

#### ⛔ 棄却したときの受け皿は「未測定」であって「無い」ではない

**前夜に `前日終値 × 1.01` の寄付指値を置く**と、板寄せの性質から
**始値 ≥ その価格のときだけ、始値で約定する** = N の合格条件とまったく同じ。
遅延ゼロ・スプレッドゼロで、バックテストの `--exec-mode auction` の前提そのもの。

⚠ ただし **発注時に委託保証金が要る**ので、置ける本数は予算で決まる
(400万 ÷ 約22.6万 ≈ 17本)。候補は中央値 91〜108件/日 あるので大半を置けない。
そして **どれを置くかは前夜に決める**ので、ギャップでは選べない(§18.48 ⑥ と同じ構造)。
§18.40 では J の前夜指値が fill率19.8% で 10ヶ月 −231万 と落ちている。

**→ 棄却するなら、まずこの形を測る。測らずに「Nを止める」と決めない。**

---

### 18.67 ⛔ 予算シミュの発注順は **live では実装できない** (2026-09-06 発覚)

`_ops_sim` は その日の合格銘柄を **|ギャップ|降順**に並べて予算を充てる
(`analyze_gap_edge.py:1278`)。⛔ **live はこの順に建てられない。**
09:00 に全銘柄のギャップが同時に分かるわけではなく、寄った順に処理するため。

#### そして「遅く寄る銘柄ほどギャップが大きい」

2026-09-04 の13件を寄り時刻で割った実測:
...
**ギャップ上位3件(+714 / +502 / +324)が全部、遅く寄った4枠に入っていた。**
(⚠ n=1日。しかも「上位3件」を見てから数えているので 1.4% は p値ではない。
  同じ集計を3営業日ぶんの板記録で回すこと)

#### ★ 帰結 — 予算が効く日は、切られるのが **いちばん良いトレード**

ランダムに切られるのではない。**遅く寄る=高ギャップ=良いトレードが最後に来て、
最初に予算から落ちる。** 予算シミュはギャップ降順なのでこれを一切見ていない。

| 2026-09-04 | 建った | 見送り |
|---|---|---|
| 400万 | 13件全部(残り12,350円) | 0 |
| **200万(= live の既定)** | **6件** | **7件 = +26,400円 = その日の利益の58%** |

⛔ **`nexec.bat` の既定は `--budget 200`、研究は 400万**。§18.9 の鉄則違反で、
§18.48 ⑤(レポート400万 vs ライブ300万)とまったく同じ形。

★ **ただし滑りの測定(1件あたりのbp)には影響しない**ので、§18.66 のゲートは
そのまま有効。⛔ **影響するのは損益の読み方**で、
**live の損益を「N の成績」として読むと体系的に過小になる**。分けて記録すること。

#### ☐ 出てきた候補(未測定): 予算の一部を遅い枠に残す

「09:05 までは予算の X% しか使わない」は**前夜に決められる**ので先読みにならない。
⚠ ただしバックテストは寄り時刻を持たない(5分足の先頭バーで代理でき、2年ぶんだけ)。
測るなら板記録か5分足。**§18.66 のゲートが先。**

#### ★ 2026-09-07 から live 予算 **200万 → 400万**(ユーザー決定)

```powershell
.\nexec --go --budget 400          # 07:50まで
python n_paper.py --close --budget 400 --seq-sides n
```

理由: 200万では予算が切れて**遅く寄る=高ギャップの良いトレードが系統的に落ちて
いた**(上表 / 9-04 は見送り7件 = その日の利益の58%)。400万にすれば
**研究(`analyze_gap_edge` / `n_capital` の既定)と live が揃う**(§18.9 の鉄則)。

| | |
|---|---:|
| 9-04 の証拠金ピーク(実測) | **3,987,650円**(17銘柄同時) |
| 委託保証金率 33.3% で必要な保証金 | 約133万円 |
| 口座の現金(掛け目100%) | 180万円 → **維持率45%** |

★ **追証は構造的にかからない**。N は同日決済なので**大引け時点の建玉がゼロ**で、
追証は引け後の建玉に対して判定される。維持率20%まで落ちるには含み損100万が必要で、
実測の最悪日(−146,350円 / 現金の8%)とは桁が違う。
⛔ ただし **§18.46 のように決済に失敗して持ち越すとその日は判定対象**になる。
⚠ 保証金は**発注時**に拘束されるので、余力の見え方は 180万 − 133万 = 47万 になる(正常)。

⛔ **§18.66 のゲート(20件 かつ 8営業日)は予算変更の影響を受けない。**
指標が **1件あたりの bp** でサイズ非依存であり、N は100株固定なので予算を上げても
**銘柄数が増えるだけで1銘柄の株数は変わらない**。判定日は日数が律速なので早まらない。

⚠ **滑りの集計は変更前後を合算してよい(bp はサイズ非依存)。損益の合計は分けること。**
そして増える取引は「遅く寄る銘柄」なので、板1周36.5秒ぶん検知が遅れる。
**滑りが少し悪く出る可能性があるが、それは200万では見えていなかった本当のコスト。**

---

### 18.68 ⛔ 銘柄そのものも選択軸にならない (2026-09-06)。**11回目のヌル・最もきれいな形**

「特に負けが多い銘柄の傾向はないか」というユーザーの問いに対する実測。

```
python analyze_gap_edge.py --days 4200 --split 2020-09-01 --min-ret1 1.753 \
  --min-gap-bp 100 --min-price 1000 --max-price 6000 --dump-picks n_picks.csv
python analyze_symbol_edge.py --csv n_picks.csv --split 2020-09-01 --min-trades 6
```

#### ★ §18.48 ⑪(J/lss)とは前提が違う。**N では測れた**

| | J/lss | **N** |
|---|---:|---:|
| データ | 5分足 25ヶ月 | **日足 11.5年** |
| 取引 / 銘柄 | 2.31件 | **11.3件** |
| 両側6件以上ある銘柄 | **0**(測定不能) | **320** |

J は「両側6件を集めるのに27ヶ月要るが5分足は25ヶ月しかない」で**測れずに終わった**。
N は測れて、**測ったらゼロ**だった。「まだ分からない」ではなく「無い」。

#### ① 全期間のばらつきは巨大に見える(が、これがワナ)

1,292銘柄 / 1株あたり 平均 +17.3bp / **σ 80.1** / 最良 +470.3 / 最悪 −399.0。
下位5は 4022.T −399.0 / 9278.T −208.6 / 1515.T −197.5 …
**全部 6〜14件の平均**。「負けが多い銘柄」に見えるのはこれ。

#### ② 持続性 = ゼロ。**TRAIN 最良が TEST で最悪**

| 分位 | 銘柄 | TRAIN bp | TEST bp |
|---|---:|---:|---:|
| 最悪 | 64 | −87.5 | **−6.6** |
| Q2 | 64 | −20.3 | +20.9 |
| Q3 | 64 | +7.6 | +9.9 |
...
**順位相関 = −0.007**(ほぼ完全にゼロ) / 分位スプレッド +4.3bp。
平均回帰どころか順位が逆転している。TRAIN 最悪分位も TEST では −6.6bp と
ほぼゼロに戻る = **「負けが多い銘柄」は次の期間には消える**。

#### ③ 帰無較正が効いた — 較正なしなら誤判定していた

```
分位スプレッド  実測 +4.3bp  / 帰無 平均 **+5.4bp** σ11.5  → z=−0.10 p=0.955
順位相関       実測 −0.007  / 帰無 平均 +0.018 σ0.053     → z=−0.45 p=0.685
```

★★ **識別力がゼロでも +5.4bp のスプレッドは普通に出る。実測はその平均より下。**
§18.48 ⑪ で戦略(5グループ)の帰無が +19.1bp だったのと同じで、
**グループに分けた時点で差は必ず出る**。ラベルは『同じ日の中だけ』でシャッフルする
(日をまたぐと日効果まで壊れて帰無分布が狭くなり偽陽性を過小評価する / §18.13)。

#### ★ 結論

**「負けが多い銘柄」は過去のばらつきとしては存在するが、未来の予測力はゼロ。**
勝っている銘柄だけ残す運用は §18.12 の BTスコアと同じ失敗になる
(過去成績で選ぶ → 先読みを外したら識別力ゼロ)。

⛔ ①の表を見て除外リストを作らないこと。**②③を通らなければ①はただのノイズ。**

---

### 18.69 ★★★ PUSH配信(WebSocket)で **50件の壁は超えられる** (2026-09-07 実測)

**§18.45 で K(watch拡大)を棄却した2つの根拠が、両方とも消えた。**

| 根拠 | 状態 |
|---|---|
| ① 登録上限50件 | ✅ **回せる**(下記の実測) |
| ② 読む時間のコスト(299銘柄で3.6分・−30bp) | ✅ **PUSH は読まない** |

#### なぜ気づかなかったか

§18.44 は「バッチ回し(50件登録→読む→解除→次の50件)は2周目も速くならない」で
棄却したが、あれは **REST の読み取り**の測定だった。PUSH は読む工程が無い。
残るのは『登録してから配信が始まるまで』の待ちだけで、**一度も測っていなかった**。

そして **始値は動かない**ので、遅れて届いても値は正しい。
問われるのは「何秒遅れるか = 何bp 失うか」だけになる。

#### 実測1: PUSH に OpeningPrice が入るか → **入る** (50/50)

`python kabu_ws.py --prod --from-signals --seconds 60 --compare-rest`
(2026-09-07 場中 / 本番 / 今日の watch50)

| | |
|---|---|
| **始値が届いた銘柄** | **50/50** |
| 例 | `2413 始値 1759.5 / OpeningPriceTime 2026-09-07T09:00:00+09:00` |
| PUSH でキャッシュが埋まるまで | 10秒(2秒で42銘柄) |
| **REST 1周** | **35.8秒** ← §18.44 の朝の実測 36.5秒とほぼ一致 |

§18.45 に「読む時間はゼロになる」と書いたまま未検証だった部分が確定した。

#### 実測2: 回せるか → **回せる**

`python kabu_ws.py --prod --from-signals --rotate 150`(2回実行)

| batch | 銘柄 | 登録 | 初回受信 | 全件揃う |
|---|---|---:|---:|---:|
| 1 | 今日の watch50 | 0.0s | 0.3s | 2.3s / 22.8s |
| **2** | **今日 未登録の50件** | **0.0s** | **0.3s** | **0.8s / 2.5s** |
| 3 | 同上 | 400 で失敗 | — | 測定外 |

**★ batch2 が本命。朝から一度も登録していない銘柄が、登録から1〜3秒で
OpeningPrice まで届いた。** = kabu は購読開始時に **現在の状態を送ってくる**
(差分だけではない)。09:00:05 に登録しても 09:00:00 の始値が取れる見込み。

§18.44 の「コールド48秒」は REST の読み取りの話で、PUSH はそこを通らない。

⚠ batch1 の 22.8s は **1銘柄のはぐれ**(初回受信は0.3s)。実運用は
『全部揃うのを待つ』のではなく **寄った銘柄から順に発注**するので、
はぐれはその銘柄だけの遅れになる。

#### ⛔ batch3 の 400 は **無効な銘柄コード**だった(件数ではない)

半分ずつ試して切り分けた(`--rotate` に内蔵)。意味が正反対の2つがあるため:

| 原因 | 意味 |
|---|---|
| (a) 無効なコードが混ざった | 実運用では起きない → **回せる** |
| (b) 総登録数が50を超えた | **回すこと自体が不可能** |

```
左25件=OK / 右25件=NG → 左12件=OK / 右13件=NG → … → 左1件=NG / 右1件=OK
✅ 犯人は 5486 (日立金属 / 2023年 上場廃止)
```

**半分ずつなら通る = (a)。** `check_board_limits._DEFAULT` に上場廃止銘柄が
残っていたのが原因で、リストから削除した(118件)。

★★ **/register は部分受理しない。1件でも無効なコードがあると PUT 全体が
400 で落ちる。** 候補リストに上場廃止銘柄を混ぜないこと。

#### 見込みの利得

| | 平均待ち | 減衰(§18.44 の1分足実測から) |
|---|---:|---:|
| 現行 REST(50銘柄 36.5秒) | 18秒 | **約 −6bp** |
| PUSH (`--ws`) | 1〜2秒 | ほぼ 0 |
| PUSH + ローテーション batch2 | 1〜3秒 | 約 −0.2〜0.7bp |

- **速度だけで +5〜6bp/件**(N のグロス +15.7bp/件 の4割弱)
- **watch 拡大**は §18.55 ③ の実測で 月+49,587 → **+87,267 (+76%)**

#### 実装 (既定OFF)

| | |
|---|---|
| `kabu_ws.py` | `BoardStream`(受けて溜めるだけ・判断しない) + 単体の疎通確認 + `--rotate` |
| `k_open_confirm.py --ws` | PUSH で **当日の始値まで届いた**銘柄だけ HTTP を省く |
| `--ws-only` | 検証用。REST で補わない |
| 日付ガード | `_open_today()` に共通化。/board は引け後も当日の始値を返すので、前日ぶんを『届いた』と誤判定すると REST で読み直さなくなる |
| `--every` | `--ws` かつ既定(10)なら **2** に下げる。PUSH は HTTP を出さないので速く回してもコストが無い |

**PUSH が無音でも未接続でも、REST 全件に落ちて今日と同じ動きになる**(fail-safe)。
合成データで4点検証済み(前日の始値を弾く / 届いたぶんだけ省く / 無音でも同一 /
`--ws-only`)。

#### ⛔ ただし順序は変えない

これは **1日・2回の場中の測定**。09:00 の板寄せ直後は最も混むので、
採否は朝に測り直してから。そして:

> **§18.66 の実約定ゲート(20件かつ8営業日)が先。**
> いまの実測は **−34.0bp** で、グロス +15.7bp に対して3倍の赤字。
> watch を1.8倍にしても、1件あたりがマイナスなら損が1.8倍になるだけ。

速度改善(+5〜6bp)を入れても −28bp で、損益分岐(−11bp)には届かない。
**残りの −28bp がどこから来ているのかは、実約定を貯めないと分からない。**

#### ⚠ 実運用に入れる前に残っていること

1. **09:00 の板寄せ直後で測り直す**(場中とは混み方が違う)。
   発注を止めずに測るなら、batch1 を発注した直後に batch2/3 を登録して
   **記録だけする**(発注経路に触れない)。
2. **429 対策**。診断の6往復で `unregister/all` と `register` の両方に429が出た。
   ローテーションを実運用に入れるなら間隔を空ける設計が要る。
3. **400 のリカバリ**。無効コード1件でバッチ全体が落ちるので、
   落ちたら犯人を外して再試行する経路が要る(現状は次周でやり直すだけ)。

---

### 18.70 ★★ 2026-09-07 の確定事項。**「不約定が本当のコスト」は誤りだった**

購入した株式歩み値(2026年8月・98,203,298行)と先物1分足(25ヶ月)で、
N の執行と発注順を実測した。**私(Claude)がこの日3回言った「②不約定が
本当のコスト」は取り消す。**

#### ① ⛔ 「保護指値」は存在しない。指値は **始値ちょうど**

| 場所 | 実際 |
|---|---|
| `k_open_confirm.py:237` `--n-limit-ticks` | **既定 0** |
| 同 `:241` `--n-limit-max-bp` | 既定 7.5 |
| 同 `:1375` | `_lim = float(_op)` でループが **0回** → 指値=始値 |

**1ティックでも下げていれば約定しない。** −3%ガードではない。

⛔ `nexec.bat` の REM には `--n-limit-ticks 100 --n-limit-max-bp 300` と
「Do NOT use a limit AT the open」が書いてあったが、**その引数はコマンド
ラインに一度も無く**、コード側は `:231` に逆の決定を根拠つきで書いていた
(2026-08-31 レビュー④)。**反対向きの決定が2つあり、片方の記述が更新されて
いなかった。** 2026-09-07 に1時間これで溶かした(不約定を「保護指値が3%の
崩れを弾いた」と読んだが、真相は「1ティック下げただけで刺さらない」)。
REM は実装に合わせて書き直した。

★ **作法: `.bat` のコメントを実装だと思わない。** 既定値はコードで確認する。

#### ② ⛔ そして約定させても損益は増えない(指値の深さスイープ)

歩み値469候補 / 20営業日 / ライブ順 / 予算400万 / **不約定でも枠は消費**:

| watch50 | 注文 | 約定 | 約定率 | 損益 |
|---|---:|---:|---:|---:|
| **始値ちょうど(現行)** | 181 | 139 | 77% | **+222,750** |
| −25bp | 181 | 156 | 86% | +229,578 |
| −100bp | 181 | 179 | 99% | +212,674 |
| 成行(+10秒) | 181 | 181 | **100%** | +212,950 |

**約定率は単調に上がるのに損益は上がらない。** 幅 +201,685〜+229,578 は
ランダム順の帯 σ=35,576 の内側 = 全部「測れていない」。

取り逃した78件の内訳:

| | 損益 | 勝ち/負け |
|---|---:|---|
| 始値で建てられたら | **+248,450** | 50 / 28 |
| **+10秒の実価格で建てたら** | **+74,500** | **42 / 36** |

**「勝つはずだった銘柄」に見えたのは始値建ての前提のせい。**
実際に注文が届く +10秒には値が動いていて、その分がほぼ全部消える。

⚠ `analyze_entry_slip` の②も「理想=始値建て」なので同じぶん過大に出る。
**あれは上限であって取り戻せる額ではない。**

→ **不約定はこのままでよい。指値=始値を変えない。**

#### ③ 実測の滑り(20件 / 5営業日)

| 日 | 件数 | 件数加重bp |
|---|---:|---:|
| 09-01 / 09-02 / 09-03 / 09-04 | 1 / 1 / 2 / 6 | +14.2 / −92.7 / −6.1 / −41.5 |
| **09-07** | **10** | **+28.2** |
| 通算 | 20 | **−2.9** |

前回まで −34.0bp だったのが今日で −2.9bp。⚠ ただし **検知遅れ(+6〜7秒)も
約定遅れ(+8〜11秒)も1日も変わっていない。変わったのは符号だけ**で、
その日の寄り直後の値動きの向きに支配されている。同日相関があるので実効
サンプルは20件ではなく **5日**(3勝2敗で負け越し)。改善したと読むのは早い。

⚠ **ゲートに逆選択の疑い**: 寄り後に下がる日 = N が勝つ日 = 滑りが不利に出る。
滑りだけ見ると「N が負ける日ほど滑りが有利」に見える。実際 09-07 は
滑り +28.2bp(有利)で実損益 −29,030円(大負け)。

#### ④ ★ 発注順: レポート(ギャップ降順) vs ライブ(寄った順)

**レポートは |ギャップ|降順に予算へ詰める。ライブは寄ったグループ順 →
グループ内でギャップ降順。** 2026-09-07 は必要資金 3,949,900 まで1円単位で
再現でき、**10件が1銘柄も重ならなかった**(レポートの10件が全部ライブの
「見送り18件」の中)。
...
| | 同一の日 | ズレた日の勝敗 | 合計差 | **t** |
|---|---:|---|---:|---:|
| watch50 | **14/20** | live 3 / gap 3 | +20,450 | **+0.41** |
| watch100 | 8/20 | live 5 / gap 7 | −48,550 | −0.35 |
| **watch150** | 7/20 | **live 3 / gap 10** | **−296,800** | **−2.24** |

- **watch50 では7割の日が1銘柄も違わない**(予算が全候補を建てられるので
  順序が関係ない)。ズレる3割も向きはコイン投げ。t=2 まで**約2年**
- → **watch50 のままならレポートはライブの偏りのない推定**。今日のズレを
  見て何かを直す必要はない
- ⛔ **watch150 は t=−2.24 で systematic**。低流動性を足すと遅寄りが増え、
  遅寄り=大ギャップ(下記⑤)なのでギャップ降順とライブ順が構造的に逆を向く。
  **レポートが watch拡大を良く見せているのは、ライブが実現できない順序で
  測っているから。** §18.65 の「watch拡大は追わない」を別角度から支持

ランダム順24本の帯に置くと watch50 は gap z=−0.08 / liq +1.39 / live +0.50 で
**3つとも帯の中**(§18.21/§18.24/§18.31 に続き発注順は4回目のヌル)。

#### ⑤ ★★ 「遅く寄る銘柄ほどギャップが大きい」が確定 (§18.67 の昇格)

490候補 / 20営業日:

| 寄り時刻 | 件数 | ギャップ平均 |
|---|---:|---:|
| 09:00:00〜:10 | 321 | 149.5bp |
| 〜09:05 | 105 | 282.7 |
| 〜09:10 | 35 | **553.0** |
| 09:10以降 | 20 | **1,077.1** |

**日ごとのスピアマン +0.651 / 19営業日すべてプラス / t=+14.37。**
§18.67 は n=1日の観察だったが、20営業日で確定した。

⛔ **ただし §18.67 の「切られるのがいちばん良いトレード」は8月には逆。**
...
ギャップ帯でも 200〜300bp が −134.3bp/件 と最悪。
**予算400万の判断は変えなくてよいが(研究と live を揃える別の理由がある)、
§18.67 の「見送り7件=その日の利益の58%」という期待は取り下げる。**

#### ⑥ 8月の歩み値・帯別 (予算前・09:10まで)

| 帯 | 件数 | 合計 | bp/件 |
|---|---:|---:|---:|
| 1〜50位 | 225 | +57,350 | **+5.4** |
| 51〜100位 | 149 | **−655,800** | **−133.5** |
| 101〜150位 | 95 | −459,900 | −159.3 |

**予算前の全候補は8月に −1,078,050円。** watch100 の追加32件が +36,900円
なのは、51〜100位の損失が**候補が多い日に集中**していて(08-05 −535,450 /
08-13 −112,000)、その日は1〜50位だけで予算が埋まるため自動的に建たないから。

| | 日数 | 51〜100位の件数 | 1日あたり |
|---|---:|---:|---:|
| 1〜50位が13件未満(枠が余る) | 15 | 77 | **+557円** |
| 1〜50位が13件以上(枠が埋まる) | 5 | 72 | **−132,830円** |

**機構であって運ではない**(候補が多い日 = 相場が上げた日 = N が負ける日 /
§18.60)。ただし §18.58① で「前夜の候補数」は TRAIN/TEST で反転して不合格
なので、恒常性は無い。**watch100 はシャドー記録を続けるだけ。採用に向かう
話ではない。**

#### ⑦ 先物1分足(DataCube)の仕様検査 = 合格

`python check_futures_spec.py --dir <folder>` (新規)。価格は1つも読まない。

| | |
|---|---|
| 検査 | **25ヶ月**(2023-01〜2024-12 + 2026-08) / 511営業日 |
| **書式ドリフト** | **ゼロ**。列15 / `session_id` {3,999} / `0845` あり(時刻は**バー開始**) |
| セッション | 999=日中 `0845`-`1540` + `1545`(クロージング) / 3=夜間 `1700`-`2359`,`0000`-`0600` |
| 出来高0の行 | 全月ゼロ = **欠損は行ごと落ちる** |
| index_type | **19=mini**(毎月限月) / **18=ラージ**(3,6,9,12月のみ) |
| mini vs ラージ | 終値の中央差 **0円** / 5円以内79%。⚠ ラージは夜間が歯抜け、日中の出来高も 44枚(mini 668枚) |

⚠ 株式歩み値では書式が変わっていた(サンプル2021-08 `session='1'` vs
実データ2026-08 `'01'` で 9,820万行が全滅)。**先物では起きていない。**

**限月交代は暦で100%再現できる:**
2023-01〜2024-12 の交代8回が**すべて四半期SQ(第2金曜)の前営業日**。例外なし。
出来高1位で選ぶと 98.4%(511日中8日外し、枯れかけた限月を使う)。
→ **カレンダー規則を使う。**

⛔ **2024-08-06 は 08:45-08:59 が 2/15分しかない。** 8/5 の暴落(−12.4%)の
翌日で、寄り直後にサーキットブレーカーで止まったため(出来高0の行は無く
「行ごと落ちる」仕様)。**サンプル中で最も極端な日が、まさに特徴量を作れない
日。**除外して記録する(`FUTURES_FREEZE.md` §1.3)。

#### ⑧ 日経予測の天井 (購入判断の根拠)

§18.60 の分解より σ削減 = `1 − √(1 − 0.235 × ②)`:

| ② | σ削減 |
|---:|---:|
| 0.045(無料日足19変数・2,611営業日) | 0.5% |
| 0.15 | 1.8% |
| **0.81** | **10%** |
| 1.00 | 12.5% |

**σ を10%削るには ②=0.81 が要る = ヘッジ/サイズ調整ルートは何を買っても
届かない。** 生きているのは裾(CVaR)だけ。

⚠ 2026-08 の n=20 で「夜間リターン → 日経日中 r=+0.604」が出たが、真の相関が
0.21 なら n=20 で 0.604 以上が出る片側確率 **2.2%**、11特徴試せば **22%**。
そして 2026-08 は探索済み = **ホールドアウトから焼けた**(`FUTURES_FREEZE.md` §1.4)。

#### ⑨ 板データは買わない

JPX「10本板ヒストリカルデータ」は **月額40万円(税抜)**、FLEX Historical の
スポットでも月4万円。**買わない。** 板・スプレッド・100株の実約定価格は
`.\nexec --go` の実約定で前向きに貯める(追加作業は不要 / §18.66)。

---

### 18.71 ⛔⛔ `^N225` の**始値**は取引できない (2026-09-07 確定)。①と β の再測定が要る

先物1分足2年(2023-2024 / OOS 369日)の検証で判明。**夜間先物が日経の日中を
予測しているように見えた相関は、`^N225` の始値の作り方によるアーティファクト。**

| OOS 369日 | 夜間リターンとの相関 |
|---|---:|
| **`^N225` 始値→終値** | **+0.364** |
| mini先物 09:00→引け | **+0.079** |
| 1321(ETF) 始値→終値 | +0.064 |

**取引できる2つは 0.06〜0.08 で一致し、`^N225` だけ 4〜5倍。**

#### 機構

日経225の「始値」は構成銘柄の寄り値から作るが、**09:00に寄っていない銘柄は
前日終値のまま**入る。だから `始値→終値` は overnight の一部を含み、
夜間先物と相関して当然。**始値では買えないので、この相関は取引できない。**

#### ★ 独立な裏付け(別期間・別実装で一致)

2026年8月の同じ20日を、目的変数だけ変えて測った:

| | 夜間リターンとの相関 |
|---|---:|
| `^N225` 始値→終値 (n=20) | +0.604 |
| **mini 0845→1545 (n=20)** | **+0.106** |
| **mini 09:00→引け (369日 OOS)** | **+0.079** |

n=20 の mini と 369日の mini が一致し、`^N225` だけ外れる。

#### ⛔ 波及: これを使っている既存の結論

| 場所 | 何 |
|---|---|
| `analyze_gap_edge.py:585` `_N225_ID` | **§18.60 ①「N損益 ← 日経の日中%」R²=0.235** |
| `analyze_market_beta.py:94` | **§18.30 lss の市場β R²=0.063** |
| — | **§18.61 / §18.65③ 先物ヘッジの β=−14,945円/日経+1% と天井 +14.5%** は上の R²=0.235 から出ている |

⚠ ヘッジの対象は先物なので、**β は先物の日中リターンに対して測るべきだった**。
`^N225` で測って先物でヘッジすると比率がずれる。

★ **方向は安全**: ① が過大なら、ヘッジの天井も予測の天井(①×②)も**さらに低く**
なるので「ヘッジしない」「日経予測は届かない」は強化される。**数字だけ要注記。**

★ **機構は生きている**: 「N は相場が上がった日に負ける」は別経路で裏が取れている
(下記 §18.72)。消えたのは**大きさ**であって向きではない。

→ **今後、日経を目的変数にするときは mini先物 か 1321 を使う。`^N225` の始値を
   含む量を使わない。**

---

### 18.72 ⛔ 寄り前の先物で N を止める = 不採用 (2026-09-07 / 2年OOS)

24ヶ月・491営業日。TRAIN=2023年前半で条件固定 → OOS は 2023年後半124日 +
2024年245日。2026年8月は仮説作りに使ったので判定から除外(§18.71 の発見も同時)。

| ルール | 2023後半 | 2024 |
|---|---|---|
| 夜間上昇日に停止 | 改善 | **大幅悪化** |
| 夜間上昇 上位10% | **+115,148円** | **−83,351円** |
| 夜間レンジ大 | — | **一律縮小にも負ける** |
| 4変数モデル | — | mini・1321・N損益の OOS R² **すべて不合格** |

**両年で符号が逆 = §18.36 判定ルール2 で不合格。**

→ **追加の先物データ購入は止める**(10年 26,400円 には進まない)。
   §18.65 の「大負け対策の入口5つは全部閉じた。残るのはサイズ調整だけ」は変わらない。

#### ★ ただし経路の発見は残る (2026-09-07 / 2026年8月 n=20)

§18.60 の分解「先物 → 日経の日中% → N損益」は**成立していなかった**:
...
| | r | 95%CI |
|---|---:|---|
| **夜間リターン → 当日の合格件数** | **+0.846** | [+0.64, +0.94] |
| 当日の合格件数 → N損益 | −0.446 | [−0.74, −0.00] |
| 夜間リターン → 合格銘柄の平均ギャップ | +0.040 | ゼロをまたぐ |

**夜間に上がる → ギャップアップ銘柄が増える → 合格が増える → 負ける。**
§18.70 ⑥ の「候補が多い日 = N が負ける日」と同じものを別角度から見ている。

⚠ **それでも2年OOSでは止められなかった。** 相関があることと、それで日を選べる
ことは別。§18.58① で「前夜の候補数」が TRAIN/TEST で反転したのと同じ結末。

#### ツール

`analyze_preopen_futures.py`(2026-09-07 追加)。`--explore` は TRAIN だけ・
TEST を画面にも出さない / `--confirm` は TEST を1回だけ。帰無較正は損益の
巡回シフトで**最良分位を選ぶ操作込み**。合成データで両方向(力がある/純ノイズ)を
検証済み。`FUTURES_FREEZE.md` に事前基準。

#### 次の優先順位

1. **実約定の検証**(§18.66 のゲート。20/30件・5営業日)
2. watch100/150 の追加枠の滑り測定
   ⚠ ただし**滑りだけでは足りない**。§18.70 ④ で watch150 はレポート(ギャップ降順)と
   ライブ(寄った順)が t=−2.24 で systematic にズレると分かっている。**発注順の差も
   同じ比較に入れること。**

---

### 18.73 ⛔ 過去高値を「抵抗線」として使う案 = 不採用 (2026-09-08 / TEST未使用)

仮説は「過去60/120日高値の直下でギャップアップした銘柄は、高値が抵抗線になり
日中に反落しやすい」。TRAIN の `--explore` で `res60_bp` / `res120_bp` の Q3 が
候補になったが、**分位の数値境界を表示すると機構が成立していなかった**。

| res120_bp | 実際の境界 | bp/件 | 解釈 |
|---|---:|---:|---|
| Q1 | −2,108.7〜+252.0bp | +9.4 | 高値を突破済み、または高値の2.5%以内。仮説の本命だがプラス側の最下位 |
| Q2 | +252.1〜+1,091.3bp | +19.8 | — |
| Q3 | +1,091.5〜+2,068.4bp | +27.9 | 探索上の候補。ただし高値は11〜21%上で、当日抵抗線にはならない |
| Q4 | +2,069.0〜+3,541.7bp | +21.3 | — |
| Q5 | +3,542.0〜+21,708.9bp | −2.6 | 高値が遠すぎる。ほぼゼロ |

`res120_bp:Q3` は t=+4.25 だったが、21軸の最良分位を選ぶ操作込みの帰無95%点を
**+1.7bpしか超えない**。`res60_bp:Q3` も超過は+0.6bp。60日と120日は同じ
仮説族であり、独立した2候補とは数えない。

仮説を直接表す `nearhi120_bp`（高値への近さ）は +25.3bp に対し帰無95%点
+26.2bpで帯の中。**高値の直下ほど良い形は出なかった。** よって Q3 の山型を
「抵抗線が効いた」と事後解釈せず、`--confirm res120_bp:Q3` は実行しない。
TEST は1回も使用していない。

#### 前夜の watch50 選定にも使えない

| 前夜の順位軸 | 取引数 | 円/件 | ランダム帯 z |
|---|---:|---:|---:|
| **liq（現行・売買代金降順）** | **9,627** | **+354円** | **+4.05** |
| nearhi120_bp | 8,961 | +257円 | −1.98 |
| nearhi60_bp | 8,897 | +271円 | −1.43 |
| res120_pc_bp | 10,307 | +280円 | +1.18 |
| touch120_pc | 8,634 | +267円 | −1.99 |

抵抗線系はすべてランダム帯の外（良い側）に出ず、現行 `liq` は取引数・円/件とも
上回り、前半/後半も同符号。**watch50 は売買代金降順のまま固定する。**

#### 作法

分位名だけで機構を説明しない。**候補が出たら実数境界を必ず表示し、当日の値幅で
到達しうる距離か確認する。** 数字上の山型が仮説と似ていても、説明変数の実距離が
機構と矛盾するなら TEST を消費しない。

---

### 18.74 ★ 鏡像のシャドー記録 = `.\mshadow` (2026-09-12 開始)

> ⛔ **利益判定には使えない → §18.75。** 併用は未使用窓の事前基準で不合格(t=1.89)、
> 60日では必要な効果の1/6しか観測できない。**続ける理由は「N2 が §18.66 のゲートで
> 落ちたときの受け皿として、鏡像の執行コストを先に知っておく」だけ。**

**鏡像(前日下げ × ギャップダウンを買う)を、N と同じ形で前向きに貯める。⛔ 発注しない。**

#### なぜ別プロセスなのか

| | |
|---|---|
| `k_open_confirm.py` は **N2 で凍結中** | §18.66 のゲート(20件かつ約定のある8営業日)を数えている最中。触ると版が変わり数え直し |
| kabu の有効トークンは1つ | `.\nexec` と同時に叩くと 401 で取り合う |
| **09:10 以降に回す** | `.\nexec` が終わってから。**始値は寄れば動かない**ので遅れても判定は正しい(§18.44)。速度が要るのは実発注のときだけで、これは発注しない |

#### 手順 (1日2コマンド。**N の手順は1つも変わらない**)

```
09:10 以降   .\mshadow          ← .\nexec が終わってから
15:40 以降   .\mshadow close
```
...
| | |
|---|---|
| 候補 | `n_paper.py --collect --mirror --watch 0` を**呼ぶ**(定義を写さない / §18.48 ⑧d)→ `m_signals_<日付>.csv` |
| 条件 | 前日リターン **≤ −1.753%** / 価格帯 1,000〜6,000円 / 売買代金順 |
| 板 | 50件バッチで**ローテーション**。既定は上位150件まで(建てられるのは上位50件だが、**壁の外側も残さないと『広げたらどうなるか』を後から測れない**) |
| 合格 | 始値が前日終値 **−100bp 以下**(N の符号を反転しただけ。上限ガードは無い / §18.55) |
| 出力 | `m_paper_<日付>.csv` = **k_paper 互換の列**。採点は `n_paper.py --close --seq-sides m` に任せる(side=−1 の採点は :846 に既にある) |

⛔ **`n_signals_<日付>.csv` / `k_paper_<日付>.csv` を1バイトも触らない。** N の記録はそのまま。

#### ⛔ 発注しないことを コードで確かめている

起動のたびに **自分のソースを AST で走査**し、`send_*` / `place_` / `cancel_order` の
参照が1つでもあれば kabu に触る前に終了する。`KabuClient(dry_run=True)` と二重。
検算で、発注行を1行足したコピーが実際に起動を拒むことを確認済み。

#### ⛔ 実装で踏みかけた穴

`unregister_many` は**失敗したら `_registered` を消さない**(fail-safe / `kabu_api.py:309`
の 2026-09-07 修正)。バッチを回すときに自前で `_registered.discard` すると、
**サーバに残っているのに「消えた」と思い込んで総数が50を超え、次の register が 400 で
丸ごと落ちる** — まさに直したばかりの fail-open を再導入することになる。
→ **解除に失敗したらそこで読み取りを打ち切る**。

#### ⚠ これは何ではないか

**鏡像に乗り換える決定ではない。** N の実約定ゲート(§18.66)は据え置きで、
`.\nexec` の中身は1行も変わっていない。鏡像は TRAIN で N と同水準(月+47,839 vs
+49,587 / §18.55)なのに **一度も前向きに記録していない**ので、その空白を埋めるだけ。

⚠ 判定の基準は、記録が貯まる前に宣言すること(§18.66 と同じ作法)。
**20〜40営業日**貯めてから見る。条件は3〜5個に固定して増やさない。

---

### 18.75 ⛔⛔ N と鏡像の併用 = **事前基準で不合格** (2026-09-12 確定)。掘り直す前にここを読む

**点推定は3窓すべてで改善するが、未使用窓の事前基準を通らなかった。本番採用しない。N2 は継続。**

#### 何を測ったか

**各側で独立に watch50 + 予算を持つ**形(両側を同じ50枠で競わせる §18.58① の旧方式とは別仕様)。
総予算400万を N/鏡像に固定配分し、同じ日・同じ銘柄集合の**対応日次差**で比較。

| 窓 | 配分 | 月平均 | 月平均÷σ | Nとの差/日 | 対応t |
|---|---|---:|---:|---:|---:|
| **2007-06〜2015-02**(1,896日) | N400万のみ | +48,239 | 0.479 | — | — |
| **★ 判定** | **200/200** | **+68,496** | **0.606** | **+994円** | **+1.89** ⛔ |
| 同 (感度分析のみ) | 300/100 | +61,998 | 0.585 | +675円 | +2.06 |
| 同 | 100/300 | +57,158 | 0.498 | +437円 | +0.62 |
| 同 | 鏡像400万のみ | +32,367 | **0.278** | −779円 | −0.82 |
| 2015-03〜2020-08(1,356日) | N400万のみ | +52,709 | 0.461 | — | — |
| 同 | 200/200 | +70,693 | 0.767 | +875円 | +1.42 |
| 2026年8月(20日・歩み値) | Nのみ | +224,450 | 1.240 | — | — |
| 同 | 50/50予約 | +359,500 | 1.573 | +6,752円 | (CIゼロまたぎ) |

#### 事前基準 (結果を見る前に凍結 / `N_MIRROR_EARLY_WINDOW_FREEZE_20260912.md`)

主候補 **200/200** について5条件を全部満たすこと:

| # | 条件 | 結果 |
|---|---|---|
| 1 | 対応日次差の平均が正で **t ≥ 2.0** | **+994円 / t=1.89** ⛔ |
| 2 | 前半・後半とも差が正 | +1,518 / +469 ✅ |
| 3 | 月平均÷σ が N単独より高い | 0.606 > 0.479 ✅ |
| 4 | 日次CVaR10% が悪化しない | −40,011 > −41,727 ✅ |

**1つ落ちたので不合格。** 300/100 は名目 t=2.06 だが、凍結文書で「感度分析だけに使い最良配分を選び直さない」と
宣言済みなので**採らない**。★ **結果を見た後に主候補を差し替えないこと。これがこの検証で最も重要な部分。**

#### ★ 私(Claude)の追加観察 3件

**① 窓は 11% 足りなかった。ただし伸ばす理由にはしない**

```
日次差 sd = 22,917円/日 / 効果 +994円/日
t=2 に必要 = 2,128営業日 / 実測 1,896日 = 89% (不足 232日 ≒ 0.9年) / 両側p ≈ 0.059
```

⛔ **t=1.89 を見た後で窓を足すのはゴールポスト移動**(§18.64 で直したのと同じ形)。**不合格でよい。**

**② ★★ 効果は高ボラ期に集中している (3窓で一貫)**

| 2007-06〜2015-02 の 200/200 | 差/日 | t |
|---|---:|---:|
| 前半(リーマン含む) | **+1,518円** | **+2.04** |
| 後半(2011〜2015-02) | +469円 | +0.63 |

2015-2020 も 前半0.996 / 後半0.544、2026年8月(高ボラ月)は単月 ÷σ 1.573。
**荒れている期間ほど効く。** 機構: 両側に候補が出る = 相場が両方向に飛んでいる日で、
平時は片側しか出ないので分散にならない。→ **「常時併用」の根拠は弱く、平時は N だけでよい。**
...
|---|---:|---:|
| CVaR10% の改善 | +24.7% | **+4.1%** |
| 鏡像単独 ÷σ | 1.421(N より上) | **0.278**(N より下) |

鏡像単独は 0.278 → 0.412 → 1.421 と **一貫して N より弱い**。8月だけ逆に見えたのは
8/31 の +141,700(全体の53.7%)。§18.56④ の「1件あたりは鏡像が上(+402 vs +345)」は
長い窓では支持されない。

#### ⛔ 60日シャドーは利益判定にならない

```
日次差 sd ≒ 22,900円/日 → 60日で t=2 に必要な効果 = 約 +5,900円/日
観測された効果 = +875〜994円/日 → 約6倍 足りない
真の効果を t=2 で検出するには 2,128営業日 ≒ 8.5年
```

**60日の前半30日/後半30日で採否を出す設計は撤回。** 実施するなら
**鏡像の約定可能性・滑り・100銘柄観測の遅延を測る技術試験に限定**する。

#### 併せて否定されたもの

| 案 | 結果 |
|---|---|
| **朝の合格件数が多い側へ切り替える** | ⛔ 相関 **−0.060** / 多い側が勝った日 9/20。5秒・30秒・60秒どれも固定配分に劣る。尾部が良く見えたのは「待ったこと」と「N寄り配分」の効果で、**合格数に追加情報は無い** |
| **現行の50枠を 25/25 に割る** | ⛔ N のみ(+224,450)より悪い(+182,400 / ÷σ 0.989)。固定分散が成立する前提は **N50件と鏡像50件を別々に観測できること** |
| 前日情報による切替 384規則 | ⛔ 帰無200本で p=0.295 (既存) |

#### ✅ 副産物: N単独値の 6% 食い違いの原因が判明

§18.55③ の +49,587円/月 と今回の +52,709円/月 の差は、**`--days 4200` がデータ終端からの
ローリング日数**で、実際の開始が 2015-03-16 だったため(2015-02 側が落ちる)。月数も
67ヶ月固定ではなく実在66ヶ月。→ **窓を固定したいときは `--days` ではなく開始日を明示する。**
§18.53 の「`fetch` に `min_start_date` を渡していなかった」と同じ系統の罠。

#### 位置づけの変更

| | |
|---|---|
| `.\mshadow`(§18.74) | **利益判定には使わない。** 続ける理由は「N2 が §18.66 のゲートで落ちたときの受け皿として、鏡像の執行コストを先に知っておく」だけ |
| 利益面の探索 | **終了。** 新しい未使用窓が自然に貯まるまで再開しない |
| N2 / 本番コード / 発注経路 | **1行も変更していない** |

#### 掘り直したくなったら

**まずこの節を読むこと。** 「バックテストでは上がる」は3窓とも本当だが、
**事前基準を凍結して未使用窓で測ると t=1.89 で落ちる**。同じ結論に到達するのに
歩み値9,820万行と2窓の再計算を要した。

再検証するなら条件は1つ: **新しい未使用窓が 2,128営業日ぶん貯まってから**。
2026-09 から数えて約8.5年後。それ以外の再探索は、窓を使い回しているだけになる。

---

### 18.76 ★ 取消締切 = **09:10 を維持。09:15/09:30 には延ばさない** (2026-09-15)

**N2 の「全銘柄が寄ったらループを抜ける」は実装ミス**で、注文の有効時間が
『その日いちばん遅く寄った watch 銘柄の時刻』で決まっていた(実例 2026-09-15:
09:02:59 に最後の1件が寄る → 09:03:10 に未約定4件を取消)。**--poll-until 09:10 は
一度も効いていなかった。** N3 で修正。

> ★ 原因を一言で: **始値を集め終わる条件**と**注文の戻りを待つ条件**を同じ
> `len(_seen) >= len(_syms)` で束ねていた。このループは元々 観測(k_paper)用で、
> 後から発注を足したときに終了条件を見直さなかった。
> **役割が2つになったら終了条件も2つ**にすること。

⚠ N3 の意味は「**発注を10分待つ**」ではなく「**出した注文を 09:10 まで残す**」。
条件を満たした銘柄は寄ったら順次発注する。

#### 実測 (2026-08 歩み値 9,820万行 / 20営業日 / 寄り1秒後に注文有効 / 100株 / 予算400万)
...
- **09:05 と 09:10 の優劣は決まらない**(差 −300円)。**09:10 を維持**する
- **09:15 / 09:30 には延ばさない**。悪化の主因は **追加銘柄側**
- 2年・482営業日の分足でも延長の安定した利益は確認できず

#### ★★ 合計 −300円 の内訳が ±9,000円で相殺していた

| 09:05 → 09:10 | |
|---|---:|
| ① 同じ注文を長く残す効果 | **+9,100円** |
| ② 遅く寄る銘柄を新たに発注できる効果 | **−9,400円** |
| 合計 | −300円 |

**合計だけ見ると「どちらでも同じ」で終わる。** 締切の効果は必ず①②に分けること。

★ ①②は厳密に足し算になる。予算は**発注時**に消費し、発注順は寄り時刻の昇順
なので、締切を伸ばして増える銘柄は**末尾に足されるだけ**で予算の押し出しが
起きない(短い締切の集合 ⊂ 長い締切の集合の前半)。

#### ☐ 出てきた仮説(未検証) — 受付と取消を別の時刻にする

①がプラス・②がマイナスなら **「受付は早く締め、注文は長く残す」**(受付09:05 /
取消09:10)が上のはず。`analyze_fill_1m.py` に **受付締切 × 取消締切のグリッド**を
入れたので直接読める。

⛔ **同じ20営業日から拾った仮説なので、別月で再現するまで採用しない**(§18.28)。
差 ±9,000円 は合計 +145,000円 に対して小さく、20営業日ではノイズと区別できない。

#### ⛔ 測り方でやった間違い2件（私(Claude)の実装。どちらもレビューで発覚）

| # | |
|---|---|
| 1 | **2つの効果を混ぜていた。** 「予算は発注時に消費するので締切を変えても注文する銘柄は同じ」と書いたが誤り。09:07 に寄る銘柄は 09:03 締切では**ポーリングが終わっていて注文できない**。全銘柄を注文済みとして扱い、短い締切を過大評価していた |
| 2 | **タッチを確実約定扱いしていた。** `high >= 指値` で約定にしていたが、指値ちょうどの約定は**時間優先(板の行列)**なので自分が刺さったとは限らない。指値より**厳密に上**で約定が出たときだけ価格優先で確実 → 下限/上限の2本立てにした |

#### ⚠ 1分足では最適時刻を確定できない

**寄りの1分の中が判別できない**(板寄せの約定=始値そのものが必ず入るので、
寄りバーの高値は定義上いつも指値以上)。上の表も実約定ではなく、同値タッチを
未確定として残したモデル。**確定には歩み値が要る**。
...
```
- 締切スイープ / 限界(①②分解・下限上限) / **受付×取消グリッド** / TRAIN/TEST
- 検算: 区間の足し上げ == いちばん遅い締切までの約定(入れ子なので必ず一致)
- `--split` が無いと「この表で締切を選べません」と出して止まる

#### ⛔ 本番は変更していない

`.\nexec --go --budget 400` は N3・09:10 固定のまま。**締切の比較はオフラインだけ。**
⚠ 09:10 まで待つあいだにプロセスが落ちると **MOC が未設置の建玉**が残る(§18.46)。
`[MOC] N件すべて設置しました` を必ず確認し、出ずに終わったら
`python panic_close.py --execute --prod`。

---

### 18.77 ⛔ 週次信用残 = 選別軸にならない (2026-09-18 確定)。★ そして「検出力」を初めて数字にした

**Standard プランを契約して10年ぶんの週次信用残を取得し、Codex と私(Claude)が
別実装・別母集団で測って、通ったものがゼロだった。**

| 測定 | 誰が | 結果 |
|---|---|---|
| 単独6指標 + 交互作用12条件 | Codex | **0/18** |
| 買残日数 / 売残日数 / 純買残日数 | Claude(公表日を変えて2回) | **候補ゼロ** |
| 交差 80:20 | Claude | TRAIN t=+1.89 **不合格** |
| 交差(Codex の閾値 1.58日 / 0.087日) | Codex | TEST t=+1.78 **不合格** |
| 危険帯 225条件から2帯 | Codex | 円ベース t=0.62 / **最悪日 改善なし** |

選別軸の否定は **12回目**(§18.12 BTスコア / §18.13 15軸78検定 / §18.24 7軸 /
§18.31 流動性 / §18.38 建値 / §18.48⑧g 母集団を帯に / §18.48⑪ 銘柄・戦略 /
§18.52 シグナルそのもの / §18.64 業種 / §18.65④ β圧縮 / §18.68 銘柄)。

#### ★★ いちばんの収穫: 検出力(MDE)を出すようにした

```
選別軸の検出下限   10bp(3軸) / 30bp(交差)
N のグロス         +15.7bp/件 (2015-2020)
```

**「候補ゼロ」は『効果がない』ではなく『10bp 以上の効果は無い』が正確。**
そして **選別軸が戦略そのもののエッジと同じ桁の識別力を持つ必要がある**という
構造が見えた。5bp の効果はありうるが、この窓では原理的に見えないし、
実運用の上積みは月2,000円程度。**選別軸を掘る作業自体の期待値が低い**ことを、
初めて数字で言えた。

→ **今後 「候補ゼロ」を報告するときは必ず MDE を併記する。**

#### ★★ 交差条件は件数が 1/4.5・MDE が 2.5倍になる

合成データ(効果あり -60 / 22,064件 / 876営業日)で実測:

| | 単独 sell_days Q5 | **交差 80:20** |
|---|---:|---:|
| 件数 | 1,897件(20%) | **422件(4.5%)** / 0.19件/日 |
| MDE | 9bp | **23bp** |
| **効果ありの合成データ** | t **-5.34** ✅検出 | t **+1.12** ⛔ **検出できず** |
| 純ノイズ | 候補ゼロ ✅ | 不合格 ✅ |

**効果が実在しても交差では見えない。** 前半/後半の符号も両ケースで反転した
(422件を半期で割ると190日に200件。安定するはずがない)。
→ **条件を増やすほど見えなくなる。3条件なら検出下限は50bp近い。**

⛔ ただし「交差」の話。**スコア化(足し算)なら件数は減らない**ので別問題
(未測定。ただし足す材料の個別が全部ヌルなので期待は薄い)。

#### ★ 除外は N では構造的に不利 (§18.64 の再確認)

```
稼働率 40%前後  →  除外して空いた枠を埋める代わりがいない
                →  除外の利得は「抜いた集団の損益」そのもの
                →  抜いた集団が TEST で -0.80bp/件 = ほぼゼロ
                →  抜いても得しない
```

Codex の言い方が的確: **「他より悪い」≠「絶対に損している」**。
相対評価があっても、絶対値がマイナスでなければ除外利益にならない。

そして **bp と円で t が食い違う**のがこの構造の指紋:
```
調整後差(bp)      t = -3.40   ← 危険帯の銘柄が悪いことは示す
日次円損益の改善  t = 0.62    ← 予算制約下では取れる額が増えていない
```

#### ★★ ランダム帯で確かめる — 3回目の実証

危険帯のσ削減は本物に見えたが、**同数をランダムに落とすのとほぼ同じ**だった:
...
CVaR もランダム帯の95%点に届かず。**σが減ったのは「件数を減らしたから」。**

★★ そして決定的: **最悪日 -146,350円 は1円も改善せず、その日には危険帯銘柄が
0件だった**。保険が発動しなかったのではなく **対象が存在しなかった**。

これで「最悪日が動かない」は **3回目**(§18.64⑤ 同業種上限 / §18.65④ β圧縮 /
本節)。**裾を削る提案は、まず最悪日に対象が何件いるかを見ること。**

#### 帰無の中央は 0 ではない

```
TRAIN 全体平均    +6.6bp
帰無の中央        +13.6bp   ← 「該当銘柄がある日」というだけで +7.0bp
実測              +43.1bp
帰無の95%点       +36.1bp
```

「最悪/最良の分位を選ぶ」操作込みで較正すると中央がずれる。**0 と比べてはいけない。**
シャッフルは **同じ日の中だけ**(日をまたぐと日内相関が壊れて帰無分布が狭くなり
偽陽性を過小評価する)。

#### 公表日を正確にしたら効果が消えた(sell_days)

| | +7暦日 | **+3営業日(公表の翌営業日)** |
|---|---|---|
| 最悪 Q4 | -8.7bp / t **-2.42** ②通過 | **-3.0bp / t -1.49** ⛔ |
| 前半 / 後半 | -4.1 / -13.9 ✅一致 | **+1.5 / -7.9** ⛔ 反転 |

**より新しい残高にしたら弱くなった** = 前回の t=-2.42 はノイズ。
★ 逆に交差条件は t が +1.88 → +1.89 と**まったく動かなかった**。
= 測っているのは「今週の需給」ではなく **銘柄の恒常的な性質**。
そして銘柄の固有値に持続性がないことは §18.68 で確定済み(順位相関 -0.007)。

#### 実装で踏んだバグ3件 — 全部「手元では動く」形

| # | 内容 |
|---|---|
| 1 | `groupby(...).index.values` を numpy の**位置**として使った。あれは**元の index ラベル**。実データ(TRAIN/TEST が混ざり index が飛ぶ)で IndexError。⚠ **範囲内なら エラーも出ずに静かに間違う** |
| 2 | NaN を落としたあと `date` を**先頭から**切った。NaN が途中にあると日付と残差の対応がズレる |
| 3 | `np.array(sorted(Series.unique()))` が **object dtype** を作る。`searchsorted` が Timestamp と int を比べて TypeError。さらに pandas 3.0 では `pd.to_datetime(ndarray)` が `DatetimeArray` を返し `.values` が無い |

★ **合成データのテストが甘かった**: TRAIN が日付順で先頭に固まっていて偶然
index が 0..n-1 だった。→ テストデータに **index が飛び飛び + NaN が途中**の
ケースを足した。**dtype は暗黙に任せず、明示して assert を置く。**

#### ⛔ 分割の検算は「できない」と分かった(2案とも失敗)

| 案 | なぜ駄目か |
|---|---|
| 結合後の買残日数が隣接レコード間で跳ぶ割合 | 実データで **25.2%**。N は前日+1.75%以上の急騰銘柄を拾うので **adv20 が跳ねる**。そもそも隣接レコードが連続する週とは限らない(取引は散発的) |
| 連続する週の買残の比が整数比に集中する割合 | 8銘柄に分割を混ぜても 156,000組中 175→176件で**埋もれる**。銘柄単位にすると分割なしでも **59銘柄が誤検出** |

★ 区別するには「信用残(未調整)と出来高(調整済)の時系列が食い違う」ことを見る
必要があるが、picks は取引日の点しか持っていない。
→ **実害の測り方に切り替えた**: 極端値が上位20%分位の何%を占めるか。
  実データでは 買残日数>50日 が12件(上位分位の0.3%) = **実害は小さい**。

⚠ なお **Codex は信用残側も分割調整していて、そちらが正しい**
(「残高日当日の分割係数で同じ株数単位へ変換」)。私は未調整のまま使った。
中央値が 買残 0.66 vs 0.50 / 売残 0.24 vs 0.12 と 1.3〜2倍ずれたのはこれと
母集団の差(941銘柄 vs 1,286銘柄 / 結合率 98.7% vs 87.4%)。
**それでも両方とも不合格**だったので、結論は頑健。

#### ⛔⛔ 貸借区分(IssueType) — 初版の記述は **誤り** だった (同日中に訂正)

**私(Claude)が §18.77 の初版に書いた「信用銘柄 = N で売れない = バックテストが
過大」は、3点とも誤り。** 指摘を受けて訂正する。

##### 誤り① 「信用銘柄は N で売れない」→ **売れる**

`IssueType`(1=信用銘柄 / 2=貸借銘柄 / 3=その他)は **制度信用の区分**。
⛔⛔ **N は制度信用で建てていない。** kabu の**一般信用デイトレ**
(`CashMargin=2` / `MarginTradeType=3`)なので、**信用銘柄でも証券会社に在庫が
あれば空売りできる**。自分の発注経路を読み違えた。

★ 実際に建てられるかを決めるのは、優先順に:
```
① 実注文の応答(エラーコード 100302 など)   ← いちばん強い
② kabu /symbol の MarginSell
③ J-Quants の IssueType                     ← いちばん弱い
```
**`IssueType` だけで信用銘柄をバックテストから除外してはいけない。**

##### 誤り② 「-9,400円だから過大」→ **符号が逆**

2026-09-18 に売建規制で建てられなかった3件のペーパーは **-9,400円**だった。
つまり **建てられなくて助かった**側。バックテストが過大になるのは
**売れなかった銘柄のペーパーが プラス** のとき。

```
売れなかった銘柄のペーパーが +  →  バックテストは過大
売れなかった銘柄のペーパーが -  →  むしろバックテストが悪く見えている
```

09-18 当日の報告では正しく「+9,400円の利益」と書いたのに、記録に落とすときに
逆にした。**符号は記録のたびに確認する。**

##### 誤り③ 「ツールで出る」→ 出なかった

内訳の実装自体は入っていたが、**ラベルが誤り**(「制度の売建 不可」)で、
「除外した後が本当の水準」という誤った解釈も表示していた。訂正済み。

##### ★ 現象そのものは実在する(集計済み)

| 期間 | 信用銘柄 | 貸借銘柄 |
|---|---:|---:|
| TRAIN | 1,023件 **+29.49bp/件** | 5,917件 +2.94bp/件 |
| TEST | 920件 **+33.80bp/件** | 10,282件 +13.14bp/件 |

**差は TRAIN/TEST 両方で再現する。** ただし上記のとおり「建てられない」ことの
証拠ではないので、**除外の根拠にはならない**。

##### ★★ 正しい測り方 = 実際の売建可否と突き合わせる

| 貸借区分 | 実際の売建可否 | ペーパー損益 | 実約定 |
|---|---|---|---|
| 信用銘柄 | 売建可能 | 集計 | 集計 |
| **信用銘柄** | **売建不可** | **集計** | **0** |
| 貸借銘柄 | 売建可能 | 集計 | 集計 |
...
これで「**バックテストの利益の何円が、本当に建てられない銘柄に依存していたか**」
が分かる。⚠ 「実際の売建可否」は過去データに無いので、**今日から前向きに貯める**
(`ordered_signals_n.csv` のエラーコード + 毎朝の `/symbol` の `MarginSell` を
`k_paper_<日付>.csv` のペーパー損益に結合する)。

**★ 貸借区分を掘るより、この結合のほうが実現可能エッジの測定として短い経路。**

#### 使わないと決めたもの / 残す使い道

| ⛔ 使わない | ✅ 残す |
|---|---|
| 信用残による N の発注除外 | 無料のシャドー記録(将来の独立期間) |
| 信用残による日全体の停止 | 貸株・空売り規制・執行失敗の警戒情報 |
| 信用残を理由とする増額・縮小 | N とは別の「信用需給を直接狙う新戦略」の材料 |
| 大負け防止装置としての採用 | 個別銘柄の集中リスクの診断 |

⚠ シャドー記録のコストは低いので取ってよいが、**決着は期待しないこと**。
危険帯の円ベース t=0.62 → t=2 に必要なサンプルは (2/0.62)² = **10.4倍**で、
TEST が1,387営業日なので **1万営業日以上**かかる。
...
python analyze_margin_days.py --picks n_picks.csv --margin margin_interest_n_10y.csv --publish-bd 3
python analyze_margin_days.py --picks ... --margin ... --publish-bd 3 --confirm-cross 80:20
```

判定条件はコードに焼き込んであり実行時に変更できない。TEST は `--confirm` /
`--confirm-cross` のときだけ開く。`--dump-picks` に `adv20`(D時点までの20日平均
出来高・株数)を追加した(`liq` では代用できない: mean(c*v) != mean(c)*mean(v))。

#### ★ この節から持ち帰る作法

1. **「候補ゼロ」には必ず MDE を併記する。**『効果がない』と『見えない』は別。
2. **条件を交差させる前に件数を数える。** 1/4.5 になり検出下限が2.5倍になる。
3. **除外案は「抜いた集団の絶対値」で判断する。** 相対的に悪いだけでは足りない。
4. **bp と円で t が食い違ったら、円が正。** 予算制約下で取れる額が本当の効果。
5. **裾を削る提案は、まず最悪日に対象が何件いるか見る。** 3回とも0件か不変だった。
6. **σ削減は必ずランダムに同数落とした帯と比べる。** 件数を減らせば必ず減る。
7. **dtype と index は暗黙に任せず、明示して assert を置く。**

---

### 18.78 ⛔ 業種ギャップの分解 = 不合格 (2026-09-19)。**選別軸の否定 13回目**

**「その銘柄の業種が、その朝どれだけ動いたか」で N の銘柄を選べるか。**
これまでの12回と違い **日内で識別できる**初めての軸だったが、事前に凍結した
基準を通らなかった。⛔ `--confirm` は実行していない。**TEST は未使用のまま。**

```
python analyze_gap_edge.py --days 4200 --min-gap-bp 100 --split 2020-09-01 \
    --min-ret1 1.753 --min-price 1000 --max-price 6000 --dump-picks n_picks.csv
python analyze_gap_edge.py ... --dump-universe-gaps n_universe.csv
python analyze_sector_residual.py --picks n_picks.csv --universe n_universe.csv
```

#### 結果 (TRAIN 2015-03〜2020-08 / 9,403件 / 1,217営業日)

| 軸(gap_bp + ret1 を日内で統制) | 端の分位 | 帰無95% | 日クラスタ t | 前半/後半 |
|---|---:|---:|---:|---|
| sector_gap | Q1 **+5.9bp** | 12.2bp ⛔ | **-1.73** ⛔ | +14.8 / -0.9 ⛔反転 |
| resid_gap | Q5 **+6.3bp** | 12.4bp ⛔ | **+1.73** ⛔ | +9.5 / +3.8 ✅ |

**①帰無超え ⛔ / ②\|t\|>=2.0 ⛔** で不合格。③だけ通った。

#### ★ 設計としては3つとも狙いどおり動いた(ここは残す)

1. **予想した符号が出た**(回す前に docstring に書いてある):
   `sector_gap` は β **-0.1669**(続伸して不利) / `resid_gap` は β **+0.1669**(反落して有利)。
   統制後は定義上 `resid_gap ≡ -sector_gap` なので、**2軸に見えて実質1検定**。
2. **検出力が上がった**。日単位の軸は MDE **10.0bp**(§18.77)、本ツールは **5.7bp**。
   日固定効果を吸収した効果で、**§18.77 の壁が約1.8倍ゆるくなった**のは確か。
3. ★★ **統制が仕事をした**。生の `resid_gap` は **日クラスタ t = +2.57** で、
   統制しなければ ② を通っていた。gap_bp と ret1 を抜くと **+1.73** に落ちる。
   = **生の信号の大半は「その銘柄自身のギャップ」**だった。
   Codex が別窓(2020-2026)で「銘柄自身の前日上昇率・ギャップ・流動性を揃えると
   差が消えた」としたのと **同じ現象が別窓で再現**した。

#### 期間で安定しない (4区分で形が揃わない)

| 窓 | resid_gap の効き |
|---|---|
| TRAIN 前半 (2015-2017) | +9.5bp |
| TRAIN 後半 (2018-2020) | +3.8bp |
| Codex 前半 (2020/9-2024/6) | **差なし**(上位20% +13.0 vs 下位20% +13.3) |
| Codex 後半 (2024/7-2026/9) | +42.5 vs +14.6 だが**統制すると消えた** |

#### 円に直すと(参考。**採用しない**)

| | bp | 月あたり |
|---|---:|---:|
| 点推定どおり Q5 に寄せられたら | +6.3 | **+19,478円** |
| **検出力の下限(MDE)** | **5.7** | **+17,623円** |

**効果の点推定が検出限界とほぼ同じ**。つまり「1.8万円/月より小さい効果は
見えない道具で、1.9万円/月の点推定が出た」という状態。⛔ 区別できない。

#### ⚠ そもそも TRAIN が既見だった

`--min-ret1 1.753` は §18.54 の `--explore` が **この窓で**見つけた閾値。
ツール自身が `⚠ 既見(閾値の出所 / in-sample)` と出す。
**in-sample でも通らなかった**ので、OOS を試すまでもない。

#### ★ 結論

**この方向を閉じる。発注設定は1行も変えていない。**
選別軸の否定は 13回目(§18.12/13/24/31/38/48⑧g/48⑪/52/64/65④/68/77/本節)。

⚠ 例によって「効果がない」ではなく **「5.7bp より大きい効果は無い」**。
ただし 5.7bp 未満の効果を取りに行っても月2万円未満で、
しかもそれを確かめるには数年ぶんの新しいデータが要る。

#### ★★ 米国の業種ETF でも同じ結論 (Codex / 2026-09-19)

**私が測ったのは日本の業種のギャップ。Codex は「前夜の米国 業種別ETF」を
別に取得して測った。構成が完全に違うのに、同じところに着地した。**

| | 統制前 | **統制後(同日・ret1・ギャップ・流動性)** |
|---|---|---:|
| 前半 2020/9-2024/6 | 米ETFが弱い +13.9 vs 強い +8.1 bp/件 | **t = -0.66** |
| 後半 2024/7-2026/9 | 米ETFが弱い +21.4 vs 強い +10.2 bp/件 | **t = -1.00** |

22業種 / 約13,500件。SOX ↔ 電気機器 は前半だけ強く、**後半は符号が反転**。

⚠ **両群とも損益はプラス**。だから「米国の業種が強い銘柄を見送れば得」には
ならない(§18.64 の『除外は N では構造的に不利。稼働率40%で空いた枠を
埋める代わりがいない』と同じ)。

#### ★ 3つの独立な測定で **符号だけが3回とも揃った**

| 測定 | 窓 | 統制後 t |
|---|---|---:|
| Claude: 日本の業種ギャップ `sector_gap` | 2015-2020 | **-1.73** |
| Codex: 米国業種ETF | 2020/9-2024/6 | **-0.66** |
| Codex: 米国業種ETF | 2024/7-2026/9 | **-1.00** |

**3回とも「業種が強い = N に不利」の向き。だが |t| は 0.66〜1.73 で
一度も 2.0 に届かない。** 検出力は両方とも MDE 5bp前後
(Claude 9,403件で 5.7bp / Codex 13,500件で約4.8bp)。

→ **『存在するかもしれないが、5bp より小さい』が3回続けて出た**、が正確な状態。
   5bp 未満なら発注順に載せても月2万円未満で、確かめるには数年ぶんの
   新しいデータが要る。**業種という方向は、日本側も米国側も閉じる。**

⚠ 記録しておく2点(次に掘る人へ):
  ・**2020-2026 の窓は Codex が3回 探索に使った**(ret1の業種相対 / 信用残の
    危険帯 / 米国業種ETF)。ここを『未使用の検証窓』として扱えない。
  ・Codex は「米日の業種対応が曖昧な銘柄を主分析から外した」。
    除外を **結果を見る前に**決めたかどうかで意味が変わる。

#### 道具は残す(どれも他の軸に使い回せる)

| | |
|---|---|
| `analyze_sector_residual.py` | 日固定効果 + 日クラスタ頑健 SE + 日内シャッフルの帰無 + **統制の強制**。**銘柄単位で日内に動く軸**なら何でも測れる |
| `analyze_gap_edge --dump-universe-gaps` | (date, 業種) ごとのギャップの合計と件数。9万行 |
| `check_preopen_cutoff.py` | 寄り前変数が締切までに確定しているか(§18.79) |

★ **作法(今回 効いたもの)**
1. **符号を回す前に書く。** 逆に出たら「そういう解釈もある」と言わない
2. **単調変換を疑う。** 順位相関を毎回印字する(§18.77 の long_share)
3. **統制の前後を両方出す。** 生で効いて統制で消えるなら、それは別の変数
4. **MDE を必ず併記する。** 「候補ゼロ」は『効果なし』ではない
5. **合成データの対照実験は下地を1回だけ引く。** ケースごとに走らせると
   乱数列がずれて別のデータになる(2026-09-19 に実際そうなった)

---

### 18.79 ★ 決算フラグ = 準備完了・判定待ち (2026-09-22)。★★ 403 は契約不足ではなく期間超過

**§18.77 で「契約が無いので決算翌日は諦めた」と書いたのは誤りだった。**
唯一残っていた未検証の選別軸が、測れる状態に戻った。

#### ⛔⛔ 訂正: `statements`・`master` の 403 の原因

| | |
|---|---|
| §18.77 の記録 | 「`statements`・`master` が 403(**契約範囲外**)」 |
| **実際** | **Standard で取れる。403 は契約期間(2016-09-23以降)より前を要求したため** |

実機の `jquants_extra/` には **`master.csv` が現に存在する**。
API は取得可能期間を明示している。

★ **作法: 403 を見たら「契約が無い」と決めつける前に、要求した期間を確認する。**
  この1行が3週間ぶん「決算データは取れない」という誤判断を生んでいた。

#### ★ 測る前に「測れるか」が決まった (実データ)

`python analyze_earnings_flag.py --no-earnings`(picks 20,777件 / 建値中央 2,168円):

| 決算あり率 r | TRAIN 6,979件 | TEST 11,250件 |
|---:|---:|---:|
| **2%** | **576円/件 ⛔** | **454円/件 ⛔** |
| 5% | 370円 ⛔ | 292円 ⚠ |
| 10% | 269円 ⚠ | 212円 ⚠ |
...
**N のグロスは +345円/件。MDE がこれを超えると「ゼロかマイナスか」を区別できない。**

⛔ **「当日(D+1)が決算発表予定日」は測れない。** 年4回/244営業日 = 1.6%で、
  しかも **D の上昇とは無関係なので偏らない**(決算で上げた銘柄が翌日ギャップ
  アップするのは『D の決算』であって『D+1 の予定』ではない)。→ r≈2% 固定。
  **取得しても結論は出ないので、この分類は最初から外す。**

★ 優先は **`earn_prev1` = シグナル日 D に決算があったか**。決算で上げた銘柄が
  N に入りやすいので r が高いはず。何倍になるかは数えるまで分からない。

#### 判定ツール `analyze_earnings_flag.py`

```
python analyze_earnings_flag.py              # picks も決算CSV も自動検出
python analyze_earnings_flag.py --no-earnings # 下見(検出限界)だけ
python analyze_earnings_flag.py --win TEST --confirm   # 候補が出たときだけ1回
python analyze_earnings_flag.py --selftest    # 合成データ(力あり/なし)+日付境界
```

**答える問いは1つだけ**(§18.64 の構造):

> 決算銘柄の **絶対円損益** がマイナスで、除外後の総円損益が
> **同数ランダム除外の帯** より改善するか

合格は3つ揃ったときだけ: ①抜く集団の絶対損益がマイナス ②z ≥ +2.0 ③|日t| ≥ 2.0。

⚠ **§18.55 との照合が検算になる**。強い材料 ≒ 大ギャップで、
  ギャップ上限150bp は月 +49,587 → **+21,075(43%)** に落ちる。
  決算銘柄の絶対損益がプラスなら整合。マイナスなら**どちらかが間違っている**。

#### ⛔ この日に踏んだ事故 — 同じ形を繰り返さないこと

**自動検出が `margin_interest_n_10y.csv`(週次信用残)を決算CSV として読み、
判定まで通した。** `Code` と `Date` の列があるので素通りする。
出た数字がそれを示していた:

```
earn_prev1 23.6%  <- 1/5 = 20%
earn_prev3 62.3%  <- 3/5 = 60%
earn_prev5 94.6%  <- 5営業日あれば週次はほぼ必ず当たる
```

決算(年4回)ではなく **週1回の公表カレンダー**の形。

★★ **「どのファイルを使ったか印字する」では防げなかった。**
  印字は『その名前を見て誤りに気づける』前提で、実際には気づけない。
  → **中身を検証する**。ツールに二重の検証を入れた:

  1. **列名のブラックリスト** — `ShortMarginTradeVolume` / `IssueType` /
     `CompanyName` / `TurnoverValue` などがあれば即拒否
  2. **1銘柄あたりの年間頻度** — 決算は年4〜8回。20回超で拒否
     (年52回=信用残 / 年244回=日足 と原因も出す)

  ⛔ 1だけでは足りない(列名が無害なこともある)。2だけでも足りない
    (合成テストを毎週ちがう銘柄で作ったら頻度が低く出て通った)。

#### ⛔ もう1つのバグ: `earn_prev{k}` が「建てる日」を含んでいた

```
D    = シグナル日。前日比 +1.75% 以上 上げた日
D+1  = 建てる日。picks の `date` はこちら

修正前  earn_prev1 = {D, D+1}   ⛔ D+1 の開示は場中かもしれない
修正後  earn_prev1 = {D}        ✅ 寄り前に確実に分かる
```

このままだと「**寄り前に分かるはずのフラグに、回避できない場中の材料が混ざる**」ので、
差が過大に出たうえ**それは発注時には使えない情報**という形の誤りになる。
`--selftest` に日付境界の回帰テストを8件入れた(バグ版に戻すと落ちることを確認済み)。

#### ⛔ 契約期間より前は「決算なし」ではなく **欠測**

実測で **2,548件(12%)**。ここを「決算なし」として混ぜると、その期間の取引が
丸ごと「材料なし」群に入り、**仮説が正しい場合に差が過大に出る**。既定で落とす。

#### ⛔ 60件のブラインドニュース調査 = 決算軸では不要

Codex が設計した「大損失30件 vs 条件の近い利益30件」は、マッチングは良い
(ギャップ 300 vs 284bp / 前日上昇率 8.08% vs 7.91% / 日付差中央値9日)が、
**検出力が足りない**。Fisher 両側で:

| 材料あり率の差 | n=30 の検出力 | 80%に必要な件数(片群) |
|---:|---:|---:|
| 10pt (95% vs 85%) | **26%** | 131 |
| 15pt (90% vs 75%) | **34%** | 99 |
| 20pt (90% vs 70%) | 50% | 57 |
| 30pt (90% vs 60%) | 80% | 31 |

**N は定義上「材料が出た銘柄」を拾う**(前日 +1.75% 以上・標本平均 8.08%)ので、
材料あり率は両群とも 85〜95% になるはず。現実的な差 5〜15pt での検出力は
**26〜34%** = 本当に差があっても7割は「差なし」と出る。9分類に割ればさらに落ちる。

★ **決算軸は全件自動集計(このツール)のほうが速く、漏れがなく、後知恵も入らない。**
  60件のブラインドは **格上げ・M&A・テーマ材料**(J-Quants の財務情報に入らない
  もの)の調査として残す価値がある。

#### ⚠ 事後の説明と事前の予測は別 (§18.60 の再掲)

負け日にニュースを探せば **75〜83% で理由が見つかる**(Codex の12日調査)。
だがそれは **N が「材料が出た銘柄」を拾う設計だから当然**で、
勝ち日でも同じ率で見つかるはず。そして市場全体については既に測ってある:

```
事後: N の損益 ← 日経の日中リターン   R² = 0.235   (説明はつく)
事前: 寄り前の19変数から予測          OOS R² = -0.064  (読めない)
```

**理由が分かることと、事前に回避できることは別。**

#### ⛔ 訂正: lss の事例を N の事例として使わない

2026-09-22 に §18.17 の **4911 資生堂(2026-08-07)** と **5632 三菱製鋼(2026-08-06)**
をニュース調査の実演に使ったが、**あれは lss(逆指値ショート・delay1 の無保護窓)の
損切り事例**で、現在の `n_picks.csv` に存在しない。N の改善根拠にはならない。

⚠ 資生堂の件は調査そのものも危うかった。1回目の検索で **1年ずれた記事(2025年8月)**
を根拠に判定しており、検証してようやく 2026-08-05 の決算(純利益3.1倍・
営業利益+131.8%)が正しいと確認できた。
★ **ニュース検索は日付を厳密に照合すること。** 日経の URL は `C25A8` = 2025年8月。

#### この節から持ち帰る作法

1. **403 を「契約が無い」と決めつけない。** 要求した期間を確認する
2. **ファイルは名前ではなく中身で検証する。** 印字だけでは防げない
3. **「寄り前に分かる」フラグは、日付の境界を回帰テストで固定する**
4. **検出力(MDE)を、測る前に出す。** 測れない分類はデータを取る前に外せる
5. **n=30 の比率比較は 25〜30pt の差がないと見えない。** 全件自動集計を優先する
