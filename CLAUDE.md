...
| `n225_research.py` | Phase 0 評価ハーネス (walk-forward + ベースライン + HAR-RV) |
| `n225_veto.py` | 日経日中上昇予測で N の発注を止める効果の検証 |
...
どちらのスクリプトも `--selftest` でネットワーク無しの配管テストができる
(シグナル無し合成データでエッジが出ないこと / 既知シグナルを検出できることを検査)。
評価コードを触ったら必ず両方の `--selftest` を通すこと。
