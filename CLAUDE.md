...
### 18.73 ★ 鏡像のシャドー記録 = `.\mshadow` (2026-09-12 開始)

> ⛔ **利益判定には使えない → §18.74。** 併用は未使用窓の事前基準で不合格(t=1.89)、
...
### 18.74 ⛔⛔ N と鏡像の併用 = **事前基準で不合格** (2026-09-12 確定)。掘り直す前にここを読む
...
| | |
|---|---|
| `.\mshadow`(§18.73) | **利益判定には使わない。** 続ける理由は「N2 が §18.66 のゲートで落ちたときの受け皿として、鏡像の執行コストを先に知っておく」だけ |
