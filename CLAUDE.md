...
**傾きは |ギャップ| の十分位ごとに推定します (2026-08 変更)。** 全体で1本の
傾きを推定して大きいギャップに線形外挿すると、控除しすぎます。ノイズが分散に
占める割合が高いのは小さいギャップの帯域で、12% のギャップではノイズは誤差の
数%にすぎないからです。§1 の下に帯域別の傾き表が出ます。

- **大きいギャップの帯域でも傾きが残っている** → 反転は本物。ノイズでは説明できない
- **大きいギャップで傾きが 0 に近づく** → 小さい帯域だけがノイズで、
  大きいギャップの α は別の要因

⛔ **この診断はデータ破損があると機能しません。** ギャップ 122万% のような行が
説明変数に入ると回帰の傾きが 0 に潰れ、控除が効かなくなります。§16.5.5 の
整合性チェックを必ず先に通してください (実測で傾きが -0.0000 → -0.0359 に変わり、
控除後の α が 57.9bp → 13.9bp に落ちた例があります)。
