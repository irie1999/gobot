...
### 16.9.05 ⛔ 「始値では建てられない」は言い過ぎだった (撤回)

§16.8 / §16.9 で「残差ギャップを知るには板寄せの約定価格が要るので原理的に
執行できない」と書きましたが、**これは誤りです**。実測 (1,121件):

- **対象銘柄の 91.7% (1,028件) は 09:00 に寄らない。中央値で12分遅れる**
- したがって寄る前に十数分の準備時間があり、その頃には指数の始値は確定済み
- 板寄せに注文を出して参加すれば、**始値そのもので約定できる**

判定を寄り前の気配で行う精度も足ります。この戦略の閾値は中央値 1,000bp で、
気配→始値の誤差 (実測 中央65bp / 95%点251bp) の 4〜15倍。251bp ずれても
閾値をまたぐのは 2件 (0.2%) だけでした。

**執行の道は塞がっていません。** ただし、その道を通しても成績は残りません
(次項)。
