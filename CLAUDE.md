...
(entry_drag = 0.03bp)」という結果が出ましたが、これは実装事故でした。
出力された trades.csv を検査したところ:

- `entry_price` が `official_open` と **完全一致: 1,117 / 1,121 行 (99.6%)**
- それにもかかわらず `entry_time` の中央値は **09:12** (09:00 ではない)
- `entry_time = 15:30` (引け) なのに `entry_price` が始値の行が **40件**
- `delay_minutes` の最大が **390分** (立会時間の全体)
- 一致しなかった 4 行の乖離は 中央値 17bp / 最大 68bp ← これが本来の桁

12分待って 0.03bp しか動かないことは、12% ギャップした銘柄では起こり得ません
(実測では **1分で 65bp** 動く)。分足の参照に失敗して公式始値へ
フォールバックしていたと考えられます。

**「劣化がほぼゼロ」という結果が出たら、まず執行価格が始値と一致していないかを
疑ってください。** `gap_reversal_intraday.py` は一致率が 5% を超えると警告を
出します。

なお、その検証は 2025-04 の1ヶ月に完全に依存してもいました
(128件で +206bp / t=8.48、この月を除くと -13.8bp / t=-1.28)。

### 16.9.05 ⛔ 未決着: 「寄りが遅れる」は実在か処理の産物か

「対象銘柄の 91.7% は 09:00 に寄らず中央値12分遅れるので、板寄せに注文を
出して参加できる」という主張が出ましたが、**根拠にした `delay_minutes` 列が
信用できません**。trades.csv を検査した結果:

- **「5分足」ファイルの `entry_time` の 69.6% が5分の倍数でない**
  (09:02 / 09:03 / 09:06 / 09:09 / 09:11 …)。一方 `exit_time` は 100% 5分
  グリッドに乗る。同一ファイル内で粒度が違う = entry は5分足由来ではない。
  1分足版と5分足版が22ヶ月まで一致したのも、同じ entry を使っていたため
- **|残差ギャップ| の5分位で `delay=0` が Q1:12件 / Q2〜Q4:厳密に0件 / Q5:81件**。
  中央 delay は単調増加なのに Q5 だけ二峰性。672件連続でゼロは確率的に
  起こらない = 別経路で処理された行が混ざっている
- corr(|ギャップ|, delay) = **-0.087** と負。特別気配が理由なら正のはず

一方で、遅延の一部は実在するようです。`entry_time = 15:30` の 40件は
**40件すべてで `official_open == daily_close` かつ `entry_price == exit_price`**。
これは「一日中売買が成立せず引けだけで約定した」形と整合し、単なる
フォールバックのバグでは説明できません。

**実在成分と処理の産物が混ざった列**なので、以下の両方を保留にします。

- 「91.7% が遅れて寄るので板寄せに参加できる」(執行可能の主張)
- 「分足参照に失敗して始値へフォールバックしている」(私が出した実装事故の判定)

決着させるには生の分足を見るしかありません。

```
python gap_reversal_intraday.py --dump-bars 5
```

|ギャップ| 上位の銘柄日について、幻バー除去も時刻の丸めもせずに先頭10本を
出します。読み方:

- **09:00 から出来高0のバーが並び、途中から出来高が付く**
  → 気配のプレースホルダ。除去すると『遅延』に見えるだけ (処理の産物)
- **09:00 台にバーが存在せず、途中から始まる**
  → 板寄せが実際に遅れて成立している (実在)

レポート冒頭の【0】でも、先頭バーの出来高0率・時刻分布・日足始値との一致率を
出すようにしました。**自分のスクリプトが同じ罠を踏んでいないかの検査**でもあります。
