...
# 同日発注数 — その日いくつシグナルが出たか。
#   ★ 前夜に確定する。注文を出す時点で自分が何件出すか分かっているので、
#     そのまま発注ルールにできる(「候補が N 件を超える日は建てない/減らす」)。
#   なぜ効きうるか: 予算400万は1日あたり13件程度しか建てられない。つまり
#   現行運用は既に「シグナルが多い日への露出」を暗黙に抑えている。
#   母集団(全トレード)がマイナスなのに予算シミュがプラスなのは、この切り捨てが
#   効いているからかもしれない。だとすればこれは構造的なエッジで、
#   銘柄選択ではなく『日の選択』が本体ということになる。
_cnt = df.groupby(df["date"].dt.date)["pnl"].transform("size")
df["同日発注数"] = _cnt.astype(float)
