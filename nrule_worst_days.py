...
def _ref_check(sig: pd.DataFrame) -> None:
    ...
    r_all = len(w) / REF["trades"] if REF["trades"] else float("nan")
    if worst >= 1.5:
        # ⚠ 比の数字だけだと数週間で無視されます。**帰結を書くこと。**
        print(f"    ⛔ 比 {r_all:.2f} — **この実装は N を再現していません。**")
        print("       ここで出る数字を N の成績として引用しないこと。")
        print("       使えるのは Δ と発動率 (正規化済み) と、形の相対的な並びだけ。")
        print("       (--select filter-first で比 0.76 まで寄りますが、残り1.3倍は")
        print("        未特定です。§16.15.4 の方針により追跡しません)")
    else:
        print(f"    ✅ 比 {r_all:.2f} — 桁は合っています")
...
# ⛔⛔ **この表は未確認です。そして銘柄区分を無視しています。**
#   TOPIX100 構成銘柄には呼値の刻みが細かい区分があり (建値2,652円で 0.5円 など)、
#   同じ bp のティック数が倍になります。N は「売買代金 上位50」で選ぶので
#   **TOPIX100 が最も濃く入る**母集団です。
#   → ここでのティック換算は **参考値**。実務では発注時に
#     「その銘柄の呼値」を記録して、想定に依存せず出すこと。
_TICK = [(0, 1.0), (3_000, 5.0), (5_000, 10.0), (30_000, 50.0),
         (50_000, 100.0), (100_000, 1_000.0)]


def tick_size(px: float) -> float:
    """⚠ 参考値。銘柄区分を見ていないので TOPIX100 では細かすぎる側に外れます。"""
    t = 1.0
    for lo, v in _TICK:
        if px >= lo:
            t = v
